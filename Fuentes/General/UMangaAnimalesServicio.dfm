inherited FAnimalesServicio: TFAnimalesServicio
  Left = 73
  Top = 77
  Caption = 'FAnimalesServicio'
  ClientHeight = 481
  ClientWidth = 870
  OldCreateOrder = True
  PixelsPerInch = 96
  ...
  object PEncabezado: TPanel [4]
    ...
    object LFinEstimada: TLabel
      Left = 112
      Top = 79
      Width = 63
      Height = 14
      Caption = 'LFinEstimada'
    end
  end
  object PSeleccion: TPanel [5]
    ...
    inline FrameSeleccion: TFrameSeleccion
      ...
      inherited IBQDisponibles: TIBQuery
        AfterOpen = FrameSeleccionIBQDisponiblesAfterOpen
        SQL.Strings = (
          
            'SELECT TA.ID_ANIMAL, TA.ID_RP, CS.NOMBRE FROM COD_SEXOS CS JOIN ' +
            'TAB_ANIMALES TA ON CS.ID_SEXO = TA.SEXO JOIN EVE_EVENTOS EE'
          
            'ON EE.ANIMAL=TA.ID_ANIMAL AND EE.ESTABLECIMIENTO=TA.ESTABLECIMIE' +
            'NTO'
          
            'JOIN EVE_INGRESO_TORO EIT ON EE.ID_EVENTO = EIT.ID_EVENTO WHERE ' +
            'EIT.SERVICIO = :SERVICIO AND TA.ID_ANIMAL NOT IN(SELECT ID_ANIMA' +
            'L FROM AUX_ANIMALES)'
          'UNION'
          
            'SELECT TA.ID_ANIMAL,TA.ID_RP, CS.NOMBRE FROM COD_SEXOS CS JOIN T' +
            'AB_ANIMALES TA ON CS.ID_SEXO = TA.SEXO JOIN EVE_EVENTOS EE'
          
            'ON EE.ANIMAL=TA.ID_ANIMAL AND EE.ESTABLECIMIENTO=TA.ESTABLECIMIE' +
            'NTO'
          
            'JOIN EVE_INGRESO_SERVICIO EIS ON EE.ID_EVENTO = EIS.ID_EVENTO WH' +
            'ERE EIS.SERVICIO = :SERVICIO AND TA.ID_ANIMAL NOT IN(SELECT ID_A' +
            'NIMAL FROM AUX_ANIMALES)')
        ParamData = <
          item
            DataType = ftUnknown
            Name = 'SERVICIO'
      ...
      inherited IBQSeleccionados: TIBQuery
        AfterOpen = FrameSeleccionIBQSeleccionadosAfterOpen
        SQL.Strings = (
          'SELECT ID_ANIMAL, ID_AUX, ID_RP, NOMBRE FROM AUX_ANIMALES ')
      end
    end
  end
  ...
  object IBQNomServicio: TIBQuery
    ...
    SQL.Strings = (
        ...
        'select nombre, fecha_inicio, fecha_fin, fecha_fin_estimada, tipo' +
        ' from tab_servicios where id_servicio = :servicio')
    Left = 600
    Top = 40
    ParamData = <
      item
  ...
  object IBSPEliminarAnimales: TIBStoredProc
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    StoredProcName = 'ELIMINAR_ANIMALES_SERVICIO'
    Left = 552
    Top = 80
  end
  object IBQEliminarServicio: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
        ...
        'delete from tab_servicios where id_servicio = :id and establecim' +
        'iento = :esta')
    Left = 312
    Top = 80
    ParamData = <
