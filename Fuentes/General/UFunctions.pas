...
function calcularCantidadRaciones(InicioS, FinS, B0, B1: TDateTime; racion:integer):integer;

function recuperarIE(cadena: string; delimitador: char):string;
procedure LogMensaje(S: String);
...
var factorSecuenciaVerificador: array[1..16] of integer = (7,6,5,4,3,2,7,6,5,4,3,2,7,6,5,4);
...
begin
  ...
  for i:= 1 to cant do
  begin
    nuevo := IncrementarIdentificadorCUIG(actual);
    actual := nuevo;
  end;
  result:= actual;
end;
...
{Para calculo de CUIG}

  function esLetra(posicion: integer; valor:string):boolean;
  var
    letra: char;
  begin
    letra := valor[posicion];
    if (ord(letra) >= 65) AND (ord(letra) <= 90) then
    begin
      esLetra:= true;
    end
    else
  ...
  function esNumero(posicion: integer; valor:string):boolean;
  var
    letra: char;
  begin
    letra := valor[posicion];
    if (ord(letra) >= 48) AND (ord(letra) <= 57) then
    begin
      esNumero:= true;
    end
    else
...
var
  ...
  intMascara, diferencia, longitud, nuevoValor : integer;
begin
  ...
  begin
    ...
    preCaravana := StringOfChar('0', diferencia) + intToStr(nuevoValor);
  ...
  begin
    generaCaravanaOficial := prefijo + intToStr(nuevoValor);
  end;
...
begin
  ...
  if (RightStr(resultado, 4) = 'Z999') then
  begin
    resultado := LeftStr(resultado, 5) + 'AA00';
  end
  else
  if (RightStr(resultado, 4) = 'ZZ99') then
  begin
    resultado := LeftStr(resultado, 5) + 'AAA0';
  end
  else
  begin
    i := 4;
    carry := 1;
    while ((i <> 0) and (carry = 1)) do
    begin
      if (esNumero(i, resultado)) then
      begin
        if (resultado[i]<>'9') then
        begin
          resultado[i] := chr(ord(resultado[i])+ carry); // Aumenta en 1
          carry := 0;                               // No hay carry
        end
        else
        begin
          resultado[i] := '0';                      // Vuelve a 0
          carry := 1;                               // Hay carry
        end
      end
      else
      begin
        if (resultado[i]<>'Z') then
        begin
          resultado[i] := chr(ord(resultado[i])+carry); // Aumenta en 1
          carry := 0;                               // No hay carry
        end
...
begin
  Result :=
    (Length(Valor) = 9) AND
    CheckFormatoCUIGEncabezado(Valor) and
    CheckFormatoCUIGUltimoIdent(valor);
end;

function CheckFormatoCUIGEncabezado(Valor : String) : Boolean;
begin
  Result :=         esLetra(1, Valor) and
        esLetra(2, Valor) and
        esNumero(3, Valor) and
        esNumero(4, Valor) and
        esNumero(5, Valor);
end;

function CheckFormatoCUIGUltimoIdent(Valor : String) : Boolean;
begin
  Result := (esLetra(1, Valor) and esNumero(2, Valor) and esNumero(3, Valor) and esNumero(4, Valor)) or
       (esLetra(1, Valor) and esLetra(2, Valor) and esNumero(3, Valor) and esNumero(4, Valor)) or
       (esLetra(1, Valor) and esLetra(2, Valor) and esLetra(3, Valor) and esNumero(4, Valor));
end;

{
Para calcular la cantidad de raciones, hay que ver como se interceptan los periodos
[   ]: Periodo de suplementacion: InicioS, FinS  (Pueden haber varios)
(   ): periodo de busqueda o filtrado: B0, B1
Caso 1: [  (     )  ]
Caso 2: (  [     ]  )
Caso 3: [  (     ]  )
Caso 4: (  [     )  ]
...
Por ejemplo en el Caso 1, la busqueda esta totalmente dentro de la suplementacion
En el caso 2, la suplementacion esta dentro de la busqueda
En el 3 y 4 tienen un tiempo en comun
En el caso 5 no tienen tiempo en comun
}
function calcularCantidadRaciones(InicioS, FinS, B0, B1: TDateTime; racion:integer):integer;
var
  delta: integer;
begin
  // Caso 1
  if (CompareDate(B0, InicioS) >= 0) AND (CompareDate(B1,FinS) <= 0) then
  begin
    delta := DaysBetween(B1, B0);
  end;

  // Caso 2
  if (CompareDate(B0, InicioS) <= 0) AND (CompareDate(B1, FinS) >= 0) then
  begin
    delta := DaysBetween(FinS, InicioS);
  end;

  // Caso 3
  if (CompareDate(InicioS,B0) <= 0) AND (CompareDate(FinS,B0) >= 0)
      AND (CompareDate(FinS,B1) <= 0) then
  begin
    delta := DaysBetween(FinS, B0);
  end;

  // Caso 4
  if (CompareDate(B0, InicioS) <= 0) AND (CompareDate(InicioS,B1) <= 0)
    AND (CompareDate(FinS,B1) >= 0) then
  begin
    delta := DaysBetween(B1, InicioS);
  end;

  // Caso 5
  if (CompareDate(FinS, B0) <= 0) then
  begin
    delta := 0;
  end;

  calcularCantidadRaciones := delta * racion;
end;
