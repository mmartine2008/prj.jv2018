...
/////////////////////////////////////////////////
  ...
  TFMapaGoogle = class(TFUniversal)
    ...
    Label75: TLabel;
    IBSPFixMapasPerdidos: TIBStoredProc;
  ...
  private
    ...
    function noRepetido(const potrero: integer; const arreglo: TArrInfo): boolean;
  public
    TerminoMapa: Boolean;
...
begin
try
  TimerUpdateValues.enabled := false;
  if WB.Browser <> nil then
  begin
...
begin
  try
    ...
    begin
      ...
      MostrarMensaje(tmAdvertencia, 'Huella tuvo un problema de conexión con la BD');
    end;
  end;
end;
...
begin
  ...
  begin
    try
      ...
      IBQCoordenadas.Post;
      //FPrincipal.IBTPrincipal.CommitRetaining;         //SACADO PORQUE SI FALLABA, ME BLOQUEABA EL POTRERO Y LUEGO NO PODIA EDITAR
    except
      begin
        noFallo:=False;
        //FPrincipal.IBTPrincipal.RollbackRetaining;
  ...
  if(noFallo) then              // No hubo problemas, y guardo toda la info en la BD. Sino, las
    begin
      FPrincipal.IBTPrincipal.CommitRetaining;
    end
  else
    begin                                                   // Sino, tablas quedaron vacias, por culpa del primer comando ejecutado acá: EliminarCoordenadas
        FPrincipal.IBTPrincipal.RollbackRetaining;          // Pero al salir de esta llamada, en TimerFinalizarTimer, llama a la funcion cargar los datos
        MostrarMensaje(tmAdvertencia, 'Huella tuvo un problema de conexión, deberá dibujar nuevamente'); // que se le agrego una tarea nueva: Recuperar los datos
    end;                                                    // Y se deja todo tal cual estaba antes de la modificación.
end;
...
begin
  ...
  begin
    if S[I] <> '/' then
    begin
      if S[I] <> ';' then                         ///DE ACA SACA LA INFO PARA TAB_COORDENADAS
        Aux := Aux + S[I]
      else
      begin
...
// leo las propiedades
  ...
  // 16 Set 2016
  // Error en Windows 10 al invocar este mensaje
  if (Sender as TJvImage).State = stDefault then
  begin
  ...
  showMessage( ' Error al enviar comando ');
end;
end;
...
function TFMapaGoogle.noRepetido(const potrero: integer; const arreglo: TArrInfo): boolean;
var
  i: integer;
begin
  if(Low(arreglo) > 0) then
    begin
      for i:= Low(arreglo) to High(arreglo) do
        begin
          if potrero = arreglo[i].indice then
            begin
              result := False;
              break;
            end;
...
var
  ...
  Img : TJvImage;
  NextTop : Integer;
  cAuxStroke, cAuxFill,jssScript : String;
  potrero: Integer;
  lat_decimal: double;
  lng_decimal: double;
begin
  //Esto intenta recuperar la información de mapas perdida
  IBSPFixMapasPerdidos.Prepare;
  with IBSPFixMapasPerdidos do
    begin
      //InicIndices;
      Execproc;
      if(IBSPFixMapasPerdidos.ParamByName('POTCOO').AsInteger > 0) then   // SI el SP devuelve que hay potreros rotos, quiere decir que no pudo recuperar la informacion desde los Logs
        begin                                                             // Y se regeneran los datos, lo unico que se pierde son los colorcitos.
          IBQInfo.Close;
          IBQInfo.ParamByName('esta').AsInteger := IBSPFixMapasPerdidos.ParamByName('ESTABLECIMIENTO').AsInteger;
          IBQInfo.Open;
          IBQInfo.Insert;
          IBQInfo.FieldByName('potrero').AsInteger := IBSPFixMapasPerdidos.ParamByName('POTCOO').AsInteger;
          IBQInfo.FieldByName('establecimiento').AsInteger := IBSPFixMapasPerdidos.ParamByName('ESTABLECIMIENTO').AsInteger;
          IBQInfo.FieldByName('nombre').AsString := IBSPFixMapasPerdidos.ParamByName('NOMBRE').AsString;                            //TO DO:RECUPERAR EL NOMRE ORIGINAL
          IBQInfo.FieldByName('info').AsString := StringReplace(ActualizarDatosPotrero(IBSPFixMapasPerdidos.ParamByName('POTCOO').AsInteger),''#$D#$A'','<br>',[rfReplaceAll]);
          IBQInfo.FieldByName('stroke_color').AsString := '#000000';
          IBQInfo.FieldByName('stroke_weigth').AsString := '3';
          IBQInfo.FieldByName('stroke_opacity').AsString := '0.7';
          IBQInfo.FieldByName('fill_color').AsString := '#ffd700';
          IBQInfo.FieldByName('fill_opacity').AsString := '0.7';
          IBQInfo.Post;
          FPrincipal.IBTPrincipal.CommitRetaining;
        end;
    end;
...
//    end;
  ...
  while not IBQInfo.Eof do
    begin
      ...
      if ((IBQInfo.FieldValues['stroke_color'] <> NULL) and (noRepetido(IBQInfo.FieldValues['potrero'], Informacion))) THEN
        BEGIN
          ...
          Informacion[I].StrokeColor := VarToStr(IBQInfo.FieldValues['stroke_color']);         // PARA EVITAR QUE SE ROMPA, en el ultimo de los casos
          if ((Informacion[I].indice < 5000) and (Informacion[I].StrokeColor <> '')) then      // Porque si no esta esta info, o sea si da NULL y no '', se rompe
          begin
...
procedure TFMapaGoogle.ActInfoPotrero(id_potrero : Integer);   ///NO SIRVE EL PARAMETRO; NO LO USA NUNCA.
var
  I : Integer;
