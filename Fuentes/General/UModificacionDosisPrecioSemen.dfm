inherited FModificacionDosisPrecioSemen: TFModificacionDosisPrecioSemen
  Left = 145
  Top = 135
  Caption = 'FModificacionDosisPrecioSemen'
  ClientHeight = 461
  ClientWidth = 824
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 824
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 440
  end
  object GBSuper: TGroupBox [4]
    Left = 0
    Top = 65
    Width = 824
    Height = 112
    Align = alTop
    Caption = '  Datos del Animal  '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    object LNombre: TLabel
      Left = 15
      Top = 48
      Width = 54
      Height = 16
      Caption = 'Nombre:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LNombreValue: TLabel
      Left = 70
      Top = 48
      Width = 250
      Height = 16
      AutoSize = False
      Caption = 'LNombreValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LApodo: TLabel
      Left = 15
      Top = 72
      Width = 45
      Height = 16
      Caption = 'Apodo:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LApodoValue: TLabel
      Left = 70
      Top = 72
      Width = 250
      Height = 16
      AutoSize = False
      Caption = 'LApodoValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LRp: TLabel
      Left = 15
      Top = 24
      Width = 21
      Height = 16
      Caption = 'Rp:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LRpValue: TLabel
      Left = 70
      Top = 24
      Width = 250
      Height = 16
      AutoSize = False
      Caption = 'LRpValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LHba: TLabel
      Left = 330
      Top = 72
      Width = 31
      Height = 16
      Caption = 'HBA:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LHbaValue: TLabel
      Left = 370
      Top = 72
      Width = 200
      Height = 16
      AutoSize = False
      Caption = 'LHbaValue'
      Color = clSilver
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LAba: TLabel
      Left = 330
      Top = 72
      Width = 31
      Height = 16
      Caption = 'ABA:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LAbaValue: TLabel
      Left = 370
      Top = 72
      Width = 200
      Height = 16
      AutoSize = False
      Caption = 'LAbaValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LFechaNac: TLabel
      Left = 584
      Top = 24
      Width = 73
      Height = 33
      AutoSize = False
      Caption = 'Fecha de Nacimiento'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      WordWrap = True
    end
    object LFechaNacValue: TLabel
      Left = 664
      Top = 32
      Width = 99
      Height = 16
      Caption = 'LFechaNacValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LRaza: TLabel
      Left = 330
      Top = 24
      Width = 36
      Height = 16
      Caption = 'Raza:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LRazaValue: TLabel
      Left = 370
      Top = 24
      Width = 200
      Height = 16
      AutoSize = False
      Caption = 'LRazaValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LColor: TLabel
      Left = 330
      Top = 48
      Width = 38
      Height = 16
      Caption = 'Color:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LColorValue: TLabel
      Left = 370
      Top = 48
      Width = 200
      Height = 16
      AutoSize = False
      Caption = 'LColorValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object LTipoRegistro: TLabel
      Left = 584
      Top = 64
      Width = 52
      Height = 32
      Caption = 'Tipo de Registro'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      WordWrap = True
    end
    object LTipoRegistroValue: TLabel
      Left = 664
      Top = 72
      Width = 150
      Height = 16
      AutoSize = False
      Caption = 'LTipoRegistroValue'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label1: TLabel
      Left = 657
      Top = 33
      Width = 4
      Height = 18
      Caption = ':'
    end
    object Label2: TLabel
      Left = 657
      Top = 72
      Width = 4
      Height = 18
      Caption = ':'
    end
  end
  object GBCentral: TGroupBox [5]
    Left = 0
    Top = 177
    Width = 824
    Height = 173
    Align = alClient
    Caption = '  Datos a Modificar  '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
    object LDosis: TLabel
      Left = 330
      Top = 72
      Width = 160
      Height = 16
      Caption = 'Nueva cantidad de Dosis:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LPrecioDosis: TLabel
      Left = 330
      Top = 110
      Width = 161
      Height = 16
      Caption = 'Nuevo Precio/Dosis:       $'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LFecha: TLabel
      Left = 330
      Top = 34
      Width = 149
      Height = 16
      Caption = 'Fecha de modificaci'#243'n:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LDosisActuales: TLabel
      Left = 15
      Top = 72
      Width = 95
      Height = 16
      Caption = 'Dosis Actuales:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LDosisActualesValue: TLabel
      Left = 144
      Top = 72
      Width = 60
      Height = 16
      Alignment = taCenter
      AutoSize = False
      Caption = 'LDosisActualesValue'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object LPrecioDosisActual: TLabel
      Left = 15
      Top = 110
      Width = 126
      Height = 16
      Caption = 'Precio/Dosis Actual:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LPrecioDosisActualValue: TLabel
      Left = 143
      Top = 110
      Width = 60
      Height = 16
      Alignment = taCenter
      AutoSize = False
      Caption = 'LPrecioDosisActualValue'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object DTPFechaModif: TDateTimePicker
      Left = 492
      Top = 31
      Width = 121
      Height = 24
      Date = 40982.434103298610000000
      Time = 40982.434103298610000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object JvEDosis: TJvEdit
      Left = 492
      Top = 68
      Width = 121
      Height = 24
      Alignment = taCenter
      Modified = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Text = 'JvEDosis'
    end
    object JvEPrecio: TJvEdit
      Left = 492
      Top = 107
      Width = 121
      Height = 24
      Alignment = taCenter
      Modified = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      Text = 'JvEPrecio'
      OnExit = JvEPrecioExit
      OnKeyPress = JvEPrecioKeyPress
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 350
    Width = 824
    Height = 70
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      824
      70)
    object IFondoB: TImage
      Left = 0
      Top = 0
      Width = 824
      Height = 70
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ITerminar: TImage
      Left = 240
      Top = -1
      Width = 51
      Height = 51
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        EEDA8A4CE0BD770000000049454E44AE426082}
      OnClick = ITerminarClick
    end
    object LTerminar: TLabel
      Left = 239
      Top = 46
      Width = 50
      Height = 15
      Cursor = crHandPoint
      Caption = 'Terminar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ITerminarClick
    end
    object LCancelar: TLabel
      Left = 492
      Top = 46
      Width = 50
      Height = 15
      Cursor = crHandPoint
      Caption = 'Cancelar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ICancelarClick
    end
    object ICancelar: TImage
      Left = 494
      Top = -2
      Width = 46
      Height = 57
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        C70000000049454E44AE426082}
      OnClick = ICancelarClick
    end
    object BBCancelar: TBitBtn
      Left = 432
      Top = 71
      Width = 81
      Height = 27
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = 'Cancelar'
      ModalResult = 2
      TabOrder = 1
      Glyph.Data = {
        
        CBFDFDFDFFFFFFFFFFFF}
    end
    object BBAceptar: TBitBtn
      Left = 346
      Top = 71
      Width = 81
      Height = 27
      Anchors = [akTop, akRight]
      Caption = 'Aceptar'
      Default = True
      ModalResult = 1
      TabOrder = 0
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 71
      Width = 32
      Height = 27
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
  end
  inherited PTitulo: TPanel
    Width = 824
    Caption = 'Modificar de Precios/Dosis'
    inherited LTitulo: TLabel
      Width = 420
      Caption = 'Modificar de Precios/Dosis'
    end
    inherited PILeft: TPanel
      Left = 616
    end
  end
  inherited Psocalo: TPanel
    Top = 439
    Width = 824
    inherited JvSocalo: TJvImage
      Width = 824
    end
  end
  inherited ALUniversal: TActionList
    Left = 731
    Top = 16
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 636
    Top = 47
  end
  inherited SDXLS: TSaveDialog
    Left = 731
    Top = 46
  end
  inherited DTXLS: TDataToXLS
    Left = 699
    Top = 47
  end
  inherited WinXP1: TWinXP
    Left = 699
    Top = 16
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 604
    Top = 15
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 636
    Top = 15
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 668
    Top = 15
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 572
    Top = 15
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 604
    Top = 47
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 572
    Top = 47
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 668
    Top = 46
  end
  inherited PLVersion: TPngImageList
    Left = 760
    Top = 16
  end
  object IBQDatosAnimalInterno: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from tab_animales where id_animal = :id and establecimi' +
        'ento = :esta')
    Left = 744
    Top = 97
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosAnimalExterno: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from tab_animales_externos where id_animal_externo = :i' +
        'd and establecimiento = :esta')
    Left = 744
    Top = 137
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQRazas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_razas where id_raza = :raza')
    Left = 744
    Top = 192
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
  end
  object IBQColor: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_colores where id_color = :color')
    Left = 744
    Top = 224
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'color'
        ParamType = ptUnknown
      end>
  end
  object IBQSubcategoria: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from cod_subcategorias where id_subcategoria = :subcate' +
        'goria')
    Left = 744
    Top = 256
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'subcategoria'
        ParamType = ptUnknown
      end>
  end
  object IBQModificarDatos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'execute procedure SP_MODIFICAR_DOSIS_PRECIO_SEMEN(:ESTABLECIMIEN' +
        'TO, :ID_ANIMAL, :FECHA, :DOSIS, :PRECIO, :TIPO_CAMBIO)')
    Left = 744
    Top = 337
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ESTABLECIMIENTO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ID_ANIMAL'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DOSIS'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'PRECIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'TIPO_CAMBIO'
        ParamType = ptUnknown
      end>
  end
end
