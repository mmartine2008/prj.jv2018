      ...
      begin
        ...
        begin
          if(Peso >= PesoMin) and (Peso <= PesoMax ) then
            ...
            MostrarMensaje(tmError, Traducir('El Peso debe estar entre ')+FloatToStr(PesoMin)+' y '+FloatToStr(PesoMax)+#13+Traducir(' SegÃºn los ParÃ¡metros del Establecimiento'));
        end
        else
          MostrarMensaje(tmError, 'El Valor ingresado no corresponde a un peso vÃ¡lido');
      end;
...
inherited;
  ...
  {if DBLCBADias.ListSource.DataSet.Lookup('VALOR',getDato('DÃ­as',anim[i]),'ID_CODIGO')<>null then
    IBDSSimple.FieldByName('CODDIAS').AsString:=DBLCBADias.ListSource.DataSet.Lookup('VALOR',getDato('DÃ­as',anim[i]),'ID_CODIGO')
  else
    IBDSSimple.FieldByName('CODDIAS').AsString:=getDato('DÃ­as',anim[i]);  }
