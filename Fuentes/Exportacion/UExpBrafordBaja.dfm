inherited FExpBrafordBaja: TFExpBrafordBaja
  Left = 187
  Top = 190
  Caption = 'Declaraci'#243'n de baja'
  ClientHeight = 617
  ClientWidth = 861
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 861
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 477
  end
  object PBusqueda: TPanel [4]
    Left = 0
    Top = 65
    Width = 861
    Height = 106
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object LDesde: TLabel
      Left = 19
      Top = 18
      Width = 45
      Height = 16
      Caption = 'Desde: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object LHasta: TLabel
      Left = 19
      Top = 61
      Width = 42
      Height = 16
      Caption = 'Hasta: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object IVerAnimales: TImage
      Left = 716
      Top = 12
      Width = 85
      Height = 65
      Cursor = crHandPoint
      Picture.Data = {
        
        00000049454E44AE426082}
      OnClick = BMostrarDatosClick
    end
    object Label2: TLabel
      Left = 705
      Top = 71
      Width = 109
      Height = 16
      Cursor = crHandPoint
      Caption = 'Mostrar animales'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = BMostrarDatosClick
    end
    object DTPDesde: TDateTimePicker
      Left = 64
      Top = 15
      Width = 92
      Height = 24
      Cursor = crHandPoint
      Date = 40612.433659212960000000
      Time = 40612.433659212960000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DTPHasta: TDateTimePicker
      Left = 64
      Top = 58
      Width = 92
      Height = 24
      Cursor = crHandPoint
      Date = 40612.433659212960000000
      Time = 40612.433659212960000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
    object BMostrarDatos: TButton
      Left = 752
      Top = 80
      Width = 105
      Height = 25
      Caption = 'Mostrar datos'
      TabOrder = 2
      Visible = False
      OnClick = BMostrarDatosClick
    end
    object CBPotrero: TComboBox
      Left = 196
      Top = 16
      Width = 200
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 3
      OnChange = CBPotreroChange
      OnCloseUp = CBPotreroCloseUp
    end
    object CBRodeo: TComboBox
      Left = 196
      Top = 56
      Width = 200
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 4
      OnChange = CBRodeoChange
      OnCloseUp = CBRodeoCloseUp
    end
    object CBTipoRegistro: TComboBox
      Left = 440
      Top = 16
      Width = 200
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 5
      OnChange = CBTipoRegistroChange
      OnCloseUp = CBTipoRegistroCloseUp
    end
  end
  object PGrilla: TPanel [5]
    Left = 0
    Top = 171
    Width = 861
    Height = 330
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object GroupBox1: TGroupBox
      Left = 1
      Top = 1
      Width = 859
      Height = 328
      Align = alClient
      Caption = '  Datos de animales  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object Label1: TLabel
        Left = 2
        Top = 18
        Width = 855
        Height = 19
        Align = alTop
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object LCantidades: TLabel
        Left = 2
        Top = 37
        Width = 855
        Height = 16
        Align = alTop
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Layout = tlCenter
      end
      object BDBGGrilla: TBitDBGrid
        Left = 2
        Top = 53
        Width = 855
        Height = 273
        Align = alClient
        DataSource = DSDatosBaja
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'Arial'
        TitleFont.Style = [fsBold]
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'RP_ANIMAL'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Rp Animal'
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'ABA_ANIMAL'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'ABA'
            Width = 210
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'TIPO_BAJA'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Motivo'
            Width = 210
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_BAJA'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha de baja'
            Width = 200
            Visible = True
          end>
      end
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 501
    Width = 861
    Height = 94
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      861
      94)
    object Image5: TImage
      Left = 0
      Top = 0
      Width = 861
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 584
      Top = 8
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        7F10006F91DD97510000000049454E44AE426082}
      OnClick = ISalirClick
    end
    object IExcel: TImage
      Left = 232
      Top = 11
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        AE426082}
      OnClick = IExcelClick
    end
    object LExcel: TLabel
      Left = 213
      Top = 64
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = IExcelClick
    end
    object LSalir: TLabel
      Left = 598
      Top = 64
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ISalirClick
    end
    object BitBtn3: TBitBtn
      Left = 1342
      Top = 8
      Width = 97
      Height = 25
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
  end
  inherited PTitulo: TPanel
    Width = 861
    Caption = 'Declaraci'#243'n de bajas'
    inherited LTitulo: TLabel
      Width = 457
      Caption = 'Declaraci'#243'n de bajas'
    end
    inherited PILeft: TPanel
      Left = 653
    end
  end
  inherited Psocalo: TPanel
    Top = 595
    Width = 861
    inherited JvSocalo: TJvImage
      Width = 861
    end
  end
  inherited ALUniversal: TActionList
    Left = 619
    Top = 16
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 588
    Top = 15
  end
  inherited SDXLS: TSaveDialog
    Left = 685
    Top = 46
  end
  inherited DTXLS: TDataToXLS
    Left = 685
    Top = 15
  end
  inherited WinXP1: TWinXP
    Left = 620
    Top = 48
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 556
    Top = 47
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 556
    Top = 15
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 652
    Top = 15
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 652
    Top = 47
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 524
    Top = 15
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Top = 47
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 589
    Top = 47
  end
  inherited PLVersion: TPngImageList
    Left = 720
    Top = 16
  end
  object DSDatosBaja: TDataSource
    DataSet = IBQDatosBaja
    Left = 8
    Top = 464
  end
  object IBQDatosBaja: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * '
      
        'FROM REP_EXP_DATOS_BAJAS_BRAFORD(:ESTA,:DESDE,:HASTA,:POTRERO,:R' +
        'ODEO,:TIPOREG)')
    Left = 40
    Top = 464
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ESTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DESDE'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'HASTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'POTRERO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'RODEO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'TIPOREG'
        ParamType = ptUnknown
      end>
  end
  object IBQCantVenta: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select count(*) as cant'
      ''
      
        'from tab_animales ta join eve_eventos ee on (ta.id_animal = ee.a' +
        'nimal)'
      'join eve_baja eb on (ee.id_evento = eb.id_evento)'
      ''
      'where ((ta.activo = '#39'N'#39')'
      ''
      'and (ta.establecimiento = :esta) and (ta.raza = 11)'
      ''
      'and (ee.fecha >= :desde) and (ee.fecha <= :hasta)'
      ''
      'and (ee.tipo = 3) and (eb.tipo = 1))')
    Left = 129
    Top = 467
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end>
  end
  object IBQCantMuerte: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select count(*) as cant'
      ''
      
        'from tab_animales ta join eve_eventos ee on (ta.id_animal = ee.a' +
        'nimal)'
      'join eve_baja eb on (ee.id_evento = eb.id_evento)'
      ''
      'where ((ta.activo = '#39'N'#39')'
      ''
      'and (ta.establecimiento = :esta) and (ta.raza = 11)'
      ''
      'and (ee.fecha >= :desde) and (ee.fecha <= :hasta)'
      ''
      'and (ee.tipo = 3) and (eb.tipo = 3))')
    Left = 161
    Top = 467
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end>
  end
  object IBQPotreros: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select id_potrero, nombre from tab_potreros where establecimient' +
        'o = :estable')
    Left = 312
    Top = 73
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'estable'
        ParamType = ptUnknown
      end>
  end
  object IBQRodeo: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select id_rodeo, nombre from tab_rodeos where establecimiento = ' +
        ':estable')
    Left = 312
    Top = 129
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'estable'
        ParamType = ptUnknown
      end>
  end
  object IBQTiposRegistro: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select cs.* '
      ''
      
        'from cod_razas cr join rel_subcategoria_raza rsr on cr.id_raza =' +
        ' rsr.id_raza join cod_subcategorias cs on cs.id_subcategoria = r' +
        'sr.id_subcategoria'
      ''
      'where (cr.visible = 1) and (cr.id_raza = :raza)')
    Left = 448
    Top = 73
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
  end
end
