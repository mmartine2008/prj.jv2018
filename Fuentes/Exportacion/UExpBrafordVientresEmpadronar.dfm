inherited FExpBrafordVientresEmpadronar: TFExpBrafordVientresEmpadronar
  Left = 186
  Top = 22
  Caption = 'Vientres Adultos a Empadronar'
  ClientHeight = 672
  ClientWidth = 826
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 826
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 442
  end
  object Panel1: TPanel [4]
    Left = 0
    Top = 65
    Width = 826
    Height = 120
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object Label1: TLabel
      Left = 19
      Top = 26
      Width = 45
      Height = 16
      Caption = 'Desde: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 19
      Top = 69
      Width = 42
      Height = 16
      Caption = 'Hasta: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object IVerAnimales: TImage
      Left = 635
      Top = 16
      Width = 85
      Height = 65
      Cursor = crHandPoint
      Picture.Data = {
        
        00000049454E44AE426082}
      OnClick = BMostrarDatosClick
    end
    object Label4: TLabel
      Left = 626
      Top = 75
      Width = 109
      Height = 16
      Cursor = crHandPoint
      Caption = 'Mostrar animales'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = BMostrarDatosClick
    end
    object DTPDesde: TDateTimePicker
      Left = 64
      Top = 23
      Width = 97
      Height = 24
      Cursor = crHandPoint
      Date = 40612.528859224520000000
      Time = 40612.528859224520000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnChange = DTPDesdeChange
    end
    object DTPHasta: TDateTimePicker
      Left = 64
      Top = 63
      Width = 97
      Height = 24
      Cursor = crHandPoint
      Date = 40612.528925057870000000
      Time = 40612.528925057870000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnChange = DTPHastaChange
    end
    object BMostrarDatos: TButton
      Left = 728
      Top = 88
      Width = 121
      Height = 25
      Caption = 'Mostrar datos'
      TabOrder = 2
      Visible = False
      OnClick = BMostrarDatosClick
    end
    object CBCategoria: TComboBox
      Left = 198
      Top = 23
      Width = 180
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 3
      OnChange = CBCategoriaChange
      OnCloseUp = CBCategoriaCloseUp
    end
    object CBInspeccion: TComboBox
      Left = 198
      Top = 63
      Width = 180
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 4
      OnChange = CBInspeccionChange
      OnCloseUp = CBInspeccionCloseUp
    end
    object CBTipoRegistro: TComboBox
      Left = 416
      Top = 23
      Width = 180
      Height = 24
      Cursor = crHandPoint
      Style = csDropDownList
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 5
      OnChange = CBTipoRegistroChange
      OnCloseUp = CBTipoRegistroCloseUp
    end
  end
  object PGrilla: TPanel [5]
    Left = 0
    Top = 185
    Width = 826
    Height = 371
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object GBGrilla: TGroupBox
      Left = 1
      Top = 1
      Width = 824
      Height = 369
      Align = alClient
      Caption = '  Datos de animales  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object Label3: TLabel
        Left = 2
        Top = 18
        Width = 820
        Height = 17
        Align = alTop
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object LCantidad: TLabel
        Left = 2
        Top = 35
        Width = 820
        Height = 18
        Align = alTop
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object BDBGGrillaDatos: TBitDBGrid
        Left = 2
        Top = 53
        Width = 820
        Height = 314
        Align = alClient
        DataSource = DSDatosGrilla
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -15
        TitleFont.Name = 'Arial'
        TitleFont.Style = []
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'RP'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Rp Animal'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 180
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'Registro'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 180
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_NAC'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha Nacimiento'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 130
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_EVE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha Inspecci'#243'n'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 130
            Visible = True
          end>
      end
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 556
    Width = 826
    Height = 94
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      826
      94)
    object Image5: TImage
      Left = 0
      Top = 0
      Width = 826
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 536
      Top = 9
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        7F10006F91DD97510000000049454E44AE426082}
      OnClick = ISalirClick
    end
    object IExcel: TImage
      Left = 136
      Top = 11
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        AE426082}
      OnClick = IExcelClick
    end
    object LExcel: TLabel
      Left = 117
      Top = 64
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = IExcelClick
    end
    object LSalir: TLabel
      Left = 550
      Top = 64
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ISalirClick
    end
    object BitBtn3: TBitBtn
      Left = 1307
      Top = 8
      Width = 97
      Height = 25
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
  end
  inherited PTitulo: TPanel
    Width = 826
    Caption = 'Vientres Adultos a Empadronar'
    inherited LTitulo: TLabel
      Width = 422
      Caption = 'Vientres Adultos a Empadronar'
    end
    inherited PILeft: TPanel
      Left = 618
    end
  end
  inherited Psocalo: TPanel
    Top = 650
    Width = 826
    inherited JvSocalo: TJvImage
      Width = 826
    end
  end
  inherited ALUniversal: TActionList
    Left = 787
    Top = 16
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 756
    Top = 79
  end
  inherited SDXLS: TSaveDialog
    Left = 789
    Top = 79
  end
  inherited DTXLS: TDataToXLS
    Left = 789
    Top = 111
  end
  inherited WinXP1: TWinXP
    Left = 788
    Top = 48
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 756
    Top = 15
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 724
    Top = 111
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 724
    Top = 47
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 756
    Top = 47
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 724
    Top = 79
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 724
    Top = 15
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 756
    Top = 111
  end
  object DSDatosGrilla: TDataSource
    DataSet = IBQGetVientres
    Left = 8
    Top = 456
  end
  object IBQGetVientres: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetVientresCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select distinct ta.id_rp as rp, ta.fecha_nacimiento as fecha_nac' +
        ', ee.fecha as fecha_eve, ta.subcategoria as subcategoria'
      ''
      
        'from eve_eventos ee join tab_animales ta on (ee.animal = ta.id_a' +
        'nimal)'
      ''
      
        'where (ee.fecha >= :desde) and (ee.fecha <= :hasta) and (ee.tipo' +
        ' = 44) '
      ''
      'and (ta.establecimiento = :esta) and (ta.inspeccion is not null)')
    Left = 40
    Top = 456
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
    object IBQGetVientresRP: TIBStringField
      FieldName = 'RP'
      Origin = 'TAB_ANIMALES.ID_RP'
      Required = True
      Size = 10
    end
    object IBQGetVientresFECHA_NAC: TDateField
      FieldName = 'FECHA_NAC'
      Origin = 'TAB_ANIMALES.FECHA_NACIMIENTO'
      Required = True
    end
    object IBQGetVientresFECHA_EVE: TDateField
      FieldName = 'FECHA_EVE'
      Origin = 'EVE_EVENTOS.FECHA'
      Required = True
    end
    object IBQGetVientresRegistro: TStringField
      FieldKind = fkCalculated
      FieldName = 'Registro'
      Calculated = True
    end
    object IBQGetVientresSUBCATEGORIA: TIntegerField
      FieldName = 'SUBCATEGORIA'
      Origin = 'TAB_ANIMALES.SUBCATEGORIA'
    end
  end
  object IBQCategorias: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_categorias where tipo_sexo = 2')
    Left = 328
    Top = 72
  end
  object IBQRegistroNombre: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select nombre from cod_subcategorias where id_subcategoria = :id' +
        '_subcate')
    Left = 120
    Top = 456
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_subcate'
        ParamType = ptUnknown
      end>
  end
  object IBQTiposInspeccion: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_inspeccion')
    Left = 328
    Top = 136
  end
  object IBQTipoRegistro: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select cs.* '
      ''
      
        'from cod_razas cr join rel_subcategoria_raza rsr on cr.id_raza =' +
        ' rsr.id_raza join cod_subcategorias cs on cs.id_subcategoria = r' +
        'sr.id_subcategoria'
      ''
      'where (cr.visible = 1) and (cr.id_raza = :raza)')
    Left = 544
    Top = 73
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
  end
end
