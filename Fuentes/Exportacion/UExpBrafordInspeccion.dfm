inherited FExpBrafordInspeccion: TFExpBrafordInspeccion
  Left = 48
  Top = 14
  Caption = 'Inspecci'#243'n'
  ClientHeight = 673
  ClientWidth = 961
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 961
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 577
  end
  object Panel1: TPanel [4]
    Left = 0
    Top = 65
    Width = 961
    Height = 128
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object BMostrarDatos: TButton
      Left = 848
      Top = 12
      Width = 105
      Height = 25
      Caption = 'Mostrar datos'
      TabOrder = 0
      Visible = False
      OnClick = BMostrarDatosClick
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 1
      Width = 959
      Height = 126
      Align = alClient
      Caption = '  Selecci'#243'n de preferencias  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object LDesde: TLabel
        Left = 19
        Top = 36
        Width = 45
        Height = 16
        Caption = 'Desde: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object LHasta: TLabel
        Left = 19
        Top = 79
        Width = 42
        Height = 16
        Caption = 'Hasta: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object IVerAnimales: TImage
        Left = 800
        Top = 24
        Width = 85
        Height = 65
        Cursor = crHandPoint
        Picture.Data = {
          
          00000049454E44AE426082}
        OnClick = BMostrarDatosClick
      end
      object Label2: TLabel
        Left = 793
        Top = 83
        Width = 109
        Height = 16
        Cursor = crHandPoint
        Caption = 'Mostrar animales'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        OnClick = BMostrarDatosClick
      end
      object DTPDesde: TDateTimePicker
        Left = 64
        Top = 33
        Width = 97
        Height = 24
        Cursor = crHandPoint
        Date = 40611.668969178240000000
        Time = 40611.668969178240000000
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnChange = DTPDesdeChange
      end
      object DTPHasta: TDateTimePicker
        Left = 64
        Top = 76
        Width = 97
        Height = 24
        Cursor = crHandPoint
        Date = 40611.669223090280000000
        Time = 40611.669223090280000000
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnChange = DTPHastaChange
      end
      object CBPotrero: TComboBox
        Left = 213
        Top = 33
        Width = 200
        Height = 24
        Cursor = crHandPoint
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ItemHeight = 16
        ParentFont = False
        TabOrder = 2
        OnChange = CBPotreroChange
        OnCloseUp = CBPotreroCloseUp
      end
      object CBRodeo: TComboBox
        Left = 213
        Top = 76
        Width = 200
        Height = 24
        Cursor = crHandPoint
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ItemHeight = 16
        ParentFont = False
        TabOrder = 3
        OnChange = CBRodeoChange
        OnCloseUp = CBRodeoCloseUp
      end
      object CBTipoRegistro: TComboBox
        Left = 464
        Top = 33
        Width = 200
        Height = 24
        Cursor = crHandPoint
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ItemHeight = 16
        ParentFont = False
        TabOrder = 4
        OnChange = CBTipoRegistroChange
        OnCloseUp = CBTipoRegistroCloseUp
      end
    end
  end
  object PDatosInspeccion: TPanel [5]
    Left = 0
    Top = 193
    Width = 961
    Height = 364
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object GBDatosInspeccion: TGroupBox
      Left = 1
      Top = 1
      Width = 959
      Height = 362
      Align = alClient
      Caption = '  Datos de inspecci'#243'n   '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object LCantAnimales: TLabel
        Left = 2
        Top = 18
        Width = 955
        Height = 25
        Align = alTop
        AutoSize = False
        Caption = 'LCantAnimales'
        Layout = tlCenter
      end
      object BDBGGrilla: TBitDBGrid
        Left = 2
        Top = 43
        Width = 955
        Height = 317
        Align = alClient
        DataSource = DSDatosInspeccion
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'Arial'
        TitleFont.Style = []
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'RP_ANIMAL'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Rp Animal'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_NAC'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha de Nacimiento'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'SEXO_NOMBRE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Sexo'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'TIPO_REG_ANIMAL'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Tipo de Registro'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end>
      end
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 557
    Width = 961
    Height = 94
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      961
      94)
    object Image5: TImage
      Left = 0
      Top = 0
      Width = 961
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 656
      Top = 8
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        7F10006F91DD97510000000049454E44AE426082}
      OnClick = ISalirClick
    end
    object IExcel: TImage
      Left = 206
      Top = 11
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        AE426082}
      OnClick = IExcelClick
    end
    object LExcel: TLabel
      Left = 187
      Top = 63
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = IExcelClick
    end
    object LSalir: TLabel
      Left = 670
      Top = 62
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ISalirClick
    end
    object BitBtn3: TBitBtn
      Left = 1442
      Top = 8
      Width = 97
      Height = 25
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
  end
  inherited PTitulo: TPanel
    Width = 961
    Caption = 'Inspecci'#243'n de productos'
    inherited LTitulo: TLabel
      Width = 557
      Caption = 'Inspecci'#243'n de productos'
    end
    inherited PILeft: TPanel
      Left = 753
    end
  end
  inherited Psocalo: TPanel
    Top = 651
    Width = 961
    inherited JvSocalo: TJvImage
      Width = 961
    end
  end
  inherited ALUniversal: TActionList
    Left = 627
    Top = 38
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 804
    Top = 47
  end
  inherited SDXLS: TSaveDialog
    Left = 742
    Top = 13
  end
  inherited DTXLS: TDataToXLS
    Left = 744
    Top = 49
  end
  inherited WinXP1: TWinXP
    Left = 624
    Top = 8
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 684
    Top = 39
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 684
    Top = 7
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 772
    Top = 15
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 804
    Top = 15
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 652
    Top = 7
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 652
    Top = 39
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 776
    Top = 48
  end
  inherited PLVersion: TPngImageList
    Left = 864
    Top = 24
  end
  object DSDatosInspeccion: TDataSource
    DataSet = IBQDatosInspenccion
    Left = 9
    Top = 498
  end
  object IBQDatosInspenccion: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from rep_inspeccion_braford(:desde,:hasta,:establecimie' +
        'nto,:potrero,:rodeo,:tiporeg)')
    Left = 8
    Top = 464
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'potrero'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'rodeo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tiporeg'
        ParamType = ptUnknown
      end>
  end
  object IBQGetDatosAnimal: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from tab_animales where id_animal = :id')
    Left = 48
    Top = 464
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end>
  end
  object IBQGetNombreSexo: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select nombre, tipo from cod_sexos where id_sexo = :id_sexo')
    Left = 89
    Top = 463
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_sexo'
        ParamType = ptUnknown
      end>
  end
  object IBQGetTipoRegistro: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select nombre from cod_subcategorias where id_subcategoria = :id' +
        '_subcate')
    Left = 128
    Top = 464
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_subcate'
        ParamType = ptUnknown
      end>
  end
  object IBQPotrero: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select id_potrero, nombre '
      ''
      'from tab_potreros'
      ''
      'where establecimiento = :estable')
    Left = 352
    Top = 80
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'estable'
        ParamType = ptUnknown
      end>
  end
  object IBQRodeo: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select id_rodeo, nombre '
      ''
      'from tab_rodeos'
      ''
      'where establecimiento = :estable')
    Left = 352
    Top = 128
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'estable'
        ParamType = ptUnknown
      end>
  end
  object IBQTiposRegistros: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select cs.* '
      ''
      
        'from cod_razas cr join rel_subcategoria_raza rsr on cr.id_raza =' +
        ' rsr.id_raza join cod_subcategorias cs on cs.id_subcategoria = r' +
        'sr.id_subcategoria'
      ''
      'where (cr.visible = 1) and (cr.id_raza = :raza)')
    Left = 616
    Top = 81
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
  end
end
