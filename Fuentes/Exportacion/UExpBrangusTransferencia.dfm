inherited FExpBrangusTransferencia: TFExpBrangusTransferencia
  Left = 252
  Top = 41
  BorderStyle = bsSingle
  Caption = 'Cesi'#243'n de Embriones a Terceros'
  ClientHeight = 693
  ClientWidth = 998
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 998
    Picture.Data = {
      
      3B}
  end
  object PGrilla: TPanel [4]
    Left = 0
    Top = 169
    Width = 998
    Height = 406
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object GBDatos: TGroupBox
      Left = 1
      Top = 1
      Width = 996
      Height = 195
      Align = alClient
      Caption = '  Datos de Colecta  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BDBGDatosColecta: TBitDBGrid
        Left = 2
        Top = 18
        Width = 992
        Height = 175
        Cursor = crHandPoint
        Align = alClient
        DataSource = DSColecta
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'Arial'
        TitleFont.Style = []
        OnCellClick = BDBGDatosColectaCellClick
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_COLECTA'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha Producci'#243'n'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 120
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA_SERVICIO'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Fecha Servicio'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 120
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'RP_P1'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Rp (Padre)'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 120
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'OTRO_ID_P1'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'AAB/HBA (Padre)'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 120
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'NOMBRE_P1'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Nombre (Padre)'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'APODO_P1'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Apodo (Padre)'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 200
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'TIPO_SANG_P1'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'N'#186' Tip. San. (Padre)'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -12
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Width = 130
            Visible = True
          end>
      end
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 196
      Width = 996
      Height = 209
      Align = alBottom
      Caption = '  Datos generales  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object GBSelector: TGroupBox
        Left = 2
        Top = 18
        Width = 373
        Height = 189
        Align = alLeft
        Caption = '  Selector de embriones  '
        TabOrder = 0
        object BDBGGrillaEmbriones: TBitDBGrid
          Left = 2
          Top = 18
          Width = 369
          Height = 169
          Cursor = crHandPoint
          Align = alClient
          DataSource = DSDatosEmbriones
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'Arial'
          TitleFont.Style = []
          AllowIncSearch = False
          ColumnVersion = 0
          DefaultColumns = True
          DrawingEllipse = True
          DrawReadOnly = False
          EnterKey = ekNone
          FillFields = False
          FooterColor = clWindow
          FooterFont.Charset = DEFAULT_CHARSET
          FooterFont.Color = clWindowText
          FooterFont.Height = -11
          FooterFont.Name = 'MS Sans Serif'
          FooterFont.Style = []
          FooterType = foNone
          InternalDrag = False
          LongCaption = True
          MultiCaption = True
          RealTitleFont.Charset = DEFAULT_CHARSET
          RealTitleFont.Color = clWindowText
          RealTitleFont.Height = -8
          RealTitleFont.Name = 'MS Sans Serif'
          RealTitleFont.Style = []
          RunTimePropEditor = True
          ShowCellHint = True
          TitleBtn = tbNone
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'SERIE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Serie'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 90
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'NRO_ENVASE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Nro Envase'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 90
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'NRO_ORDEN'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Nro Orden'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 90
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'SINONIMO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Destino'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 90
              Visible = True
            end>
        end
      end
      object GBSelectorVientres: TGroupBox
        Left = 392
        Top = 18
        Width = 602
        Height = 189
        Align = alRight
        Caption = '  Selector de vientres  '
        TabOrder = 1
        object BDBGGrillaMadres: TDBGrid
          Left = 2
          Top = 18
          Width = 598
          Height = 169
          Cursor = crHandPoint
          Align = alClient
          DataSource = DSDatosMadres
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'Arial'
          TitleFont.Style = [fsBold]
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'FECHA'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Fec. Implante'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 85
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'ID_RP'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Rp (Receptora)'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 105
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'ID_SENASA'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Caravana (Receptora)'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 145
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'RAZANOMBRE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Raza (Receptora)'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 115
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'REGISTRO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Arial'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Registro (Receptora)'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -12
              Title.Font.Name = 'Arial'
              Title.Font.Style = [fsBold]
              Width = 135
              Visible = True
            end>
        end
      end
    end
  end
  object PFiltro: TPanel [5]
    Left = 0
    Top = 65
    Width = 998
    Height = 104
    Align = alTop
    Color = clWhite
    TabOrder = 3
    object Label1: TLabel
      Left = 232
      Top = 40
      Width = 141
      Height = 17
      Caption = 'Seleccionar donante: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object DBLCBADonantes: TDBLookupComboBoxAuto
      Left = 374
      Top = 37
      Width = 399
      Height = 25
      Cursor = crHandPoint
      Color = 14679807
      DropDownRows = 10
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      KeyField = 'ID_ANIMAL'
      ListField = 'NOMBRE'
      ListSource = DSDonantes
      ParentFont = False
      TabOrder = 0
      OnCloseUp = DBLCBADonantesCloseUp
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 575
    Width = 998
    Height = 96
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      998
      96)
    object Image5: TImage
      Left = 0
      Top = 0
      Width = 998
      Height = 96
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 646
      Top = 9
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        7F10006F91DD97510000000049454E44AE426082}
      OnClick = ISalirClick
    end
    object IExcel: TImage
      Left = 240
      Top = 11
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        AE426082}
      OnClick = IExcelClick
    end
    object LExcel: TLabel
      Left = 221
      Top = 64
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = IExcelClick
    end
    object LSalir: TLabel
      Left = 662
      Top = 64
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ISalirClick
    end
    object BitBtn3: TBitBtn
      Left = 1895
      Top = 8
      Width = 97
      Height = 25
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
  end
  inherited PTitulo: TPanel
    Width = 998
    Caption = 'Cesi'#243'n de Embriones a Terceros'
    inherited LTitulo: TLabel
      Width = 594
      Caption = 'Cesi'#243'n de Embriones a Terceros'
    end
    inherited PILeft: TPanel
      Left = 790
    end
  end
  inherited Psocalo: TPanel
    Top = 671
    Width = 998
    inherited JvSocalo: TJvImage
      Width = 998
    end
  end
  inherited ALUniversal: TActionList
    Left = 944
    Top = 47
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 852
    Top = 47
  end
  inherited SDXLS: TSaveDialog
    Left = 944
    Top = 16
  end
  inherited DTXLS: TDataToXLS
    Left = 914
    Top = 47
  end
  inherited WinXP1: TWinXP
    Left = 914
    Top = 15
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 820
    Top = 15
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 852
    Top = 15
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 884
    Top = 15
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 788
    Top = 15
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 820
    Top = 47
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 788
    Top = 47
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 884
    Top = 47
  end
  inherited PLVersion: TPngImageList
    Left = 976
    Top = 16
  end
  object DSDonantes: TDataSource
    DataSet = IBQDonantes
    Left = 96
    Top = 96
  end
  object IBQDonantes: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select distinct(ta.id_animal) as id_animal, ta.id_rp || '#39' - '#39' ||' +
        ' ta.nombre as nombre, ta.id_hba, ta.raza, ta.tipificacion_sangui' +
        'nea, ta.criador'
      
        'from tab_animales ta join eve_eventos ee on ee.animal = ta.id_an' +
        'imal and ee.establecimiento = ta.establecimiento join eve_flushi' +
        'ng ef on ee.id_evento = ef.id_evento'
      
        'where ee.establecimiento = :esta and ee.tipo = 47 and ef.cantida' +
        'd_embriones > 0'
      '')
    Left = 128
    Top = 96
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosColecta: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from get_datos_colecta_brangus(:animal,:establecimiento' +
        ')')
    Left = 8
    Top = 272
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'animal'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end>
  end
  object DSColecta: TDataSource
    DataSet = IBQDatosColecta
    Left = 40
    Top = 273
  end
  object DSDatosEmbriones: TDataSource
    DataSet = IBQDatosEmbriones
    Left = 8
    Top = 448
  end
  object IBQDatosEmbriones: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select tae.id_embrion, tae.serie, tae.nro_envase, tae.nro_orden,' +
        ' cde.sinonimo'
      
        'from tab_embriones tae join cod_destino_embrion cde on (tae.dest' +
        'ino = cde.id_destino_embrion)'
      
        'where tae.madre = :id and tae.establecimiento = :esta and tae.fe' +
        'cha_coleccion = :fecha and tae.usado = '#39'S'#39
      '')
    Left = 40
    Top = 448
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fecha'
        ParamType = ptUnknown
      end>
  end
  object DSDatosMadres: TDataSource
    DataSet = IBQDatosMadres
    Left = 632
    Top = 472
  end
  object IBQDatosMadres: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQDatosMadresCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select ta.id_animal as id_animal, ta.id_rp, ta.id_senasa, ta.raz' +
        'a, ta.nombre as nombre, ta.subcategoria, te.dias_gestacion, te.e' +
        'stadio, te.calidad, ee.fecha '
      ''
      
        'from tab_animales ta join eve_eventos ee on ta.id_animal = ee.an' +
        'imal and ta.establecimiento = ee.establecimiento join eve_transf' +
        'erencia et on ee.id_evento = et.id_evento join tab_embriones te ' +
        'on et.embrion = te.id_embrion '
      ''
      
        'where te.fecha_coleccion = :fecha and te.madre = :madre and te.p' +
        'adre_1 = :p1 and te.padre_2 = :p2')
    Filtered = True
    Left = 664
    Top = 472
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'fecha'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'madre'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'p1'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'p2'
        ParamType = ptUnknown
      end>
    object IBQDatosMadresID_ANIMAL: TIntegerField
      FieldName = 'ID_ANIMAL'
      Origin = 'TAB_ANIMALES.ID_ANIMAL'
      Required = True
    end
    object IBQDatosMadresID_RP: TIBStringField
      FieldName = 'ID_RP'
      Origin = 'TAB_ANIMALES.ID_RP'
      Required = True
      Size = 10
    end
    object IBQDatosMadresID_SENASA: TIBStringField
      FieldName = 'ID_SENASA'
      Origin = 'TAB_ANIMALES.ID_SENASA'
      Size = 9
    end
    object IBQDatosMadresRAZA: TIntegerField
      FieldName = 'RAZA'
      Origin = 'TAB_ANIMALES.RAZA'
      Required = True
    end
    object IBQDatosMadresNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'TAB_ANIMALES.NOMBRE'
      Size = 100
    end
    object IBQDatosMadresSUBCATEGORIA: TIntegerField
      FieldName = 'SUBCATEGORIA'
      Origin = 'TAB_ANIMALES.SUBCATEGORIA'
    end
    object IBQDatosMadresDIAS_GESTACION: TIntegerField
      FieldName = 'DIAS_GESTACION'
      Origin = 'TAB_EMBRIONES.DIAS_GESTACION'
    end
    object IBQDatosMadresESTADIO: TIntegerField
      FieldName = 'ESTADIO'
      Origin = 'TAB_EMBRIONES.ESTADIO'
    end
    object IBQDatosMadresCALIDAD: TIntegerField
      FieldName = 'CALIDAD'
      Origin = 'TAB_EMBRIONES.CALIDAD'
    end
    object IBQDatosMadresFECHA: TDateField
      FieldName = 'FECHA'
      Origin = 'EVE_EVENTOS.FECHA'
      Required = True
    end
    object IBQDatosMadresREGISTRO: TStringField
      FieldKind = fkCalculated
      FieldName = 'REGISTRO'
      Calculated = True
    end
    object SELECCIONADO: TStringField
      FieldKind = fkCalculated
      FieldName = 'SELECCIONADO'
      Calculated = True
    end
    object IBQDatosMadresRAZANOMBRE: TStringField
      FieldKind = fkCalculated
      FieldName = 'RAZANOMBRE'
      Calculated = True
    end
  end
  object IBQRegistroRecep: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select csc.nombre as nombre'
      ''
      
        'from COD_SUBCATEGORIAS csc join TAB_ANIMALES ta on (ta.subcatego' +
        'ria = csc.id_subcategoria)'
      ''
      'where ta.id_animal = :id_animal')
    Left = 632
    Top = 512
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id_animal'
        ParamType = ptUnknown
      end>
    object IBQRegistroRecepNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'COD_SUBCATEGORIAS.NOMBRE'
      Size = 50
    end
  end
  object IBQNombreRaza: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select nombre, sinonimo from cod_razas where id_raza = :id')
    Left = 720
    Top = 512
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosMadresExtension: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQDatosMadresExtensionCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select ta.id_animal as animal, ta.id_rp as rp, ta.id_senasa, ta.' +
        'raza as raza, ta.nombre as nombre, ta.subcategoria as sub, te.di' +
        'as_gestacion, te.estadio, te.calidad, ee.fecha '
      ''
      
        'from tab_animales ta join eve_eventos ee on ta.id_animal = ee.an' +
        'imal and ta.establecimiento = ee.establecimiento join eve_transf' +
        'erencia et on ee.id_evento = et.id_evento join tab_embriones te ' +
        'on et.embrion = te.id_embrion '
      ''
      
        'where te.fecha_coleccion = :fecha and te.madre = :madre and te.p' +
        'adre_1 = :p1 and te.padre_2 = :p2')
    Filtered = True
    OnFilterRecord = IBQDatosMadresExtensionFilterRecord
    Left = 719
    Top = 473
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'fecha'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'madre'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'p1'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'p2'
        ParamType = ptUnknown
      end>
    object IBQDatosMadresExtensionRegistro: TStringField
      FieldKind = fkCalculated
      FieldName = 'registro'
      Calculated = True
    end
    object IBQDatosMadresExtensionNombreRaza: TStringField
      FieldKind = fkCalculated
      FieldName = 'nombreraza'
      Calculated = True
    end
    object IBQDatosMadresExtensionANIMAL: TIntegerField
      FieldName = 'ANIMAL'
      Origin = 'TAB_ANIMALES.ID_ANIMAL'
      Required = True
    end
    object IBQDatosMadresExtensionRP: TIBStringField
      FieldName = 'RP'
      Origin = 'TAB_ANIMALES.ID_RP'
      Required = True
      Size = 10
    end
    object IBQDatosMadresExtensionID_SENASA: TIBStringField
      FieldName = 'ID_SENASA'
      Origin = 'TAB_ANIMALES.ID_SENASA'
      Size = 9
    end
    object IBQDatosMadresExtensionRAZA: TIntegerField
      FieldName = 'RAZA'
      Origin = 'TAB_ANIMALES.RAZA'
      Required = True
    end
    object IBQDatosMadresExtensionNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'TAB_ANIMALES.NOMBRE'
      Size = 100
    end
    object IBQDatosMadresExtensionSUB: TIntegerField
      FieldName = 'SUB'
      Origin = 'TAB_ANIMALES.SUBCATEGORIA'
    end
    object IBQDatosMadresExtensionDIAS_GESTACION: TIntegerField
      FieldName = 'DIAS_GESTACION'
      Origin = 'TAB_EMBRIONES.DIAS_GESTACION'
    end
    object IBQDatosMadresExtensionESTADIO: TIntegerField
      FieldName = 'ESTADIO'
      Origin = 'TAB_EMBRIONES.ESTADIO'
    end
    object IBQDatosMadresExtensionCALIDAD: TIntegerField
      FieldName = 'CALIDAD'
      Origin = 'TAB_EMBRIONES.CALIDAD'
    end
    object IBQDatosMadresExtensionFECHA: TDateField
      FieldName = 'FECHA'
      Origin = 'EVE_EVENTOS.FECHA'
      Required = True
    end
  end
  object IBQExportDatos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from rep_cesion_embriones(:donante, :esta, :fecha_colec' +
        'ta, :id_padre, :id_madre)')
    Filtered = True
    OnFilterRecord = IBQExportDatosFilterRecord
    Left = 440
    Top = 610
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'donante'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fecha_colecta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_padre'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'id_madre'
        ParamType = ptUnknown
      end>
  end
end
