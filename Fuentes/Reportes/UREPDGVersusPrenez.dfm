inherited FREPDGVersusPrenez: TFREPDGVersusPrenez
  Left = 122
  Top = 1
  Caption = 'FREPDGVersusPrenez'
  ClientHeight = 736
  ClientWidth = 860
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 860
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 476
  end
  object PTop: TPanel [4]
    Left = 0
    Top = 65
    Width = 860
    Height = 79
    Align = alTop
    Color = clWhite
    UseDockManager = False
    TabOrder = 4
    object Splitter2: TSplitter
      Left = 139
      Top = 1
      Height = 77
    end
    object GBSeleccionFechas: TGroupBox
      Left = 407
      Top = 1
      Width = 250
      Height = 77
      Align = alLeft
      Caption = ' Selecci'#243'n de fechas '
      Enabled = False
      TabOrder = 0
      object LDesde: TLabel
        Left = 18
        Top = 25
        Width = 37
        Height = 14
        Caption = 'Desde: '
      end
      object LHasta: TLabel
        Left = 22
        Top = 48
        Width = 34
        Height = 14
        Caption = 'Hasta: '
      end
      object DTPADesde: TDateTimePickerAuto
        Left = 69
        Top = 17
        Width = 151
        Height = 21
        Date = 38744.433197870370000000
        Time = 38744.433197870370000000
        Color = clWhite
        TabOrder = 0
        OnCloseUp = DTPACloseUpFechas
        OnKeyUp = DTPADesdeKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object DTPAHasta: TDateTimePickerAuto
        Left = 69
        Top = 43
        Width = 151
        Height = 21
        Date = 38744.433227719900000000
        Time = 38744.433227719900000000
        Color = clWhite
        TabOrder = 1
        OnCloseUp = DTPACloseUpFechas
        OnKeyUp = DTPADesdeKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object GBGrupos: TGroupBox
      Left = 142
      Top = 1
      Width = 265
      Height = 77
      Align = alLeft
      Caption = ' Selecci'#243'n de grupo de diagn'#243'stico de gestaci'#243'n '
      TabOrder = 1
      object DBLCBASeleccionGrupo: TDBLookupComboBoxAuto
        Left = 16
        Top = 26
        Width = 232
        Height = 22
        Color = clWhite
        DropDownRows = 20
        KeyField = 'ID_GRUPO'
        ListField = 'NOMBRE'
        ListSource = DSGrupos
        TabOrder = 0
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object RGTipo: TRadioGroup
      Left = 1
      Top = 1
      Width = 138
      Height = 77
      Align = alLeft
      Caption = ' Tipo de selecci'#243'n '
      ItemIndex = 0
      Items.Strings = (
        'Grupo'
        'Fecha')
      TabOrder = 2
      OnClick = RGTipoClick
    end
    object GConsulta: TGroupBox
      Left = 657
      Top = 1
      Width = 202
      Height = 77
      Align = alClient
      Caption = 'Consulta'
      TabOrder = 3
      object JvIBotonBuscar: TJvImage
        Tag = 4
        Left = 51
        Top = 29
        Width = 97
        Height = 32
        Cursor = crHandPoint
        Picture.Data = {
          
          E9FF0056DF9FC352571EAFFFD9}
        OnClick = JvIBotonBuscarClick
      end
      object BConsulta: TButton
        Left = 72
        Top = 56
        Width = 65
        Height = 17
        Caption = 'Consultar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        Visible = False
        OnClick = BConsultaClick
      end
    end
  end
  object PConfiguracion: TPanel [5]
    Left = 0
    Top = 144
    Width = 860
    Height = 129
    Align = alTop
    Color = clWhite
    TabOrder = 5
    object RGTipoReporte: TRadioGroup
      Left = 1
      Top = 1
      Width = 233
      Height = 127
      Align = alLeft
      Caption = ' Tipo Reporte '
      Color = clWhite
      ItemIndex = 0
      Items.Strings = (
        'Pre'#241'ez Vs Condici'#243'n Corporal'
        'Pre'#241'ez Vs Cronolog'#237'a Dentaria')
      ParentColor = False
      TabOrder = 0
      OnClick = RGTipoReporteClick
    end
    object RGFiltro: TRadioGroup
      Left = 234
      Top = 1
      Width = 253
      Height = 127
      Align = alLeft
      Caption = ' Filtro '
      ItemIndex = 2
      Items.Strings = (
        'Rodeos'
        'Categor'#237'as'
        'Todos')
      TabOrder = 1
      OnClick = RGFiltroClick
    end
    object DBLCBARodeos: TDBLookupComboBoxAuto
      Left = 347
      Top = 20
      Width = 129
      Height = 22
      Color = clWhite
      DropDownRows = 20
      Enabled = False
      KeyField = 'ID_RODEO'
      ListField = 'NOMBRE'
      ListSource = DMSoftvet.DSRodeo
      TabOrder = 2
      OnCloseUp = Rodeos
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object CBACategoria: TComboBoxAuto
      Left = 347
      Top = 58
      Width = 129
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      Enabled = False
      ItemHeight = 14
      ItemIndex = 0
      TabOrder = 3
      Text = 'VACA'
      OnChange = CBACategoriaChange
      Items.Strings = (
        'VACA'
        'VAQUILLONA')
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object RGGrafico: TRadioGroup
      Left = 487
      Top = 1
      Width = 177
      Height = 127
      Align = alLeft
      Caption = ' Ver '
      ItemIndex = 0
      Items.Strings = (
        'Gr'#225'fico'
        'Tabla'
        '')
      TabOrder = 4
      OnClick = RGGraficoClick
    end
    object CBImprimirGrafico: TCheckBox
      Left = 586
      Top = 25
      Width = 61
      Height = 18
      Caption = 'Imprimir'
      Checked = True
      State = cbChecked
      TabOrder = 5
    end
    object CBImprimirTabla: TCheckBox
      Left = 586
      Top = 60
      Width = 61
      Height = 19
      Caption = 'Imprimir'
      TabOrder = 6
    end
    object GroupBox1: TGroupBox
      Left = 664
      Top = 1
      Width = 195
      Height = 127
      Align = alClient
      Caption = ' Gr'#225'fico Pre'#241'adas '
      TabOrder = 7
      object CBCabeza: TCheckBox
        Left = 71
        Top = 26
        Width = 68
        Height = 18
        Caption = 'Cabeza'
        Checked = True
        State = cbChecked
        TabOrder = 0
        OnClick = CBCabezaClick
      end
      object CBCuerpo: TCheckBox
        Left = 71
        Top = 58
        Width = 70
        Height = 18
        Caption = 'Cuerpo'
        Checked = True
        State = cbChecked
        TabOrder = 1
        OnClick = CBCuerpoClick
      end
      object CBCola: TCheckBox
        Left = 71
        Top = 93
        Width = 60
        Height = 18
        Caption = 'Cola'
        Checked = True
        State = cbChecked
        TabOrder = 2
        OnClick = CBColaClick
      end
    end
    object CBListado: TCheckBox
      Left = 495
      Top = 96
      Width = 164
      Height = 18
      Caption = 'Imprimir Listado de Animales'
      Checked = True
      State = cbChecked
      TabOrder = 8
    end
  end
  object PGraficos: TPanel [6]
    Left = 0
    Top = 273
    Width = 860
    Height = 195
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 858
      Height = 193
      Align = alClient
      TabOrder = 0
    end
    object Ptactadas: TPanel
      Left = 1
      Top = 1
      Width = 858
      Height = 193
      Align = alClient
      TabOrder = 1
      object GBTabla: TGroupBox
        Left = 1
        Top = 1
        Width = 856
        Height = 191
        Align = alClient
        Caption = ' Tabla '
        TabOrder = 1
        Visible = False
        object SGTabla: TStringGrid
          Left = 237
          Top = 53
          Width = 505
          Height = 172
          BorderStyle = bsNone
          Color = clBtnFace
          DefaultRowHeight = 18
          FixedCols = 0
          RowCount = 7
          FixedRows = 0
          TabOrder = 0
          OnDrawCell = SGTablaDrawCell
          ColWidths = (
            64
            97
            90
            96
            86)
        end
        object Panel3: TPanel
          Left = 2
          Top = 52
          Width = 20
          Height = 137
          Align = alLeft
          BevelOuter = bvNone
          TabOrder = 1
        end
        object Panel4: TPanel
          Left = 835
          Top = 52
          Width = 19
          Height = 137
          Align = alRight
          BevelOuter = bvNone
          TabOrder = 2
        end
        object PTituloTabla: TPanel
          Left = 2
          Top = 16
          Width = 852
          Height = 36
          Align = alTop
          BevelOuter = bvNone
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 3
        end
      end
      object GBGrafico: TGroupBox
        Left = 1
        Top = 1
        Width = 856
        Height = 191
        Align = alClient
        Caption = ' Gr'#225'fico  '
        Color = clWhite
        ParentColor = False
        TabOrder = 0
        object DBCPrenezPreniadas: TDBChart
          Left = 432
          Top = 16
          Width = 422
          Height = 173
          AllowPanning = pmHorizontal
          AllowZoom = False
          AnimatedZoomSteps = 16
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            'Condici'#243'n Corporal en las pre'#241'adas')
          BottomAxis.Automatic = False
          BottomAxis.AutomaticMaximum = False
          BottomAxis.AutomaticMinimum = False
          BottomAxis.ExactDateTime = False
          BottomAxis.LabelsMultiLine = True
          BottomAxis.LabelStyle = talValue
          BottomAxis.Maximum = 11.000000000000000000
          BottomAxis.MinorTickCount = 5
          BottomAxis.MinorTickLength = 0
          BottomAxis.Title.Caption = 'Condiciones corporales'
          Chart3DPercent = 40
          LeftAxis.ExactDateTime = False
          LeftAxis.Title.Caption = 'Cantidad'
          Legend.ColorWidth = 10
          Legend.LegendStyle = lsSeries
          Legend.TextStyle = ltsXValue
          View3D = False
          View3DOptions.Elevation = 349
          View3DOptions.ZoomText = False
          OnBeforeDrawAxes = DBCPrenezPreniadasBeforeDrawAxes
          OnGetAxisLabel = DBCPrenezPreniadasGetAxisLabel
          Align = alClient
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 1
          object Series5: TBarSeries
            Marks.ArrowLength = 5
            Marks.BackColor = clWhite
            Marks.Clip = True
            Marks.Frame.Visible = False
            Marks.Style = smsValue
            Marks.Transparent = True
            Marks.Visible = True
            DataSource = IBQCantidades
            SeriesColor = clYellow
            Title = 'Cola'
            XLabelsSource = 'LA'
            OnBeforeAdd = Series5BeforeAdd
            BarWidthPercent = 100
            XValues.DateTime = False
            XValues.Name = 'X'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'VALOR'
            YValues.DateTime = False
            YValues.Name = 'Bar'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
          end
          object Series4: TBarSeries
            Marks.ArrowLength = 5
            Marks.BackColor = clWhite
            Marks.Clip = True
            Marks.Frame.Visible = False
            Marks.Style = smsValue
            Marks.Transparent = True
            Marks.Visible = True
            DataSource = IBQCantidades
            SeriesColor = clGreen
            Title = 'Cuerpo'
            XLabelsSource = 'LA'
            OnBeforeAdd = Series4BeforeAdd
            BarWidthPercent = 100
            XValues.DateTime = False
            XValues.Name = 'X'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'VALOR'
            YValues.DateTime = False
            YValues.Name = 'Bar'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
          end
          object Series6: TBarSeries
            Marks.ArrowLength = 5
            Marks.BackColor = clWhite
            Marks.Clip = True
            Marks.Frame.Visible = False
            Marks.Style = smsValue
            Marks.Transparent = True
            Marks.Visible = True
            DataSource = IBQCantidades
            SeriesColor = 16744448
            Title = 'Cabeza'
            OnBeforeAdd = Series6BeforeAdd
            BarWidthPercent = 100
            XValues.DateTime = False
            XValues.Name = 'X'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'VALOR'
            YValues.DateTime = False
            YValues.Name = 'Bar'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
          end
        end
        object DBCPrenezVacias: TDBChart
          Left = 2
          Top = 16
          Width = 430
          Height = 173
          AllowPanning = pmHorizontal
          AllowZoom = False
          AnimatedZoomSteps = 16
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            'Condici'#243'n corporal en las vacias')
          BottomAxis.Automatic = False
          BottomAxis.AutomaticMaximum = False
          BottomAxis.AutomaticMinimum = False
          BottomAxis.ExactDateTime = False
          BottomAxis.LabelsMultiLine = True
          BottomAxis.LabelsSeparation = 0
          BottomAxis.LabelStyle = talValue
          BottomAxis.Maximum = 10.000000000000000000
          BottomAxis.Minimum = -1.000000000000000000
          BottomAxis.MinorTickCount = 5
          BottomAxis.MinorTickLength = 0
          BottomAxis.Title.Caption = 'Condiciones corporales'
          Chart3DPercent = 40
          LeftAxis.ExactDateTime = False
          LeftAxis.Title.Caption = 'Cantidad'
          Legend.LegendStyle = lsSeries
          Legend.TextStyle = ltsXValue
          View3D = False
          View3DOptions.Elevation = 349
          View3DOptions.Perspective = 5
          View3DOptions.ZoomText = False
          OnBeforeDrawAxes = DBCPrenezVaciasBeforeDrawAxes
          OnGetAxisLabel = DBCPrenezVaciasGetAxisLabel
          Align = alLeft
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 0
          object Series7: TBarSeries
            Marks.ArrowLength = 5
            Marks.BackColor = clWhite
            Marks.Clip = True
            Marks.Style = smsValue
            Marks.Visible = True
            DataSource = IBQCantidades
            SeriesColor = clBlue
            Title = 'Vacias'
            XLabelsSource = 'LA'
            OnBeforeAdd = Series7BeforeAdd
            BarWidthPercent = 20
            XValues.DateTime = False
            XValues.Name = 'X'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'VALOR'
            YValues.DateTime = False
            YValues.Name = 'Bar'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
          end
        end
      end
    end
  end
  object PAnimales: TPanel [7]
    Left = 0
    Top = 468
    Width = 860
    Height = 150
    Align = alBottom
    Color = clWhite
    TabOrder = 2
    object GBAnimales: TGroupBox
      Left = 1
      Top = 1
      Width = 858
      Height = 148
      Align = alClient
      Caption = ' Animales [0]'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BitDBGrid1: TBitDBGrid
        Left = 2
        Top = 18
        Width = 854
        Height = 128
        Align = alClient
        DataSource = DSAnimales
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = BitDBGrid1DblClick
        OnKeyPress = BitDBGrid1KeyPress
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Expanded = False
            FieldName = 'RP_ANIMAL'
            Title.Caption = 'RP Animal'
            Width = 85
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Pre'#241'ez'
            Width = 107
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CONDICION'
            Title.Caption = 'Condici'#243'n Corporal'
            Width = 112
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CRONOLOGIA'
            Title.Caption = 'Cronolog'#237'a Dentaria'
            Width = 141
            Visible = True
          end>
      end
    end
  end
  object PBotones: TPanel [8]
    Left = 0
    Top = 618
    Width = 860
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 6
    DesignSize = (
      860
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 858
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 728
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = ASalirExecute
    end
    object LSalir: TLabel
      Left = 739
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ASalirExecute
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      OnClick = BBXlsClick
    end
    object IPreview: TImage
      Left = 309
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = AImprimirReporteExecute
    end
    object LPreview: TLabel
      Left = 306
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AImprimirReporteExecute
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object LExpExcel: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBXlsClick
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 751
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      OnClick = BBXlsClick
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 860
    Caption = 'Comparaci'#243'n de Pre'#241'ez'
    inherited LTitulo: TLabel
      Width = 456
    end
    inherited PILeft: TPanel
      Left = 652
    end
  end
  inherited Psocalo: TPanel
    Top = 714
    Width = 860
    inherited JvSocalo: TJvImage
      Width = 860
    end
  end
  inherited ALUniversal: TActionList
    Left = 123
    Top = 65534
    object ASalir: TAction
      Caption = 'Salir'
      ShortCut = 27
      OnExecute = ASalirExecute
    end
    object AImprimirReporte: TAction
      Caption = 'Imprimir'
      Enabled = False
      OnExecute = AImprimirReporteExecute
    end
  end
  inherited SDXLS: TSaveDialog
    Left = 286
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQAnimales
    Columns = <
      item
        DataField = 'RP_ANIMAL'
        Title = 'RP Animal'
      end
      item
        DataField = 'NOMBRE'
        Title = 'Pre'#241'ez'
      end
      item
        DataField = 'CONDICION'
        Title = 'Condicion Corporal'
      end
      item
        DataField = 'CRONOLOGIA'
        Title = 'Cronologia Dentaria'
      end>
    Title.Text = 'Comparacion de Pre'#241'ez'
  end
  inherited WinXP1: TWinXP
    Left = 344
    Top = 72
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 380
    Top = 183
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 268
    Top = 23
  end
  object DSAnimales: TDataSource
    DataSet = IBQAnimales
    Left = 8
    Top = 341
  end
  object IBQAnimales: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQAnimalesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from GET_DISTRIBUCION_PRENEZ (1, '#39'01-27-2006'#39', '#39'01-27-2' +
        '007'#39', null, '#39'today'#39') order by orden, valorcronologia'
      '')
    Left = 40
    Top = 341
  end
  object DSGrupos: TDataSource
    DataSet = IBQGrupos
    Left = 40
    Top = 93
  end
  object IBQGrupos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BeforeOpen = IBQGruposBeforeOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from tab_grupos where (tipo_evento = 13) and (estableci' +
        'miento = :esta) and visible=1 order by upper(nombre) asc')
    Left = 88
    Top = 101
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQCA: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcronologia as la, Cronologia a' +
        's valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-' +
        '27-2006'#39', '#39'01-27-2007'#39', :grupo, '#39'today'#39') where (id_codigo_prenez' +
        ' = '#39'CA'#39') group by cronologia, valorCronologia,  id_codigo_prenez')
    Left = 512
    Top = 565
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end>
  end
  object IBQCU: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcronologia as la, Cronologia a' +
        's valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-' +
        '27-2006'#39', '#39'01-27-2007'#39', :grupo, '#39'today'#39') where (id_codigo_prenez' +
        ' = '#39'CU'#39') group by cronologia, valorCronologia,  id_codigo_prenez'
      '')
    Left = 544
    Top = 533
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end>
  end
  object IBQCO: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcronologia as la, Cronologia a' +
        's valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-' +
        '27-2006'#39', '#39'01-27-2007'#39', :grupo, '#39'today'#39') where (id_codigo_prenez' +
        ' = '#39'CO'#39') group by cronologia, valorCronologia,  id_codigo_prenez')
    Left = 472
    Top = 565
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end>
  end
  object IBQNN: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcronologia as la, Cronologia a' +
        's valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-' +
        '27-2006'#39', '#39'01-27-2007'#39', :grupo, '#39'today'#39') where (id_codigo_prenez' +
        ' = '#39'NN'#39') group by cronologia, valorCronologia,  id_codigo_prenez'
      '')
    Left = 552
    Top = 565
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end>
  end
  object IBQVA: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcondicion as la, condicion as ' +
        'valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-27' +
        '-2006'#39', '#39'01-27-2007'#39', 5, '#39'today'#39') where (id_codigo_prenez = '#39'V'#39')' +
        ' group by condicion, valorCondicion,  id_codigo_prenez'
      '')
    Left = 464
    Top = 533
  end
  object IBQAux: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_condiciones_corporales')
    Left = 192
    Top = 109
  end
  object IBQPreniadas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcronologia as la, Cronologia a' +
        's valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-' +
        '27-2006'#39', '#39'01-27-2007'#39', :grupo, '#39'today'#39') where (id_codigo_prenez' +
        ' <> '#39'VA'#39') group by cronologia, valorCronologia,  id_codigo_prene' +
        'z'
      '')
    Left = 504
    Top = 533
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end>
  end
  object IBQCondCorp: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select valor from rel_parametros_establecimiento where estableci' +
        'miento=:esta and parametro=3')
    Left = 164
    Top = 111
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQCodsCronDentVacias: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select codigo from cod_cronologias_dentarias')
    Left = 52
    Top = 159
  end
  object IBQCodsCronDentPreniadas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select codigo from cod_cronologias_dentarias')
    Left = 124
    Top = 111
  end
  object IBQDistribucion: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select valor from rel_parametros_establecimiento where parametro' +
        '=41 and establecimiento=:esta')
    Left = 217
    Top = 553
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQCantidades: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQCantidadesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select count(*) as cantidad, valorcondicion as la, condicion as ' +
        'valor,  id_codigo_prenez from GET_DISTRIBUCION_PRENEZ (1, '#39'01-27' +
        '-2006'#39', '#39'01-27-2007'#39', 5, '#39'today'#39') group by condicion, valorCondi' +
        'cion,  id_codigo_prenez')
    Left = 249
    Top = 553
  end
end
