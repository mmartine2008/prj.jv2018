inherited FRepInfoMortandad: TFRepInfoMortandad
  Left = 269
  Top = 72
  Caption = 'FRepInfoMortandad'
  ClientHeight = 626
  ClientWidth = 683
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 683
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 299
  end
  object DBCDisMuertesMensual: TDBChart [4]
    Left = 126
    Top = 166
    Width = 457
    Height = 362
    AllowPanning = pmHorizontal
    AllowZoom = False
    BackWall.Brush.Color = clWhite
    BackWall.Color = clSilver
    BottomWall.Brush.Color = clWhite
    BottomWall.Brush.Style = bsClear
    Gradient.EndColor = clWhite
    LeftWall.Brush.Color = clWhite
    LeftWall.Brush.Style = bsClear
    PrintProportional = False
    Title.AdjustFrame = False
    Title.Color = clWhite
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -11
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Title.Text.Strings = (
      'Distribuci'#243'n Mensual de las Muertes'
      '')
    BackColor = clSilver
    BottomAxis.LabelsAngle = 90
    LeftAxis.Grid.Visible = False
    LeftAxis.GridCentered = True
    LeftAxis.Title.Angle = 0
    Legend.Alignment = laBottom
    Legend.ColorWidth = 10
    Legend.TextStyle = ltsLeftPercent
    Legend.Visible = False
    View3DOptions.Elevation = 315
    View3DOptions.Orthogonal = False
    View3DOptions.Perspective = 0
    View3DOptions.Rotation = 360
    View3DWalls = False
    Align = alCustom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 6
    Visible = False
    object BarSeries1: TBarSeries
      ColorEachPoint = True
      Marks.ArrowLength = 4
      Marks.Style = smsPercent
      Marks.Visible = True
      SeriesColor = clRed
      Title = 'DistPreniez'
      XLabelsSource = 'ENFERMEDAD'
      BarWidthPercent = 30
      MultiBar = mbNone
      XValues.DateTime = False
      XValues.Name = 'X'
      XValues.Multiplier = 1.000000000000000000
      XValues.Order = loAscending
      YValues.DateTime = False
      YValues.Name = 'Bar'
      YValues.Multiplier = 1.000000000000000000
      YValues.Order = loNone
      YValues.ValueSource = 'CANTIDAD'
    end
  end
  object Panel4: TPanel [5]
    Left = 0
    Top = 366
    Width = 683
    Height = 142
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object GBAnimales: TGroupBox
      Left = 1
      Top = 1
      Width = 681
      Height = 140
      Align = alClient
      Caption = 'Animales'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BDBGAnimales: TBitDBGrid
        Left = 2
        Top = 18
        Width = 677
        Height = 120
        Align = alClient
        DataSource = DSInfoMortandad
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = BDBGAnimalesDblClick
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Expanded = False
            FieldName = 'FECHA'
            Title.Caption = 'Fecha Muerte'
            Width = 82
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RP'
            Title.Caption = 'RP Animal'
            Width = 92
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EDADCAPTION'
            Title.Caption = 'Edad'
            Width = 44
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CATEGORIA'
            Title.Caption = 'Categor'#237'a'
            Width = 59
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'POTRERO'
            Title.Caption = 'Potrero'
            Width = 72
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RODEO'
            Title.Caption = 'Rodeo'
            Width = 63
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RESPONSABLE'
            Title.Caption = 'Responsable'
            Width = 85
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ENFERMEDAD'
            Title.Caption = 'Enfermedad'
            Width = 258
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'OBSERVACIONES'
            Title.Caption = 'Observaciones'
            Visible = True
          end>
      end
    end
  end
  object Panel3: TPanel [6]
    Left = 0
    Top = 299
    Width = 683
    Height = 67
    Align = alTop
    Color = clWhite
    TabOrder = 3
    object GroupBox1: TGroupBox
      Left = 1
      Top = 1
      Width = 681
      Height = 65
      Align = alClient
      Caption = ' Observaciones '
      TabOrder = 0
      object MObservaciones: TMemo
        Left = 2
        Top = 16
        Width = 677
        Height = 47
        Align = alClient
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
  end
  object Panel2: TPanel [7]
    Left = 0
    Top = 179
    Width = 683
    Height = 120
    Align = alTop
    Color = clWhite
    TabOrder = 4
    object RGFiltros: TRadioGroup
      Left = 1
      Top = 1
      Width = 327
      Height = 118
      Align = alLeft
      Caption = ' Filtro '
      ItemIndex = 2
      Items.Strings = (
        'Rodeos'
        'Categor'#237'as'
        'Todos')
      TabOrder = 0
    end
    object GroupBox2: TGroupBox
      Left = 328
      Top = 1
      Width = 354
      Height = 118
      Align = alClient
      Caption = ' Imprimir Listado '
      TabOrder = 1
      object CBListado: TCheckBox
        Left = 18
        Top = 53
        Width = 164
        Height = 18
        Caption = 'Imprimir Listado de Animales'
        Checked = True
        State = cbChecked
        TabOrder = 0
      end
    end
    object DBLCBARodeos: TDBLookupComboBoxAuto
      Left = 123
      Top = 22
      Width = 129
      Height = 22
      Color = clWhite
      DropDownRows = 20
      Enabled = False
      KeyField = 'ID_RODEO'
      ListField = 'NOMBRE'
      ListSource = DMSoftvet.DSRodeo
      TabOrder = 2
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object CBACategoria: TComboBoxAuto
      Left = 123
      Top = 58
      Width = 129
      Height = 21
      CharCase = ecUpperCase
      Color = clWhite
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 13
      ItemIndex = 0
      ParentFont = False
      TabOrder = 3
      Text = 'VACA'
      Items.Strings = (
        'VACA'
        'VAQUILLONA')
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
  end
  object Panel1: TPanel [8]
    Left = 0
    Top = 65
    Width = 683
    Height = 114
    Align = alTop
    Color = clWhite
    TabOrder = 5
    object RGTipo: TRadioGroup
      Left = 1
      Top = 1
      Width = 96
      Height = 112
      Align = alLeft
      Caption = ' Tipo de selecci'#243'n '
      ItemIndex = 0
      Items.Strings = (
        'Grupo'
        'Fecha')
      TabOrder = 0
      OnClick = RGTipoClick
    end
    object GBGrupos: TGroupBox
      Left = 97
      Top = 1
      Width = 256
      Height = 112
      Align = alLeft
      Caption = ' Selecci'#243'n de grupo de diagn'#243'stico de gestaci'#243'n '
      TabOrder = 1
      object DBLCBASeleccionGrupo: TDBLookupComboBoxAuto
        Left = 16
        Top = 43
        Width = 232
        Height = 22
        Color = clWhite
        DropDownRows = 20
        KeyField = 'ID_GRUPO'
        ListField = 'NOMBRE'
        ListSource = DSGrupos
        TabOrder = 0
        OnCloseUp = DBLCBASeleccionGrupoCloseUp
        OnKeyUp = DBLCBASeleccionGrupoKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object GBSeleccionFechas: TGroupBox
      Left = 353
      Top = 1
      Width = 168
      Height = 112
      Align = alLeft
      Caption = ' Selecci'#243'n de fechas '
      Enabled = False
      TabOrder = 2
      object LDesde: TLabel
        Left = 18
        Top = 25
        Width = 37
        Height = 14
        Caption = 'Desde: '
      end
      object LHasta: TLabel
        Left = 14
        Top = 66
        Width = 34
        Height = 14
        Caption = 'Hasta: '
      end
      object DTPADesde: TDateTimePickerAuto
        Left = 69
        Top = 17
        Width = 84
        Height = 21
        Date = 38744.433197870370000000
        Time = 38744.433197870370000000
        Color = clWhite
        TabOrder = 0
        OnCloseUp = FechaCloseUp
        OnKeyUp = DTPADesdeKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object DTPAHasta: TDateTimePickerAuto
        Left = 69
        Top = 60
        Width = 84
        Height = 21
        Date = 38744.433227719900000000
        Time = 38744.433227719900000000
        Color = clWhite
        TabOrder = 1
        OnCloseUp = FechaCloseUp
        OnKeyUp = DTPADesdeKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object GConsulta: TGroupBox
      Left = 521
      Top = 1
      Width = 161
      Height = 112
      Align = alClient
      Caption = 'Consulta'
      TabOrder = 3
      object JvIBotonBuscar: TJvImage
        Tag = 4
        Left = 27
        Top = 43
        Width = 97
        Height = 32
        Cursor = crHandPoint
        Picture.Data = {
          
          E9FF0056DF9FC352571EAFFFD9}
        OnClick = JvIBotonBuscarClick
      end
      object BConsulta: TButton
        Left = 40
        Top = 96
        Width = 57
        Height = 17
        Caption = 'Consultar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        Visible = False
        OnClick = BConsultaClick
      end
    end
  end
  object DBCDistMuerteEnfermedad: TDBChart [9]
    Left = -9
    Top = 95
    Width = 441
    Height = 265
    AllowPanning = pmNone
    AllowZoom = False
    BackWall.Brush.Color = clWhite
    BackWall.Brush.Style = bsClear
    BackWall.Pen.Visible = False
    BottomWall.Brush.Color = clWhite
    BottomWall.Brush.Style = bsClear
    Gradient.EndColor = clWhite
    LeftWall.Brush.Color = clWhite
    LeftWall.Brush.Style = bsClear
    PrintProportional = False
    Title.AdjustFrame = False
    Title.Color = clWhite
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -11
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Title.Text.Strings = (
      'Distribuci'#243'n de las muertes por enfermedad'
      '')
    AxisVisible = False
    ClipPoints = False
    Frame.Visible = False
    Legend.Alignment = laBottom
    Legend.ColorWidth = 5
    View3DOptions.Elevation = 315
    View3DOptions.Orthogonal = False
    View3DOptions.Perspective = 0
    View3DOptions.Rotation = 360
    View3DWalls = False
    Align = alCustom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 8
    Visible = False
    object PieSeries2: TPieSeries
      Marks.ArrowLength = 4
      Marks.Style = smsPercent
      Marks.Visible = True
      DataSource = IBQDistMuerteEnfermedad
      SeriesColor = clRed
      Title = 'DistPreniez'
      XLabelsSource = 'ENFERMEDAD'
      OtherSlice.Text = 'Other'
      PieValues.DateTime = False
      PieValues.Name = 'Pie'
      PieValues.Multiplier = 1.000000000000000000
      PieValues.Order = loNone
      PieValues.ValueSource = 'CANTIDAD'
    end
  end
  object DBCDistMuerteCategoria: TDBChart [10]
    Left = 268
    Top = 9
    Width = 458
    Height = 282
    AllowPanning = pmHorizontal
    AllowZoom = False
    BackWall.Brush.Color = clWhite
    BackWall.Color = clSilver
    BottomWall.Brush.Color = clWhite
    BottomWall.Brush.Style = bsClear
    Gradient.EndColor = clWhite
    LeftWall.Brush.Color = clWhite
    LeftWall.Brush.Style = bsClear
    PrintProportional = False
    Title.AdjustFrame = False
    Title.Color = clWhite
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -11
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Title.Text.Strings = (
      'Distribuci'#243'n de las muertes por categor'#237'a'
      '')
    BackColor = clSilver
    BottomAxis.LabelsAngle = 90
    LeftAxis.Grid.Visible = False
    LeftAxis.GridCentered = True
    LeftAxis.Title.Angle = 0
    Legend.Alignment = laBottom
    Legend.ColorWidth = 10
    Legend.TextStyle = ltsLeftPercent
    Legend.Visible = False
    View3DOptions.Elevation = 315
    View3DOptions.Orthogonal = False
    View3DOptions.Perspective = 0
    View3DOptions.Rotation = 360
    View3DWalls = False
    Align = alCustom
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 7
    Visible = False
    object PieSeries1: TBarSeries
      ColorEachPoint = True
      Marks.ArrowLength = 4
      Marks.Style = smsPercent
      Marks.Visible = True
      SeriesColor = clRed
      Title = 'DistPreniez'
      XLabelsSource = 'ENFERMEDAD'
      BarWidthPercent = 30
      MultiBar = mbNone
      XValues.DateTime = False
      XValues.Name = 'X'
      XValues.Multiplier = 1.000000000000000000
      XValues.Order = loAscending
      YValues.DateTime = False
      YValues.Name = 'Bar'
      YValues.Multiplier = 1.000000000000000000
      YValues.Order = loNone
      YValues.ValueSource = 'CANTIDAD'
    end
  end
  object PBotones: TPanel [11]
    Left = 0
    Top = 508
    Width = 683
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 9
    DesignSize = (
      683
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 681
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
      OnClick = BBSalirClick
    end
    object ISalir: TImage
      Left = 584
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = BBSalirClick
    end
    object LSalir: TLabel
      Left = 595
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBSalirClick
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      OnClick = BBXlsClick
    end
    object IPreview: TImage
      Left = 309
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = BBImprimirReporteClick
    end
    object LPreview: TLabel
      Left = 306
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBImprimirReporteClick
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object LExpExcel: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBXlsClick
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 1072
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      OnClick = BBXlsClick
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 683
    Caption = 'Reporte de Mortandad'
    inherited LTitulo: TLabel
      Width = 279
    end
    inherited PILeft: TPanel
      Left = 475
    end
  end
  inherited Psocalo: TPanel
    Top = 604
    Width = 683
    inherited JvSocalo: TJvImage
      Width = 683
    end
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 164
    Top = 15
  end
  inherited SDXLS: TSaveDialog
    Left = 78
    Top = 13
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQInfoMortandad
    Columns = <
      item
        DataField = 'FECHA'
        Title = 'Fecha Muerte'
      end
      item
        DataField = 'RP'
        Title = 'RP'
      end
      item
        DataField = 'FECHANACIMIENTO'
        Title = 'Edad'
      end
      item
        DataField = 'CATEGORIA'
        Title = 'Categor'#237'a'
      end
      item
        DataField = 'POTRERO'
        Title = 'Potrero'
      end
      item
        DataField = 'RODEO'
        Title = 'Rodeo'
      end
      item
        DataField = 'RESPONSABLE'
        Title = 'Responsable'
      end
      item
        DataField = 'ENFERMEDAD'
        Title = 'Enfermedad'
      end
      item
        DataField = 'OBSERVACIONES'
        Title = 'Observaciones'
      end>
    Title.Text = 'Informe de Mortandad'
    Left = 48
    Top = 9
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 116
    Top = 7
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 12
    Top = 7
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 444
    Top = 15
  end
  object IBQDistMuerteEnfermedad: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select enfermedad,count(*) as cantidad from rep_mortandad(:estab' +
        'lecimiento,:grupo,:desde,:hasta,:actual)'
      'group by enfermedad')
    Filtered = True
    Left = 361
    Top = 321
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actual'
        ParamType = ptUnknown
      end>
    object IBQDistMuerteEnfermedadENFERMEDAD: TIBStringField
      FieldName = 'ENFERMEDAD'
      Size = 50
    end
    object IBQDistMuerteEnfermedadCANTIDAD: TIntegerField
      FieldName = 'CANTIDAD'
      Required = True
    end
  end
  object IBQDistMuerteCategoria: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQDistMuerteCategoriaAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select categoria,count(*) as cantidad from rep_mortandad(:establ' +
        'ecimiento,:grupo,:desde,:hasta,:actual)'
      'group by categoria')
    Filtered = True
    Left = 393
    Top = 321
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actual'
        ParamType = ptUnknown
      end>
    object IBQDistMuerteCategoriaCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Size = 50
    end
    object IBQDistMuerteCategoriaCANTIDAD: TIntegerField
      FieldName = 'CANTIDAD'
      Required = True
    end
  end
  object IBQInfoMortandad: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQInfoMortandadAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from rep_mortandad(:establecimiento,:grupo,:desde,:hast' +
        'a,:actual)')
    Left = 553
    Top = 322
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actual'
        ParamType = ptUnknown
      end>
    object IBQInfoMortandadFECHA: TDateField
      FieldName = 'FECHA'
      Origin = 'REP_MORTANDAD.FECHA'
    end
    object IBQInfoMortandadRP: TIBStringField
      FieldName = 'RP'
      Origin = 'REP_MORTANDAD.RP'
      Size = 50
    end
    object IBQInfoMortandadFECHANACIMIENTO: TDateField
      FieldName = 'FECHANACIMIENTO'
      Origin = 'REP_MORTANDAD.FECHANACIMIENTO'
    end
    object IBQInfoMortandadCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Origin = 'REP_MORTANDAD.CATEGORIA'
      Size = 50
    end
    object IBQInfoMortandadPOTRERO: TIBStringField
      FieldName = 'POTRERO'
      Origin = 'REP_MORTANDAD.POTRERO'
      Size = 50
    end
    object IBQInfoMortandadRODEO: TIBStringField
      FieldName = 'RODEO'
      Origin = 'REP_MORTANDAD.RODEO'
      Size = 50
    end
    object IBQInfoMortandadRESPONSABLE: TIBStringField
      FieldName = 'RESPONSABLE'
      Origin = 'REP_MORTANDAD.RESPONSABLE'
      Size = 50
    end
    object IBQInfoMortandadENFERMEDAD: TIBStringField
      FieldName = 'ENFERMEDAD'
      Origin = 'REP_MORTANDAD.ENFERMEDAD'
      Size = 50
    end
    object IBQInfoMortandadOBSERVACIONES: TIBStringField
      FieldName = 'OBSERVACIONES'
      Origin = 'REP_MORTANDAD.OBSERVACIONES'
      Size = 255
    end
    object IBQInfoMortandadTOTALMUERTOS: TIntegerField
      FieldName = 'TOTALMUERTOS'
      Origin = 'REP_MORTANDAD.TOTALMUERTOS'
    end
    object IBQInfoMortandadID_ANIMAL: TIntegerField
      FieldName = 'ID_ANIMAL'
      Origin = 'REP_MORTANDAD.ID_ANIMAL'
    end
    object IBQInfoMortandadEDADCAPTION: TIBStringField
      FieldName = 'EDADCAPTION'
      Origin = 'REP_MORTANDAD.EDADCAPTION'
      Size = 12
    end
  end
  object DSInfoMortandad: TDataSource
    DataSet = IBQInfoMortandad
    Left = 585
    Top = 322
  end
  object IBQGrupos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select id_grupo, nombre from tab_grupos where establecimiento=:e' +
        'sta and tipo_evento=13 and visible=1 order by upper(nombre) asc')
    Left = 264
    Top = 8
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object DSGrupos: TDataSource
    DataSet = IBQGrupos
    Left = 296
    Top = 8
  end
  object IBQExistenciaPromedioTotal: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT promedioexistencia from GET_EXISTENCIA_PROMEDIO_TOTAL(:es' +
        'tablecimiento,:actividad,:desde,:hasta)')
    Filtered = True
    Left = 433
    Top = 321
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end>
    object IBQExistenciaPromedioTotalPROMEDIOEXISTENCIA: TIBBCDField
      FieldName = 'PROMEDIOEXISTENCIA'
      Origin = 'GET_EXISTENCIA_PROMEDIO_TOTAL.PROMEDIOEXISTENCIA'
      Precision = 18
      Size = 2
    end
  end
  object IBQDistMuertesMensual: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQDistMuertesMensualAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select extract (month from fecha) as mes,extract (YEAR from fech' +
        'a) as ANIO, count(*) as cantidad'
      
        'from rep_mortandad(:establecimiento,:grupo,:desde,:hasta,:actual' +
        ')'
      'group by extract (month from fecha),extract (YEAR from fecha)'
      'order by extract (YEAR from fecha),extract (month from fecha)')
    Filtered = True
    Left = 473
    Top = 321
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actual'
        ParamType = ptUnknown
      end>
    object IBQDistMuertesMensualMES: TSmallintField
      FieldName = 'MES'
    end
    object IBQDistMuertesMensualANIO: TSmallintField
      FieldName = 'ANIO'
    end
    object IBQDistMuertesMensualCANTIDAD: TIntegerField
      FieldName = 'CANTIDAD'
      Required = True
    end
  end
end
