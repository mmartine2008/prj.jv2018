inherited FREPProduccionCarne: TFREPProduccionCarne
  Left = 291
  Top = -17
  Caption = 'FREPProduccionCarne'
  ClientHeight = 730
  ClientWidth = 1046
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 1046
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 624
  end
  object PDetalle: TPanel [4]
    Left = 0
    Top = 217
    Width = 1046
    Height = 395
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object Shape8: TShape
      Left = 205
      Top = 206
      Width = 268
      Height = 49
    end
    object Shape1: TShape
      Left = 352
      Top = 115
      Width = 294
      Height = 46
      Pen.Width = 2
    end
    object Label1: TLabel
      Left = 359
      Top = 121
      Width = 113
      Height = 14
      Caption = 'Prod. Carne /ha:   kg/ha'
    end
    object Label4: TLabel
      Left = 359
      Top = 139
      Width = 106
      Height = 14
      Caption = 'Ingreso Neto /ha: $/ha'
    end
    object LProdHa: TLabel
      Left = 477
      Top = 121
      Width = 107
      Height = 13
      AutoSize = False
      Caption = '0'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LIngresoHa: TLabel
      Left = 477
      Top = 139
      Width = 107
      Height = 13
      AutoSize = False
      Caption = '0'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Shape2: TShape
      Left = 197
      Top = 138
      Width = 156
      Height = 2
    end
    object Shape3: TShape
      Left = 645
      Top = 137
      Width = 163
      Height = 2
    end
    object Shape4: TShape
      Left = 489
      Top = 3
      Width = 3
      Height = 113
    end
    object Shape5: TShape
      Left = 489
      Top = 160
      Width = 3
      Height = 123
    end
    object Shape6: TShape
      Left = 205
      Top = 44
      Width = 251
      Height = 47
    end
    object Shape7: TShape
      Left = 524
      Top = 44
      Width = 251
      Height = 47
    end
    object Shape9: TShape
      Left = 524
      Top = 206
      Width = 268
      Height = 49
    end
    object Shape10: TShape
      Left = 205
      Top = 230
      Width = 268
      Height = 2
    end
    object Shape11: TShape
      Left = 524
      Top = 230
      Width = 268
      Height = 2
    end
    object Shape12: TShape
      Left = 205
      Top = 66
      Width = 251
      Height = 1
    end
    object Shape13: TShape
      Left = 524
      Top = 65
      Width = 251
      Height = 1
    end
    object Shape14: TShape
      Left = 283
      Top = 206
      Width = 1
      Height = 48
    end
    object Shape15: TShape
      Left = 372
      Top = 206
      Width = 1
      Height = 48
    end
    object Shape16: TShape
      Left = 609
      Top = 207
      Width = 1
      Height = 47
    end
    object Shape17: TShape
      Left = 695
      Top = 208
      Width = 1
      Height = 46
    end
    object Shape18: TShape
      Left = 282
      Top = 44
      Width = 1
      Height = 46
    end
    object Shape19: TShape
      Left = 371
      Top = 44
      Width = 1
      Height = 47
    end
    object Shape24: TShape
      Left = 607
      Top = 44
      Width = 1
      Height = 47
    end
    object Shape25: TShape
      Left = 693
      Top = 44
      Width = 1
      Height = 47
    end
    object Label5: TLabel
      Left = 300
      Top = 9
      Width = 54
      Height = 16
      Caption = 'Salidas'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 619
      Top = 9
      Width = 63
      Height = 16
      Caption = 'Entradas'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label7: TLabel
      Left = 263
      Top = 171
      Width = 118
      Height = 16
      Caption = 'Existencia FINAL'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 591
      Top = 171
      Width = 127
      Height = 16
      Caption = 'Existencia INICIAL'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label9: TLabel
      Left = 211
      Top = 48
      Width = 19
      Height = 14
      Alignment = taCenter
      Caption = 'Cab'
    end
    object Label10: TLabel
      Left = 288
      Top = 48
      Width = 13
      Height = 14
      Alignment = taCenter
      Caption = 'Kg'
    end
    object Label11: TLabel
      Left = 376
      Top = 48
      Width = 38
      Height = 14
      Alignment = taCenter
      Caption = '$ (neto)'
    end
    object Label12: TLabel
      Left = 698
      Top = 48
      Width = 38
      Height = 14
      Alignment = taCenter
      Caption = '$ (neto)'
    end
    object Label13: TLabel
      Left = 612
      Top = 48
      Width = 13
      Height = 14
      Alignment = taCenter
      Caption = 'Kg'
    end
    object Label14: TLabel
      Left = 529
      Top = 48
      Width = 19
      Height = 14
      Alignment = taCenter
      Caption = 'Cab'
    end
    object Label15: TLabel
      Left = 378
      Top = 211
      Width = 90
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '$ (neto)'
    end
    object Label16: TLabel
      Left = 289
      Top = 211
      Width = 79
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = 'Kg'
    end
    object Label17: TLabel
      Left = 211
      Top = 211
      Width = 67
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = 'Cab'
    end
    object Label18: TLabel
      Left = 699
      Top = 211
      Width = 38
      Height = 14
      Alignment = taCenter
      Caption = '$ (neto)'
    end
    object Label19: TLabel
      Left = 613
      Top = 211
      Width = 13
      Height = 14
      Alignment = taCenter
      Caption = 'Kg'
    end
    object Label20: TLabel
      Left = 531
      Top = 211
      Width = 19
      Height = 14
      Alignment = taCenter
      Caption = 'Cab'
    end
    object Shape26: TShape
      Left = 145
      Top = 285
      Width = 147
      Height = 46
    end
    object Label21: TLabel
      Left = 164
      Top = 300
      Width = 114
      Height = 14
      Alignment = taCenter
      Caption = 'Diferencia de Inventario'
    end
    object Shape27: TShape
      Left = 317
      Top = 285
      Width = 165
      Height = 45
    end
    object Shape28: TShape
      Left = 389
      Top = 286
      Width = 1
      Height = 42
    end
    object Shape29: TShape
      Left = 318
      Top = 307
      Width = 164
      Height = 1
    end
    object Label22: TLabel
      Left = 322
      Top = 290
      Width = 13
      Height = 14
      Alignment = taCenter
      Caption = 'Kg'
    end
    object Label23: TLabel
      Left = 395
      Top = 290
      Width = 38
      Height = 14
      Alignment = taCenter
      Caption = '$ (neto)'
    end
    object Label24: TLabel
      Left = 507
      Top = 290
      Width = 125
      Height = 14
      Caption = 'Existencia promedio diaria'
    end
    object Label25: TLabel
      Left = 507
      Top = 310
      Width = 98
      Height = 14
      Caption = 'Superficie ganadera'
    end
    object LExistPromDiaria: TLabel
      Left = 662
      Top = 289
      Width = 70
      Height = 14
      AutoSize = False
      Caption = '0'
      Color = clInfoBk
      ParentColor = False
    end
    object LSupGanadera: TLabel
      Left = 662
      Top = 312
      Width = 70
      Height = 14
      AutoSize = False
      Caption = '0'
      Color = clInfoBk
      ParentColor = False
    end
    object Label26: TLabel
      Left = 739
      Top = 288
      Width = 19
      Height = 14
      Caption = 'Cab'
    end
    object Label27: TLabel
      Left = 739
      Top = 310
      Width = 18
      Height = 14
      Caption = 'has'
    end
    object Label28: TLabel
      Left = 145
      Top = 337
      Width = 150
      Height = 13
      Caption = 'PRODUCCION DE CARNE'
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label29: TLabel
      Left = 145
      Top = 354
      Width = 159
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = 'INGRESO NETO'
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label30: TLabel
      Left = 366
      Top = 337
      Width = 18
      Height = 13
      Caption = 'KG'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label31: TLabel
      Left = 334
      Top = 354
      Width = 51
      Height = 13
      Caption = '$ (netos)'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Shape30: TShape
      Left = 403
      Top = 335
      Width = 88
      Height = 16
    end
    object Shape31: TShape
      Left = 403
      Top = 352
      Width = 88
      Height = 16
    end
    object Shape32: TShape
      Left = 507
      Top = 335
      Width = 268
      Height = 16
    end
    object Shape22: TShape
      Left = 507
      Top = 352
      Width = 268
      Height = 16
    end
    object Label32: TLabel
      Left = 513
      Top = 336
      Width = 35
      Height = 14
      Caption = 'Por Ha:'
    end
    object Label33: TLabel
      Left = 513
      Top = 353
      Width = 35
      Height = 14
      Caption = 'Por Ha:'
    end
    object Shape23: TShape
      Left = 128
      Top = 436
      Width = 212
      Height = 21
    end
    object Label34: TLabel
      Left = 131
      Top = 439
      Width = 175
      Height = 14
      Caption = 'PROMEDIO EXISTENCIAS HACIENDA'
    end
    object Label35: TLabel
      Left = 368
      Top = 370
      Width = 18
      Height = 13
      Caption = 'KG'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label36: TLabel
      Left = 347
      Top = 436
      Width = 23
      Height = 13
      Caption = 'Cab'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label37: TLabel
      Left = 352
      Top = 457
      Width = 17
      Height = 13
      Caption = 'EV'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Shape33: TShape
      Left = 403
      Top = 369
      Width = 88
      Height = 19
      Brush.Color = clInfoBk
    end
    object Shape34: TShape
      Left = 403
      Top = 434
      Width = 88
      Height = 19
      Brush.Color = clInfoBk
    end
    object Shape35: TShape
      Left = 403
      Top = 452
      Width = 88
      Height = 20
      Brush.Color = clInfoBk
    end
    object Shape36: TShape
      Left = 507
      Top = 369
      Width = 173
      Height = 19
      Brush.Color = clInactiveCaptionText
    end
    object Label38: TLabel
      Left = 509
      Top = 372
      Width = 114
      Height = 13
      Alignment = taCenter
      Caption = 'EFICIENCIA STOCK'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label39: TLabel
      Left = 696
      Top = 371
      Width = 10
      Height = 13
      Caption = '%'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Shape37: TShape
      Left = 722
      Top = 369
      Width = 79
      Height = 19
    end
    object LCabExistFinal: TLabel
      Left = 210
      Top = 237
      Width = 68
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LCabExistInicial: TLabel
      Left = 528
      Top = 237
      Width = 76
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKGExistFinal: TLabel
      Left = 288
      Top = 237
      Width = 79
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKGExistInicial: TLabel
      Left = 612
      Top = 237
      Width = 79
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKGDifInventario: TLabel
      Left = 320
      Top = 311
      Width = 65
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LCabSalidas: TLabel
      Left = 210
      Top = 71
      Width = 67
      Height = 17
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LCabEntradas: TLabel
      Left = 527
      Top = 71
      Width = 76
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKgTotalDestino: TLabel
      Left = 286
      Top = 71
      Width = 81
      Height = 17
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKgTotalCampo: TLabel
      Left = 612
      Top = 71
      Width = 77
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LValorVenta: TLabel
      Left = 376
      Top = 71
      Width = 75
      Height = 17
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LValorCompra: TLabel
      Left = 698
      Top = 71
      Width = 73
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LPesosDifInventario: TLabel
      Left = 395
      Top = 311
      Width = 81
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LKGProduccionCarne: TLabel
      Left = 408
      Top = 336
      Width = 6
      Height = 14
      Alignment = taCenter
      Caption = '0'
    end
    object LNetoExistFinal: TLabel
      Left = 377
      Top = 237
      Width = 89
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LNetoExistInicial: TLabel
      Left = 699
      Top = 237
      Width = 87
      Height = 14
      Alignment = taCenter
      AutoSize = False
      Caption = '0'
      Transparent = True
    end
    object LIngresoNeto: TLabel
      Left = 408
      Top = 353
      Width = 6
      Height = 14
      Alignment = taCenter
      Caption = '0'
    end
    object LKgProduccionCarneHa: TLabel
      Left = 558
      Top = 336
      Width = 6
      Height = 14
      Caption = '0'
    end
    object Label42: TLabel
      Left = 625
      Top = 336
      Width = 26
      Height = 14
      Caption = 'kg/ha'
    end
    object LIngresoNetoHa: TLabel
      Left = 558
      Top = 353
      Width = 6
      Height = 14
      Caption = '0'
    end
    object Label43: TLabel
      Left = 625
      Top = 353
      Width = 21
      Height = 14
      Caption = '$/ha'
    end
    object LCabExistHacienda: TLabel
      Left = 429
      Top = 436
      Width = 6
      Height = 14
      Caption = '0'
      Transparent = True
    end
    object LKGExistHacienda: TLabel
      Left = 408
      Top = 372
      Width = 6
      Height = 14
      Alignment = taCenter
      Caption = '0'
      Transparent = True
    end
    object LEficienciaStock: TLabel
      Left = 727
      Top = 372
      Width = 6
      Height = 14
      Alignment = taCenter
      Caption = '0'
      Transparent = True
    end
    object LEVExistHacienda: TLabel
      Left = 429
      Top = 453
      Width = 6
      Height = 14
      Caption = '0'
      Transparent = True
    end
    object Label41: TLabel
      Left = 601
      Top = 121
      Width = 26
      Height = 14
      Caption = 'kg/ha'
    end
    object Label44: TLabel
      Left = 601
      Top = 139
      Width = 21
      Height = 14
      Caption = '$/ha'
    end
    object BBEntradasSalidas: TBitBtn
      Left = 435
      Top = 5
      Width = 113
      Height = 27
      Caption = 'Entradas y Salidas...'
      Enabled = False
      TabOrder = 0
      OnClick = BBEntradasSalidasClick
    end
    object BBCambiarSuperficieGanadera: TBitBtn
      Left = 765
      Top = 304
      Width = 113
      Height = 22
      Caption = 'Cambiar Superficie'
      TabOrder = 1
      OnClick = BBCambiarSuperficieGanaderaClick
    end
    object EACambiarSuperficieGanadera: TEditAuto
      Left = 657
      Top = 308
      Width = 75
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      TabOrder = 2
      Visible = False
      OnKeyPress = EACambiarSuperficieGanaderaKeyPress
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object BBCancelarSuperficie: TBitBtn
      Left = 799
      Top = 330
      Width = 78
      Height = 21
      Caption = 'Cancelar'
      TabOrder = 3
      Visible = False
      OnClick = BBCancelarSuperficieClick
    end
  end
  object PFiltros: TPanel [5]
    Left = 0
    Top = 65
    Width = 1046
    Height = 152
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object GroupBox5: TGroupBox
      Left = 382
      Top = 1
      Width = 292
      Height = 150
      Align = alLeft
      Caption = 'Valores Utilizados (Kg/$)'
      TabOrder = 0
      object Shape21: TShape
        Left = 4
        Top = 103
        Width = 355
        Height = 1
      end
      object LNombreSalidas: TLabel
        Left = 211
        Top = 33
        Width = 36
        Height = 14
        Caption = 'nombre'
      end
      object LFechaSalidas: TLabel
        Left = 128
        Top = 33
        Width = 28
        Height = 14
        Caption = 'fecha'
      end
      object LSaliads: TLabel
        Left = 7
        Top = 33
        Width = 46
        Height = 13
        Caption = 'Salidas:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object LEntradas: TLabel
        Left = 7
        Top = 51
        Width = 55
        Height = 13
        Caption = 'Entradas:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object LFechaEntradas: TLabel
        Left = 128
        Top = 51
        Width = 28
        Height = 14
        Caption = 'fecha'
      end
      object LNombreEntradas: TLabel
        Left = 211
        Top = 51
        Width = 36
        Height = 14
        Caption = 'nombre'
      end
      object LNombreExistenciaFinal: TLabel
        Left = 211
        Top = 68
        Width = 36
        Height = 14
        Caption = 'nombre'
      end
      object LFechaExistenciaFinal: TLabel
        Left = 128
        Top = 68
        Width = 28
        Height = 14
        Caption = 'fecha'
      end
      object LExistenciaFinal: TLabel
        Left = 7
        Top = 68
        Width = 94
        Height = 13
        Caption = 'Existencia Final:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object LExistenciaInicial: TLabel
        Left = 7
        Top = 85
        Width = 101
        Height = 13
        Caption = 'Existencia Inicial:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object LFechaExistenciaInicial: TLabel
        Left = 128
        Top = 85
        Width = 28
        Height = 14
        Caption = 'fecha'
      end
      object LNombreExistenciaInicial: TLabel
        Left = 211
        Top = 85
        Width = 36
        Height = 14
        Caption = 'nombre'
      end
      object Shape20: TShape
        Left = 6
        Top = 26
        Width = 355
        Height = 1
      end
      object BBCambiar: TBitBtn
        Left = 9
        Top = 111
        Width = 121
        Height = 27
        Caption = 'Cambiar Valores...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        OnClick = BBCambiarClick
      end
    end
    object Panel1: TPanel
      Left = 159
      Top = 1
      Width = 223
      Height = 150
      Align = alLeft
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 1
      object GroupBox2: TGroupBox
        Left = 0
        Top = 0
        Width = 223
        Height = 87
        Align = alTop
        Caption = 'Actividad'
        TabOrder = 0
        object RBTodas: TRadioButton
          Left = 9
          Top = 25
          Width = 70
          Height = 19
          Caption = 'Todas'
          Checked = True
          TabOrder = 0
          TabStop = True
          OnClick = RBTodasClick
        end
        object RBActividad: TRadioButton
          Left = 9
          Top = 54
          Width = 78
          Height = 18
          Caption = 'Actividad'
          TabOrder = 1
          OnClick = RBActividadClick
        end
        object DBLCBAActividad: TDBLookupComboBoxAuto
          Left = 84
          Top = 51
          Width = 133
          Height = 22
          Color = clWhite
          Enabled = False
          KeyField = 'ID_ACTIVIDAD'
          ListField = 'SINONIMO'
          ListSource = DMSoftvet.DSActividad
          TabOrder = 2
          OnCloseUp = DBLCBAActividadCloseUp
          OnKeyUp = DBLCBAActividadKeyUp
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
      end
      object GroupBox3: TGroupBox
        Left = 0
        Top = 87
        Width = 223
        Height = 63
        Align = alClient
        Caption = 'Detalle'
        TabOrder = 1
        object CBDetalle: TCheckBox
          Left = 9
          Top = 17
          Width = 113
          Height = 19
          Caption = 'Imprimir detalle'
          Checked = True
          State = cbChecked
          TabOrder = 0
        end
        object CBPrecio: TCheckBox
          Left = 113
          Top = 16
          Width = 96
          Height = 19
          Caption = 'Calcular Capital'
          Checked = True
          State = cbChecked
          TabOrder = 1
          OnClick = CBPrecioClick
        end
        object CBEv: TCheckBox
          Left = 9
          Top = 40
          Width = 88
          Height = 19
          Caption = 'Calcular EV'
          TabOrder = 2
          OnClick = CBPrecioClick
        end
      end
    end
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 158
      Height = 150
      Align = alLeft
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 2
      object GroupBox1: TGroupBox
        Left = 0
        Top = 0
        Width = 158
        Height = 87
        Align = alTop
        Caption = 'Ejercicio'
        TabOrder = 0
        object Label2: TLabel
          Left = 5
          Top = 29
          Width = 34
          Height = 14
          Caption = 'Desde:'
        end
        object Label3: TLabel
          Left = 6
          Top = 58
          Width = 31
          Height = 14
          Caption = 'Hasta:'
        end
        object DTPADesde: TDateTimePickerAuto
          Left = 50
          Top = 26
          Width = 104
          Height = 21
          Date = 39333.689785150460000000
          Time = 39333.689785150460000000
          Color = clWhite
          TabOrder = 0
          OnCloseUp = closeUpFechas
          OnChange = DTPADesdeChange
          OnKeyUp = DTPAFechasKeyUp
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
        object DTPAHasta: TDateTimePickerAuto
          Left = 50
          Top = 54
          Width = 104
          Height = 21
          Date = 39699.689785150460000000
          Time = 39699.689785150460000000
          Color = clWhite
          TabOrder = 1
          OnCloseUp = closeUpFechas
          OnChange = DTPAHastaChange
          OnKeyUp = DTPAFechasKeyUp
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
      end
      object GroupBox7: TGroupBox
        Left = 0
        Top = 87
        Width = 158
        Height = 63
        Align = alClient
        Caption = 'Consulta'
        TabOrder = 1
        object JvIBotonBuscar: TJvImage
          Tag = 4
          Left = 27
          Top = 23
          Width = 97
          Height = 32
          Cursor = crHandPoint
          Picture.Data = {
            
            E9FF0056DF9FC352571EAFFFD9}
          OnClick = JvIBotonBuscarClick
        end
        object BBCalcular: TBitBtn
          Left = 44
          Top = 40
          Width = 69
          Height = 17
          Caption = 'Calcular'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 0
          Visible = False
          OnClick = BBCalcularClick
        end
      end
    end
    object GBCategoria: TGroupBox
      Left = 674
      Top = 1
      Width = 371
      Height = 150
      Align = alClient
      Caption = 'Buscar por Categor'#237'a'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      object PMangaCategoria: TPanel
        Left = 2
        Top = 16
        Width = 367
        Height = 132
        Align = alClient
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 0
        object DBGDisponibles: TDBGrid
          Left = 8
          Top = 0
          Width = 129
          Height = 127
          DataSource = DSDisponibles
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Options = [dgTitles, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Arial'
          TitleFont.Style = []
          Columns = <
            item
              Color = clWhite
              Expanded = False
              FieldName = 'NOMBRE'
              Title.Alignment = taCenter
              Title.Caption = 'Disponibles'
              Title.Color = clTeal
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWhite
              Title.Font.Height = -13
              Title.Font.Name = 'Arial'
              Title.Font.Style = []
              Width = 120
              Visible = True
            end>
        end
        object DBGSeleccionados: TDBGrid
          Left = 191
          Top = 0
          Width = 135
          Height = 126
          DataSource = DSSeleccionados
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Options = [dgTitles, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Arial'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'NOMBRE'
              Title.Alignment = taCenter
              Title.Caption = 'Seleccionadas'
              Title.Color = clTeal
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWhite
              Title.Font.Height = -13
              Title.Font.Name = 'Arial'
              Title.Font.Style = []
              Width = 131
              Visible = True
            end>
        end
        object BPasar1: TButton
          Left = 144
          Top = 43
          Width = 41
          Height = 25
          Caption = '>'
          TabOrder = 2
          OnClick = BPasar1Click
        end
        object BSacar1: TButton
          Left = 144
          Top = 75
          Width = 41
          Height = 25
          Caption = '<'
          Enabled = False
          TabOrder = 3
          OnClick = BSacar1Click
        end
      end
    end
  end
  object PBotones: TPanel [6]
    Left = 0
    Top = 612
    Width = 1046
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 4
    DesignSize = (
      1046
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 1044
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 920
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = BBSalirClick
    end
    object LSalir: TLabel
      Left = 931
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBSalirClick
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      Visible = False
    end
    object IPreview: TImage
      Left = 509
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = BBImprimirReporteClick
    end
    object LPreview: TLabel
      Left = 506
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBImprimirReporteClick
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object Label45: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 937
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 1046
    Caption = 'Producci'#243'n de Carne'
    inherited LTitulo: TLabel
      Width = 642
      Caption = 'Producci'#243'n de Carne'
    end
    inherited PILeft: TPanel
      Left = 838
    end
  end
  inherited Psocalo: TPanel
    Top = 708
    Width = 1046
    inherited JvSocalo: TJvImage
      Width = 1046
    end
  end
  inherited ALUniversal: TActionList
    Left = 227
    Top = 65534
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 84
    Top = 65535
  end
  inherited SDXLS: TSaveDialog
    Left = 142
    Top = 65533
  end
  inherited DTXLS: TDataToXLS
    Left = 112
    Top = 1
  end
  inherited WinXP1: TWinXP
    Left = 0
    Top = 0
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 196
    Top = 65535
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 292
    Top = 65535
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 28
    Top = 65535
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 172
    Top = 65535
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 260
    Top = 65535
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 324
    Top = 65535
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 56
    Top = 0
  end
  inherited PLVersion: TPngImageList
    Left = 568
    Top = 24
  end
  object IBQREPProduccionSalidas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQREPProduccionSalidasCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select fecha, tipobaja, categoria, count(id_animal) as cabezas,'
      
        '       avg(pesocampo) as prompesocampo, avg(pesodestino) as prom' +
        'pesodestino,'
      
        '       avg(preciokilo) as preciokilo, idcategoria, idaltabaja, t' +
        'ipodetalle, avg(precio) as precio, avg(gastos) as gastos'
      
        'from rep_produccion_carne(:establecimiento,:actividad,:desde,:ha' +
        'sta,:fechareferencia,:precio)'
      'where TIPODETALLE = '#39'SALIDAS'#39
      
        'group by fecha, tipobaja, categoria,idcategoria, idaltabaja, tip' +
        'odetalle')
    Left = 872
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fechareferencia'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'precio'
        ParamType = ptUnknown
      end>
    object IBQREPProduccionSalidasFECHA: TDateField
      FieldName = 'FECHA'
    end
    object IBQREPProduccionSalidasTIPOBAJA: TIBStringField
      FieldName = 'TIPOBAJA'
      Size = 50
    end
    object IBQREPProduccionSalidasCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Size = 50
    end
    object IBQREPProduccionSalidasCABEZAS: TIntegerField
      FieldName = 'CABEZAS'
      Required = True
    end
    object IBQREPProduccionSalidasKGTOTALDESTINO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'KGTOTALDESTINO'
      Calculated = True
    end
    object IBQREPProduccionSalidasDESBASTE: TFloatField
      FieldKind = fkCalculated
      FieldName = 'DESBASTE'
      Calculated = True
    end
    object IBQREPProduccionSalidasIDCATEGORIA: TIntegerField
      FieldName = 'IDCATEGORIA'
    end
    object IBQREPProduccionSalidasIDALTABAJA: TIntegerField
      FieldName = 'IDALTABAJA'
    end
    object IBQREPProduccionSalidasTIPODETALLE: TIBStringField
      FieldName = 'TIPODETALLE'
      Size = 50
    end
    object IBQREPProduccionSalidasPROMPESOCAMPO: TFloatField
      FieldName = 'PROMPESOCAMPO'
      DisplayFormat = '#0.##'
    end
    object IBQREPProduccionSalidasPROMPESODESTINO2: TFloatField
      FieldName = 'PROMPESODESTINO'
      DisplayFormat = '#0.##'
    end
    object IBQREPProduccionSalidasPRECIOKILO: TIBBCDField
      FieldName = 'PRECIOKILO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionSalidasPRECIO: TIBBCDField
      FieldName = 'PRECIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionSalidasGASTOS: TIBBCDField
      FieldName = 'GASTOS'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionSalidasGASTOSCOMER: TIBBCDField
      FieldKind = fkCalculated
      FieldName = 'GASTOSCOMER'
      DisplayFormat = '#0.##'
      Calculated = True
    end
    object IBQREPProduccionSalidasVALORVENTA: TIBBCDField
      FieldKind = fkCalculated
      FieldName = 'VALORVENTA'
      DisplayFormat = '#0.##'
      Calculated = True
    end
  end
  object DSREPProduccionSalidas: TDataSource
    DataSet = IBQREPProduccionSalidas
    Left = 840
    Top = 17
  end
  object DSREPProduccionEntradas: TDataSource
    DataSet = IBQREPProduccionEntradas
    Left = 464
    Top = 649
  end
  object IBQREPProduccionEntradas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQREPProduccionEntradasCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select fecha, tipobaja, categoria, count(id_animal) as cabezas,'
      
        '       avg(pesocampo) as prompesocampo, avg(pesodestino) as prom' +
        'pesodestino,'
      
        '       avg(preciokilo) as preciokilo, idcategoria, idaltabaja, t' +
        'ipodetalle, avg(precio) as precio, avg(gastos) as gastos'
      
        'from rep_produccion_carne(:establecimiento,:actividad,:desde,:ha' +
        'sta,:fechareferencia,:precio)'
      'where TIPODETALLE = '#39'ENTRADAS'#39
      
        'group by fecha, tipobaja, categoria, idcategoria, idaltabaja, ti' +
        'podetalle')
    Left = 496
    Top = 649
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fechareferencia'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'precio'
        ParamType = ptUnknown
      end>
    object IBQREPProduccionEntradasFECHA: TDateField
      FieldName = 'FECHA'
    end
    object IBQREPProduccionEntradasTIPOBAJA: TIBStringField
      FieldName = 'TIPOBAJA'
      Size = 50
    end
    object IBQREPProduccionEntradasCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Size = 50
    end
    object IBQREPProduccionEntradasCABEZAS: TIntegerField
      FieldName = 'CABEZAS'
      Required = True
    end
    object IBQREPProduccionEntradasKGTOTALCAMPO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'KGTOTALCAMPO'
      Calculated = True
    end
    object IBQREPProduccionEntradasIDCATEGORIA: TIntegerField
      FieldName = 'IDCATEGORIA'
    end
    object IBQREPProduccionEntradasIDALTABAJA: TIntegerField
      FieldName = 'IDALTABAJA'
    end
    object IBQREPProduccionEntradasTIPODETALLE: TIBStringField
      FieldName = 'TIPODETALLE'
      Size = 50
    end
    object IBQREPProduccionEntradasPROMPESOCAMPO: TFloatField
      FieldName = 'PROMPESOCAMPO'
      DisplayFormat = '#0.##'
    end
    object IBQREPProduccionEntradasPROMPESODESTINO: TFloatField
      FieldName = 'PROMPESODESTINO'
      DisplayFormat = '#0.##'
    end
    object IBQREPProduccionEntradasPRECIOKILO: TIBBCDField
      FieldName = 'PRECIOKILO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionEntradasPRECIO: TIBBCDField
      FieldName = 'PRECIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionEntradasGASTOS: TIBBCDField
      FieldName = 'GASTOS'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQREPProduccionEntradasVALORCOMPRA: TIBBCDField
      FieldKind = fkCalculated
      FieldName = 'VALORCOMPRA'
      DisplayFormat = '#0.##'
      Calculated = True
    end
    object IBQREPProduccionEntradasGASTOSCOMER: TIBBCDField
      FieldKind = fkCalculated
      FieldName = 'GASTOSCOMER'
      DisplayFormat = '#0.##'
      Calculated = True
    end
  end
  object IBQGetExistenciaFinal: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaFinalCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from get_existencia(:establecimiento, :actividad, :hast' +
        'a,:precio)'
      ' ')
    Left = 640
    Top = 625
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'precio'
        ParamType = ptUnknown
      end>
    object IBQGetExistenciaFinalANIMAL: TIntegerField
      FieldName = 'ANIMAL'
      Origin = 'GET_EXISTENCIA.ANIMAL'
    end
    object IBQGetExistenciaFinalCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Origin = 'GET_EXISTENCIA.CATEGORIA'
      Size = 50
    end
    object IBQGetExistenciaFinalPESOCAMPO: TFloatField
      FieldName = 'PESOCAMPO'
      Origin = 'GET_EXISTENCIA.PESOCAMPO'
      DisplayFormat = '#0.##'
    end
    object IBQGetExistenciaFinalTOTALPRECIO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'TOTALPRECIO'
      DisplayFormat = '#0.##'
      Calculated = True
    end
    object IBQGetExistenciaFinalIDCATEGORIA: TIntegerField
      FieldName = 'IDCATEGORIA'
      Origin = 'GET_EXISTENCIA.IDCATEGORIA'
    end
    object IBQGetExistenciaFinalEQUIV: TFloatField
      FieldName = 'EQUIV'
      Origin = 'GET_EXISTENCIA.EQUIV'
    end
    object IBQGetExistenciaFinalEQUIVTOTAL: TFloatField
      FieldKind = fkCalculated
      FieldName = 'EQUIVTOTAL'
      Calculated = True
    end
    object IBQGetExistenciaFinalTOTALEXISTENCIA: TIBBCDField
      FieldName = 'TOTALEXISTENCIA'
      Origin = 'GET_EXISTENCIA.TOTALEXISTENCIA'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaFinalKILOSTOTAL: TIBBCDField
      FieldName = 'KILOSTOTAL'
      Origin = 'GET_EXISTENCIA.KILOSTOTAL'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaFinalPROMPESOCAMPO: TIBBCDField
      FieldName = 'PROMPESOCAMPO'
      Origin = 'GET_EXISTENCIA.PROMPESOCAMPO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaFinalPRECIOUNITARIO: TIBBCDField
      FieldName = 'PRECIOUNITARIO'
      Origin = 'GET_EXISTENCIA.PRECIOUNITARIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaFinalPROMPRECIOUNITARIO: TIBBCDField
      FieldName = 'PROMPRECIOUNITARIO'
      Origin = 'GET_EXISTENCIA.PROMPRECIOUNITARIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaFinalGASTOS: TIBBCDField
      FieldName = 'GASTOS'
      Origin = 'GET_EXISTENCIA.GASTOS'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
  end
  object DSGetExistenciaFinal: TDataSource
    DataSet = IBQGetExistenciaFinal
    Left = 592
    Top = 625
  end
  object DSGetExistenciaInicial: TDataSource
    DataSet = IBQGetExistenciaInicial
    Left = 776
    Top = 17
  end
  object IBQGetExistenciaInicial: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaInicialCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from get_existencia(:establecimiento,:actividad,:hasta,' +
        ':precio)')
    Left = 808
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'precio'
        ParamType = ptUnknown
      end>
    object IBQGetExistenciaInicialANIMAL: TIntegerField
      FieldName = 'ANIMAL'
      Origin = 'GET_EXISTENCIA.ANIMAL'
    end
    object IBQGetExistenciaInicialCATEGORIA: TIBStringField
      FieldName = 'CATEGORIA'
      Origin = 'GET_EXISTENCIA.CATEGORIA'
      Size = 50
    end
    object IBQGetExistenciaInicialPESOCAMPO: TFloatField
      FieldName = 'PESOCAMPO'
      Origin = 'GET_EXISTENCIA.PESOCAMPO'
      DisplayFormat = '#0.##'
    end
    object IBQGetExistenciaInicialTOTALPRECIO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'TOTALPRECIO'
      DisplayFormat = '#0.##'
      Calculated = True
    end
    object IBQGetExistenciaInicialIDCATEGORIA: TIntegerField
      FieldName = 'IDCATEGORIA'
      Origin = 'GET_EXISTENCIA.IDCATEGORIA'
    end
    object IBQGetExistenciaInicialEQUIV: TFloatField
      FieldName = 'EQUIV'
      Origin = 'GET_EXISTENCIA.EQUIV'
    end
    object IBQGetExistenciaInicialEQUIVTOTAL: TFloatField
      FieldKind = fkCalculated
      FieldName = 'EQUIVTOTAL'
      Calculated = True
    end
    object IBQGetExistenciaInicialTOTALEXISTENCIA: TIBBCDField
      FieldName = 'TOTALEXISTENCIA'
      Origin = 'GET_EXISTENCIA.TOTALEXISTENCIA'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaInicialKILOSTOTAL: TIBBCDField
      FieldName = 'KILOSTOTAL'
      Origin = 'GET_EXISTENCIA.KILOSTOTAL'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaInicialPROMPESOCAMPO: TIBBCDField
      FieldName = 'PROMPESOCAMPO'
      Origin = 'GET_EXISTENCIA.PROMPESOCAMPO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaInicialPRECIOUNITARIO: TIBBCDField
      FieldName = 'PRECIOUNITARIO'
      Origin = 'GET_EXISTENCIA.PRECIOUNITARIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaInicialPROMPRECIOUNITARIO: TIBBCDField
      FieldName = 'PROMPRECIOUNITARIO'
      Origin = 'GET_EXISTENCIA.PROMPRECIOUNITARIO'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
    object IBQGetExistenciaInicialGASTOS: TIBBCDField
      FieldName = 'GASTOS'
      Origin = 'GET_EXISTENCIA.GASTOS'
      DisplayFormat = '#0.##'
      Precision = 18
      Size = 3
    end
  end
  object DSSupGanaderaEfectiva: TDataSource
    Left = 768
    Top = 617
  end
  object IBQExistenciaPromedioTotal: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT promedioexistencia from GET_EXISTENCIA_PROMEDIO_TOTAL(:es' +
        'tablecimiento,:actividad,:desde,:hasta)')
    Filtered = True
    Left = 801
    Top = 585
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end>
    object IBQExistenciaPromedioTotalPROMEDIOEXISTENCIA: TIBBCDField
      FieldName = 'PROMEDIOEXISTENCIA'
      Origin = 'GET_EXISTENCIA_PROMEDIO_TOTAL.PROMEDIOEXISTENCIA'
      Precision = 18
      Size = 2
    end
  end
  object IBQExistPromEV: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT totalexistencia, promedioexistencia from GET_EXIST_PROM_E' +
        'V(:establecimiento,:actividad,:desde,:hasta)')
    Filtered = True
    Left = 769
    Top = 681
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'actividad'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end>
    object IBQExistPromEVPROMEDIOEXISTENCIA: TFloatField
      FieldName = 'PROMEDIOEXISTENCIA'
      Origin = 'GET_EXIST_PROM_EV.PROMEDIOEXISTENCIA'
    end
    object IBQExistPromEVTOTALEXISTENCIA: TFloatField
      FieldName = 'TOTALEXISTENCIA'
      Origin = 'GET_EXIST_PROM_EV.TOTALEXISTENCIA'
    end
  end
  object IBQDatosRefPesoPrecioSalidas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaInicialCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from TAB_REF_PESO_PRECIO'
      'where id_tab_ref_cat = :idtabrefcat'
      'order by categoria asc'
      '')
    Left = 640
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'idtabrefcat'
        ParamType = ptUnknown
      end>
  end
  object IBQEntradasAuxEventos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select F1 AS PESO, F3  AS PRECIO'
      'from aux_eventos'
      'where D1 = :FECHA and V2 = :TIPOBAJA and'
      '          V3 = :CATEGORIA and I1 = :IDCATEGORIA'
      ' ')
    Left = 608
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'TIPOBAJA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CATEGORIA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'IDCATEGORIA'
        ParamType = ptUnknown
      end>
  end
  object IBQSalidasAuxEventos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select PRECIO_BRUTO AS PESO, PRECIO_UNITARIO AS PRECIO'
      'from aux_animales'
      'where FECHA_ALTA = :FECHA and V1 = :TIPOBAJA and'
      '          V2 = :CATEGORIA and ACTIVIDAD = :IDCATEGORIA'
      ' ')
    Left = 32
    Top = 225
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'TIPOBAJA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CATEGORIA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'IDCATEGORIA'
        ParamType = ptUnknown
      end>
  end
  object IBQTABREFCAT: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from tab_ref_cat'
      'where establecimiento = :establecimiento and ultimo = 1'
      'order by nombre asc')
    Left = 912
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosRefPesoPrecioEntradas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaInicialCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from TAB_REF_PESO_PRECIO'
      'where id_tab_ref_cat = :idtabrefcat'
      'order by categoria asc'
      '')
    Left = 672
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'idtabrefcat'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosRefPesoPrecioExistenciaFinal: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaInicialCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from TAB_REF_PESO_PRECIO'
      'where id_tab_ref_cat = :idtabrefcat'
      'order by categoria asc'
      '')
    Left = 704
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'idtabrefcat'
        ParamType = ptUnknown
      end>
  end
  object IBQDatosRefPesoPrecioExistenciaInicial: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    OnCalcFields = IBQGetExistenciaInicialCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from TAB_REF_PESO_PRECIO'
      'where id_tab_ref_cat = :idtabrefcat'
      'order by categoria asc'
      '')
    Left = 736
    Top = 17
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'idtabrefcat'
        ParamType = ptUnknown
      end>
  end
  object IBQSupGanaderaEfectiva: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select supganaderaefectiva from GET_SUP_GANADERA_EFECTIVA(:ESTA,' +
        ':DESDE,:HASTA)')
    Left = 808
    Top = 620
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ESTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DESDE'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'HASTA'
        ParamType = ptUnknown
      end>
  end
  object DSDisponibles: TDataSource
    DataSet = IBQDisponibles
    Left = 696
    Top = 153
  end
  object IBQDisponibles: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from cod_categorias where id_categoria not in (select i' +
        'd_animal_externo from aux_animales_externos)')
    Left = 729
    Top = 154
    object IBQDisponiblesNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'COD_CATEGORIAS.NOMBRE'
      Required = True
      Size = 50
    end
    object IBQDisponiblesID_CATEGORIA: TIntegerField
      FieldName = 'ID_CATEGORIA'
      Origin = 'COD_CATEGORIAS.ID_CATEGORIA'
      Required = True
    end
    object IBQDisponiblesSINONIMO: TIBStringField
      FieldName = 'SINONIMO'
      Origin = 'COD_CATEGORIAS.SINONIMO'
      Required = True
      Size = 50
    end
    object IBQDisponiblesTIPO_SEXO: TIntegerField
      FieldName = 'TIPO_SEXO'
      Origin = 'COD_CATEGORIAS.TIPO_SEXO'
      Required = True
    end
    object IBQDisponiblesCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = 'COD_CATEGORIAS.CODIGO'
      Required = True
      Size = 2
    end
  end
  object IBUSeleccionados: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select *'
      'from aux_animales_externos '
      'where'
      '  ID_ANIMAL_EXTERNO = :ID_ANIMAL_EXTERNO')
    ModifySQL.Strings = (
      'update aux_animales_externos'
      'set'
      '  ID_ANIMAL_EXTERNO = :ID_ANIMAL_EXTERNO,'
      '  ID_AUX = :ID_AUX,'
      '  NOMBRE = :NOMBRE'
      'where'
      '  ID_ANIMAL_EXTERNO = :OLD_ID_ANIMAL_EXTERNO')
    InsertSQL.Strings = (
      'insert into aux_animales_externos'
      '  (ID_ANIMAL_EXTERNO, ID_AUX, NOMBRE)'
      'values'
      '  (:ID_ANIMAL_EXTERNO, :ID_AUX, :NOMBRE)')
    DeleteSQL.Strings = (
      'delete from aux_animales_externos'
      'where'
      '  ID_ANIMAL_EXTERNO = :OLD_ID_ANIMAL_EXTERNO')
    Left = 872
    Top = 177
  end
  object DSSeleccionados: TDataSource
    DataSet = IBQSeleccionados
    Left = 904
    Top = 160
  end
  object IBQSeleccionados: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from aux_animales_externos')
    UpdateObject = IBUSeleccionados
    Left = 961
    Top = 160
    object IBQSeleccionadosID_ANIMAL_EXTERNO: TIntegerField
      FieldName = 'ID_ANIMAL_EXTERNO'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_ANIMAL_EXTERNO'
      Required = True
    end
    object IBQSeleccionadosID_AUX: TIntegerField
      FieldName = 'ID_AUX'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_AUX'
      Required = True
    end
    object IBQSeleccionadosID_OTRO: TIBStringField
      FieldName = 'ID_OTRO'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_OTRO'
      Size = 10
    end
    object IBQSeleccionadosID_PC: TIBStringField
      FieldName = 'ID_PC'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_PC'
      Size = 10
    end
    object IBQSeleccionadosID_HBA: TIBStringField
      FieldName = 'ID_HBA'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_HBA'
      Size = 10
    end
    object IBQSeleccionadosID_RA: TIBStringField
      FieldName = 'ID_RA'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_RA'
      Size = 10
    end
    object IBQSeleccionadosID_RP: TIBStringField
      FieldName = 'ID_RP'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_RP'
      Size = 10
    end
    object IBQSeleccionadosID_SENASA: TIBStringField
      FieldName = 'ID_SENASA'
      Origin = 'AUX_ANIMALES_EXTERNOS.ID_SENASA'
      Size = 9
    end
    object IBQSeleccionadosNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'AUX_ANIMALES_EXTERNOS.NOMBRE'
      Size = 100
    end
    object IBQSeleccionadosAPODO: TIBStringField
      FieldName = 'APODO'
      Origin = 'AUX_ANIMALES_EXTERNOS.APODO'
      Size = 100
    end
    object IBQSeleccionadosRAZA: TIntegerField
      FieldName = 'RAZA'
      Origin = 'AUX_ANIMALES_EXTERNOS.RAZA'
    end
    object IBQSeleccionadosSEXO: TIntegerField
      FieldName = 'SEXO'
      Origin = 'AUX_ANIMALES_EXTERNOS.SEXO'
    end
    object IBQSeleccionadosINTERNO: TIBStringField
      FieldName = 'INTERNO'
      Origin = 'AUX_ANIMALES_EXTERNOS.INTERNO'
      Size = 1
    end
    object IBQSeleccionadosFECHA: TDateField
      FieldName = 'FECHA'
      Origin = 'AUX_ANIMALES_EXTERNOS.FECHA'
    end
    object IBQSeleccionadosCRIADOR: TIBStringField
      FieldName = 'CRIADOR'
      Origin = 'AUX_ANIMALES_EXTERNOS.CRIADOR'
      Size = 50
    end
    object IBQSeleccionadosPROPIETARIO: TIntegerField
      FieldName = 'PROPIETARIO'
      Origin = 'AUX_ANIMALES_EXTERNOS.PROPIETARIO'
    end
    object IBQSeleccionadosADN: TIBStringField
      FieldName = 'ADN'
      Origin = 'AUX_ANIMALES_EXTERNOS.ADN'
      Size = 255
    end
    object IBQSeleccionadosTIPIFICACION_SANGUINEA: TIBStringField
      FieldName = 'TIPIFICACION_SANGUINEA'
      Origin = 'AUX_ANIMALES_EXTERNOS.TIPIFICACION_SANGUINEA'
      Size = 255
    end
  end
end
