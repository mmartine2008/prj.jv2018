inherited FREPPAProyeccionPartos: TFREPPAProyeccionPartos
  Left = 32
  Top = 39
  BorderStyle = bsSingle
  Caption = 'FREPPAProyeccionPartos'
  ClientHeight = 631
  ClientWidth = 964
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 964
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 576
  end
  object PAnimales: TPanel [4]
    Left = 0
    Top = 144
    Width = 415
    Height = 369
    Align = alLeft
    Color = clWhite
    TabOrder = 4
    object GBAnimales: TGroupBox
      Left = 1
      Top = 1
      Width = 413
      Height = 334
      Align = alClient
      Caption = ' Animales [0]'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BitDBGrid1: TBitDBGrid
        Left = 2
        Top = 18
        Width = 409
        Height = 314
        Align = alClient
        DataSource = DSAnimales
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -26
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = BitDBGrid1DblClick
        OnKeyPress = BitDBGrid1KeyPress
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Expanded = False
            FieldName = 'RP'
            Title.Caption = 'RP Animal'
            Width = 67
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CAPTION'
            Title.Caption = 'Fecha Estimada de Parto'
            Width = 106
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'INI_PERIODO'
            Title.Caption = 'Inicio de Periodo de Parto'
            Width = 97
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FIN_PERIODO'
            Title.Caption = 'Fin de Periodo de Parto'
            Width = 99
            Visible = True
          end>
      end
    end
    object Panel1: TPanel
      Left = 1
      Top = 335
      Width = 413
      Height = 33
      Align = alBottom
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 1
      object Label1: TLabel
        Left = 19
        Top = 5
        Width = 207
        Height = 20
        Caption = 'Total de animales sin definir:  '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -17
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object LtotalIndefinido: TLabel
        Left = 241
        Top = 5
        Width = 70
        Height = 22
        AutoSize = False
        Caption = '0'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -17
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
  end
  object PGraficos: TPanel [5]
    Left = 415
    Top = 144
    Width = 549
    Height = 369
    Align = alClient
    Color = clWhite
    TabOrder = 3
    object Ptactadas: TPanel
      Left = 1
      Top = 1
      Width = 547
      Height = 367
      Align = alClient
      TabOrder = 0
      object GBTactadas: TGroupBox
        Left = 1
        Top = 1
        Width = 545
        Height = 365
        Align = alClient
        Caption = ' Gr'#225'fico  '
        Color = clWhite
        ParentColor = False
        TabOrder = 0
        object DBCDistribucion: TDBChart
          Left = 2
          Top = 16
          Width = 541
          Height = 245
          AllowZoom = False
          AnimatedZoomSteps = 1
          BackWall.Brush.Color = clWhite
          BottomWall.Brush.Color = clWhite
          BottomWall.Brush.Style = bsClear
          Foot.AdjustFrame = False
          Foot.Visible = False
          Gradient.EndColor = clWhite
          LeftWall.Brush.Color = clWhite
          LeftWall.Brush.Style = bsClear
          MarginBottom = 0
          MarginRight = 15
          MarginTop = 3
          PrintProportional = False
          Title.AdjustFrame = False
          Title.Color = clWhite
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Frame.SmallDots = True
          Title.Text.Strings = (
            'Proyecci'#243'n de partos'
            '')
          BottomAxis.Automatic = False
          BottomAxis.AutomaticMaximum = False
          BottomAxis.AutomaticMinimum = False
          BottomAxis.DateTimeFormat = 'dd/mm/yy'
          BottomAxis.ExactDateTime = False
          BottomAxis.GridCentered = True
          BottomAxis.Increment = 1.000000000000000000
          BottomAxis.LabelsMultiLine = True
          BottomAxis.LabelsSeparation = 0
          BottomAxis.Maximum = 16.000000000000000000
          BottomAxis.MinorTickCount = 0
          BottomAxis.MinorTickLength = 0
          BottomAxis.TickOnLabelsOnly = False
          BottomAxis.Title.Caption = '  '
          BottomAxis.TitleSize = 35
          Chart3DPercent = 10
          DepthAxis.Automatic = False
          DepthAxis.AutomaticMaximum = False
          DepthAxis.AutomaticMinimum = False
          DepthAxis.Maximum = 0.500000000000000000
          DepthAxis.Minimum = -0.500000000000000000
          LeftAxis.Automatic = False
          LeftAxis.AutomaticMaximum = False
          LeftAxis.AutomaticMinimum = False
          LeftAxis.ExactDateTime = False
          LeftAxis.Increment = 1.000000000000000000
          LeftAxis.LabelsMultiLine = True
          LeftAxis.TickOnLabelsOnly = False
          LeftAxis.Title.Caption = 'Cantidad de partos esperados'
          LeftAxis.TitleSize = 5
          Legend.Alignment = laBottom
          Legend.DividingLines.Visible = True
          Legend.TextStyle = ltsXValue
          Legend.Visible = False
          RightAxis.Visible = False
          TopAxis.LabelsMultiLine = True
          TopAxis.RoundFirstLabel = False
          TopAxis.Visible = False
          View3D = False
          View3DOptions.Elevation = 315
          View3DOptions.Perspective = 0
          View3DOptions.Rotation = 360
          OnGetAxisLabel = DBCDistribucionGetAxisLabel
          OnGetLegendText = DBCDistribucionGetLegendText
          Align = alClient
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 0
          object Label2: TLabel
            Left = 46
            Top = 209
            Width = 432
            Height = 14
            Caption = 
              '________________________________________________________________' +
              '________'
            Transparent = True
          end
          object Label3: TLabel
            Left = 160
            Top = 221
            Width = 2
            Height = 14
            Caption = '|'
          end
          object Label4: TLabel
            Left = 47
            Top = 221
            Width = 2
            Height = 14
            Caption = '|'
          end
          object Label5: TLabel
            Left = 273
            Top = 221
            Width = 2
            Height = 14
            Caption = '|'
          end
          object Label6: TLabel
            Left = 380
            Top = 221
            Width = 2
            Height = 14
            Caption = '|'
          end
          object LM1: TLabel
            Left = 87
            Top = 221
            Width = 20
            Height = 14
            Caption = 'LM1'
            Transparent = True
          end
          object LM2: TLabel
            Left = 204
            Top = 221
            Width = 20
            Height = 14
            Caption = 'LM2'
            Transparent = True
          end
          object LM3: TLabel
            Left = 316
            Top = 221
            Width = 20
            Height = 14
            Caption = 'LM3'
            Transparent = True
          end
          object LM4: TLabel
            Left = 422
            Top = 221
            Width = 20
            Height = 14
            Caption = 'LM4'
            Transparent = True
          end
          object Label7: TLabel
            Left = 474
            Top = 201
            Width = 45
            Height = 14
            Caption = 'Semanas'
          end
          object Series1: TBarSeries
            Active = False
            Marks.Arrow.Visible = False
            Marks.ArrowLength = 2
            Marks.Font.Charset = DEFAULT_CHARSET
            Marks.Font.Color = clBlack
            Marks.Font.Height = -8
            Marks.Font.Name = 'Arial'
            Marks.Font.Style = []
            Marks.Style = smsXValue
            Marks.Visible = False
            SeriesColor = clRed
            ShowInLegend = False
            XLabelsSource = 'CANTIDAD'
            MultiBar = mbStacked
            YOrigin = 1.000000000000000000
            XValues.DateTime = False
            XValues.Name = 'X'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'SEMANA'
            YValues.DateTime = False
            YValues.Name = 'Bar'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
          end
          object Series2: TGanttSeries
            Active = False
            ColorEachPoint = True
            Marks.Arrow.Color = clRed
            Marks.Arrow.Width = 8
            Marks.Arrow.Visible = False
            Marks.ArrowLength = 0
            Marks.Style = smsValue
            Marks.Visible = False
            DataSource = IBQGrafico
            SeriesColor = clGreen
            ShowInLegend = False
            XLabelsSource = 'CANTIDAD'
            Pointer.InflateMargins = True
            Pointer.Style = psRectangle
            Pointer.Visible = False
            XValues.DateTime = True
            XValues.Name = 'Start'
            XValues.Multiplier = 1.000000000000000000
            XValues.Order = loAscending
            XValues.ValueSource = 'INI_PERIODO'
            YValues.DateTime = False
            YValues.Name = 'Y'
            YValues.Multiplier = 1.000000000000000000
            YValues.Order = loNone
            YValues.ValueSource = 'CANTIDAD'
            StartValues.DateTime = True
            StartValues.Name = 'Start'
            StartValues.Multiplier = 1.000000000000000000
            StartValues.Order = loAscending
            StartValues.ValueSource = 'INI_PERIODO'
            EndValues.DateTime = True
            EndValues.Name = 'End'
            EndValues.Multiplier = 1.000000000000000000
            EndValues.Order = loNone
            EndValues.ValueSource = 'FIN_PERIODO'
            NextTask.DateTime = True
            NextTask.Name = 'NextTask'
            NextTask.Multiplier = 1.000000000000000000
            NextTask.Order = loNone
            NextTask.ValueSource = 'INI_PERIODO'
          end
        end
        object GBDatos: TGroupBox
          Left = 2
          Top = 261
          Width = 541
          Height = 102
          Align = alBottom
          Caption = 'Datos de la Distribuci'#243'n'
          Color = clWhite
          ParentColor = False
          TabOrder = 1
          object LMes1: TLabel
            Left = 34
            Top = 20
            Width = 32
            Height = 14
            Caption = 'LMes1'
          end
          object LMes2: TLabel
            Left = 34
            Top = 41
            Width = 32
            Height = 14
            Caption = 'LMes2'
          end
          object LMes3: TLabel
            Left = 34
            Top = 61
            Width = 32
            Height = 14
            Caption = 'LMes3'
          end
          object LTMes1: TLabel
            Left = 194
            Top = 22
            Width = 37
            Height = 14
            Caption = 'LTMes1'
          end
          object LTMes2: TLabel
            Left = 194
            Top = 41
            Width = 37
            Height = 14
            Caption = 'LTMes2'
          end
          object LTMes3: TLabel
            Left = 194
            Top = 61
            Width = 37
            Height = 14
            Caption = 'LTMes3'
          end
          object LVentana: TLabel
            Left = 267
            Top = 43
            Width = 113
            Height = 14
            Caption = 'Ventana de Gestaci'#243'n: '
          end
          object LValVentana: TLabel
            Left = 405
            Top = 43
            Width = 60
            Height = 14
            Caption = 'LValVentana'
          end
          object LMes4: TLabel
            Left = 34
            Top = 81
            Width = 32
            Height = 14
            Caption = 'LMes4'
          end
          object LTMes4: TLabel
            Left = 194
            Top = 82
            Width = 37
            Height = 14
            Caption = 'LTMes4'
          end
          object Label8: TLabel
            Left = 121
            Top = 22
            Width = 62
            Height = 14
            Caption = 'Cant. Partos:'
          end
          object Label9: TLabel
            Left = 121
            Top = 40
            Width = 62
            Height = 14
            Caption = 'Cant. Partos:'
          end
          object Label10: TLabel
            Left = 121
            Top = 60
            Width = 62
            Height = 14
            Caption = 'Cant. Partos:'
          end
          object Label11: TLabel
            Left = 121
            Top = 81
            Width = 62
            Height = 14
            Caption = 'Cant. Partos:'
          end
        end
      end
    end
  end
  object PTop: TPanel [6]
    Left = 0
    Top = 65
    Width = 964
    Height = 79
    Align = alTop
    Color = clWhite
    TabOrder = 2
    object Splitter2: TSplitter
      Left = 139
      Top = 1
      Height = 77
    end
    object GBCategorias: TGroupBox
      Left = 329
      Top = 1
      Width = 216
      Height = 77
      Align = alLeft
      Caption = ' Selecci'#243'n de categor'#237'as  '
      Enabled = False
      TabOrder = 0
      object DBLCBACategorias: TDBLookupComboBoxAuto
        Left = 16
        Top = 26
        Width = 177
        Height = 22
        Color = clWhite
        DropDownRows = 20
        KeyField = 'ID_CATEGORIA'
        ListField = 'NOMBRE'
        ListSource = DSCategorias
        TabOrder = 0
        OnKeyPress = DBLCBACategoriasKeyPress
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object GBRodeos: TGroupBox
      Left = 142
      Top = 1
      Width = 187
      Height = 77
      Align = alLeft
      Caption = ' Selecci'#243'n de Rodeos '
      Enabled = False
      TabOrder = 1
      object DBLCBARodeos: TDBLookupComboBoxAuto
        Left = 16
        Top = 26
        Width = 153
        Height = 22
        Color = clWhite
        DropDownRows = 20
        KeyField = 'ID_RODEO'
        ListField = 'NOMBRE'
        ListSource = DMSoftvet.DSRodeo
        TabOrder = 0
        OnKeyPress = DBLCBACategoriasKeyPress
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object RGTipo: TRadioGroup
      Left = 1
      Top = 1
      Width = 138
      Height = 77
      Align = alLeft
      Caption = ' Tipo de selecci'#243'n '
      ItemIndex = 2
      Items.Strings = (
        'Rodeos'
        'Categor'#237'as'
        'Todos')
      TabOrder = 2
      OnClick = RGTipoClick
    end
    object GroupBox2: TGroupBox
      Left = 545
      Top = 1
      Width = 216
      Height = 77
      Align = alLeft
      Caption = ' Imprimir Listado '
      TabOrder = 3
      object CBListado: TCheckBox
        Left = 18
        Top = 36
        Width = 164
        Height = 18
        Caption = 'Imprimir Listado de Animales'
        Checked = True
        State = cbChecked
        TabOrder = 0
      end
    end
    object GroupBox1: TGroupBox
      Left = 761
      Top = 1
      Width = 202
      Height = 77
      Align = alClient
      Caption = 'Consulta'
      TabOrder = 4
      object JvIBotonBuscar: TJvImage
        Tag = 4
        Left = 51
        Top = 27
        Width = 97
        Height = 32
        Cursor = crHandPoint
        Picture.Data = {
          
          E9FF0056DF9FC352571EAFFFD9}
        OnClick = JvIBotonBuscarClick
      end
      object Button1: TButton
        Left = 56
        Top = 56
        Width = 73
        Height = 18
        Caption = 'Consultar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        Visible = False
        OnClick = Button1Click
      end
    end
  end
  object PBotones: TPanel [7]
    Left = 0
    Top = 513
    Width = 964
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 5
    DesignSize = (
      964
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 962
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 864
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = ASalirExecute
    end
    object LSalir: TLabel
      Left = 875
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = ASalirExecute
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      OnClick = BBXlsClick
    end
    object IPreview: TImage
      Left = 309
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = AImprimirReporteExecute
    end
    object LPreview: TLabel
      Left = 306
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AImprimirReporteExecute
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object LExpExcel: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBXlsClick
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 855
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      OnClick = BBXlsClick
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 964
    Caption = 'Proyecci'#243'n de Partos'
    inherited LTitulo: TLabel
      Width = 560
    end
    inherited PILeft: TPanel
      Left = 756
    end
  end
  inherited Psocalo: TPanel
    Top = 609
    Width = 964
    inherited JvSocalo: TJvImage
      Width = 964
    end
  end
  inherited ALUniversal: TActionList
    object ASalir: TAction
      Caption = 'Salir'
      ShortCut = 27
      OnExecute = ASalirExecute
    end
    object AImprimirReporte: TAction
      Caption = 'Imprimir'
      OnExecute = AImprimirReporteExecute
    end
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 340
    Top = 79
  end
  inherited SDXLS: TSaveDialog
    Left = 534
    Top = 237
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQAnimales
    Columns = <
      item
        DataField = 'RP'
        Title = 'RP Animal'
      end
      item
        DataField = 'CAPTION'
        Title = 'Fecha Estimada de Parto'
      end
      item
        DataField = 'INI_PERIODO'
        Title = 'Inicio de Periodo de Parto'
      end
      item
        DataField = 'FIN_PERIODO'
        Title = 'Fin de Periodo de Parto'
      end>
    Title.Text = 'Proyeccion de Partos'
    Left = 504
    Top = 233
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 516
    Top = 183
  end
  object IBQAnimales: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQAnimalesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from rep_proyeccion_partos(1,'#39'N'#39',2) order by anio, mes')
    Left = 560
    Top = 129
    object IBQAnimalesRP: TIBStringField
      FieldName = 'RP'
      Origin = 'REP_PROYECCION_PARTOS.RP'
      Size = 10
    end
    object IBQAnimalesRODEO: TIntegerField
      FieldName = 'RODEO'
      Origin = 'REP_PROYECCION_PARTOS.RODEO'
    end
    object IBQAnimalesCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
      Origin = 'REP_PROYECCION_PARTOS.CATEGORIA'
    end
    object IBQAnimalesINDEFINIDO: TIBStringField
      FieldName = 'INDEFINIDO'
      Origin = 'REP_PROYECCION_PARTOS.INDEFINIDO'
      Size = 1
    end
    object IBQAnimalesMES: TIntegerField
      FieldName = 'MES'
      Origin = 'REP_PROYECCION_PARTOS.MES'
    end
    object IBQAnimalesANIO: TIntegerField
      FieldName = 'ANIO'
      Origin = 'REP_PROYECCION_PARTOS.ANIO'
    end
    object IBQAnimalesCAPTION: TIBStringField
      FieldName = 'CAPTION'
      Origin = 'REP_PROYECCION_PARTOS.CAPTION'
      Size = 50
    end
    object IBQAnimalesID_ANIMAL: TIntegerField
      FieldName = 'ID_ANIMAL'
      Origin = 'REP_PROYECCION_PARTOS.ID_ANIMAL'
    end
    object IBQAnimalesINI_PERIODO: TDateField
      FieldName = 'INI_PERIODO'
      Origin = 'REP_PROYECCION_PARTOS.INI_PERIODO'
    end
    object IBQAnimalesFIN_PERIODO: TDateField
      FieldName = 'FIN_PERIODO'
      Origin = 'REP_PROYECCION_PARTOS.FIN_PERIODO'
    end
  end
  object DSCategorias: TDataSource
    DataSet = IBQCategorias
    Left = 520
    Top = 32
  end
  object IBQCategorias: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from cod_categorias where (tipo_sexo = 2) and (id_categ' +
        'oria <> 4)')
    Left = 552
    Top = 33
  end
  object DSAnimales: TDataSource
    DataSet = IBQAnimales
    Left = 528
    Top = 128
  end
  object IBQGrafico: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select (count(*)) as cantidad, anio, mes, fecha_parto, caption, ' +
        'ini_periodo, fin_periodo from rep_proyeccion_partos(1,'#39'N'#39',2) whe' +
        're (indefinido <> '#39'S'#39') group by anio, mes, fecha_parto, caption,' +
        ' ini_periodo, fin_periodo')
    Left = 560
    Top = 177
    object IBQGraficoCANTIDAD: TIntegerField
      FieldName = 'CANTIDAD'
      Required = True
    end
    object IBQGraficoANIO: TIntegerField
      FieldName = 'ANIO'
    end
    object IBQGraficoMES: TIntegerField
      FieldName = 'MES'
    end
    object IBQGraficoCAPTION: TIBStringField
      FieldName = 'CAPTION'
      Size = 50
    end
    object IBQGraficoINI_PERIODO: TDateField
      FieldName = 'INI_PERIODO'
    end
    object IBQGraficoFIN_PERIODO: TDateField
      FieldName = 'FIN_PERIODO'
    end
    object IBQGraficoFECHA_PARTO: TDateField
      FieldName = 'FECHA_PARTO'
    end
  end
  object DSGrafico: TDataSource
    DataSet = IBQGrafico
    Left = 528
    Top = 176
  end
  object IBQContarIndefinidos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from rep_proyeccion_partos(1) order by anio, mes')
    Left = 616
    Top = 121
  end
  object IBQGetVentana: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select valor from rel_parametros_establecimiento where parametro' +
        '=30 and establecimiento=:esta')
    Left = 832
    Top = 186
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
end
