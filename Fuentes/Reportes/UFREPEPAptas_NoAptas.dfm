inherited FREPPreservicio: TFREPPreservicio
  Left = 86
  Top = 33
  Caption = 'FREPPreservicio'
  ClientHeight = 733
  ClientWidth = 911
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 911
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 527
  end
  object PAnimales: TPanel [4]
    Left = 0
    Top = 65
    Width = 911
    Height = 550
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object GBAnimales: TGroupBox
      Left = 1
      Top = 333
      Width = 909
      Height = 216
      Align = alBottom
      Caption = ' Animales [0]'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BDBGAnimales: TBitDBGrid
        Left = 2
        Top = 18
        Width = 905
        Height = 196
        Align = alClient
        DataSource = DSAnimales
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = BDBGAnimalesDblClick
        OnKeyPress = BDBGAnimalesKeyPress
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Expanded = False
            FieldName = 'RP_ANIMAL'
            Title.Alignment = taCenter
            Title.Caption = 'RP Animal'
            Width = 143
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'RAZA'
            Title.Alignment = taCenter
            Title.Caption = 'Raza'
            Width = 94
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'AREA_PELVICA'
            Title.Alignment = taCenter
            Title.Caption = #193'rea P'#233'lvica'
            Width = 85
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FRAME'
            Title.Alignment = taCenter
            Title.Caption = 'Frame'
            Width = 95
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'GDR'
            Title.Alignment = taCenter
            Title.Caption = 'Gdr'
            Width = 138
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PESO'
            Title.Alignment = taCenter
            Title.Caption = 'Peso'
            Width = 91
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'ROBO'
            Title.Alignment = taCenter
            Title.Caption = 'Pre'#241'ez por Robo'
            Width = 124
            Visible = True
          end>
      end
    end
    object PGraficos: TPanel
      Left = 1
      Top = 80
      Width = 909
      Height = 257
      Align = alTop
      Color = clWhite
      TabOrder = 1
      object DBCAreaPFrame: TDBChart
        Left = 138
        Top = 20
        Width = 344
        Height = 280
        AllowZoom = False
        BackWall.Brush.Color = clWhite
        BackWall.Brush.Style = bsClear
        MarginBottom = 15
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          'Area P'#233'lvica y Frame promedio'
          'discriminado por raza')
        BottomAxis.ExactDateTime = False
        BottomAxis.Grid.Visible = False
        BottomAxis.LabelsSeparation = 0
        LeftAxis.AxisValuesFormat = '#,##0.#'
        LeftAxis.ExactDateTime = False
        LeftAxis.Grid.Visible = False
        LeftAxis.GridCentered = True
        LeftAxis.Increment = 10.000000000000000000
        LeftAxis.Title.Caption = 'Area Pelvica'
        Legend.Alignment = laTop
        Legend.LegendStyle = lsSeries
        RightAxis.Automatic = False
        RightAxis.AutomaticMaximum = False
        RightAxis.AutomaticMinimum = False
        RightAxis.AxisValuesFormat = '#,##0.#'
        RightAxis.ExactDateTime = False
        RightAxis.Grid.Visible = False
        RightAxis.GridCentered = True
        RightAxis.Increment = 1.000000000000000000
        RightAxis.Maximum = 9.000000000000000000
        RightAxis.Title.Caption = 'Frame'
        View3D = False
        OnGetAxisLabel = DBCAreaPFrameGetAxisLabel
        OnGetLegendText = DBCAreaPFrameGetLegendText
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 5
      end
      object DBCFrameTotal: TDBChart
        Left = 207
        Top = 35
        Width = 344
        Height = 280
        AllowZoom = False
        BackWall.Brush.Color = clWhite
        BackWall.Brush.Style = bsClear
        MarginBottom = 15
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          'Histograma de Frame'
          'Total de Vaquillonas')
        BottomAxis.ExactDateTime = False
        BottomAxis.Grid.Visible = False
        BottomAxis.LabelsSeparation = 0
        LeftAxis.AxisValuesFormat = '#,##0.#'
        LeftAxis.ExactDateTime = False
        LeftAxis.Grid.Visible = False
        LeftAxis.GridCentered = True
        LeftAxis.Title.Caption = 'Frecuencia'
        Legend.Alignment = laTop
        Legend.LegendStyle = lsSeries
        Legend.Visible = False
        RightAxis.Automatic = False
        RightAxis.AutomaticMaximum = False
        RightAxis.AutomaticMinimum = False
        RightAxis.AxisValuesFormat = '#,##0.#'
        RightAxis.ExactDateTime = False
        RightAxis.Grid.Visible = False
        RightAxis.Increment = 5.000000000000000000
        RightAxis.Maximum = 60.000000000000000000
        View3D = False
        OnGetAxisLabel = DBCFrameTotalGetAxisLabel
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 3
      end
      object DBCFrameAptas: TDBChart
        Left = 292
        Top = 35
        Width = 345
        Height = 280
        AllowZoom = False
        BackWall.Brush.Color = clWhite
        BackWall.Brush.Style = bsClear
        MarginBottom = 15
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          'Histograma de Frame'
          'Vaquillonas Aptas')
        BottomAxis.ExactDateTime = False
        BottomAxis.Grid.Visible = False
        BottomAxis.LabelsSeparation = 0
        LeftAxis.AxisValuesFormat = '#,##0.#'
        LeftAxis.ExactDateTime = False
        LeftAxis.Grid.Visible = False
        LeftAxis.GridCentered = True
        LeftAxis.Title.Caption = 'Frecuencia'
        Legend.Alignment = laTop
        Legend.LegendStyle = lsSeries
        Legend.Visible = False
        RightAxis.Automatic = False
        RightAxis.AutomaticMaximum = False
        RightAxis.AutomaticMinimum = False
        RightAxis.AxisValuesFormat = '#,##0.#'
        RightAxis.ExactDateTime = False
        RightAxis.Grid.Visible = False
        RightAxis.Increment = 5.000000000000000000
        RightAxis.Maximum = 60.000000000000000000
        View3D = False
        OnGetAxisLabel = DBCFrameAptasGetAxisLabel
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 4
      end
      object DBCAreaPAptas: TDBChart
        Left = 11
        Top = 22
        Width = 345
        Height = 280
        AllowZoom = False
        BackWall.Brush.Color = clWhite
        BackWall.Brush.Style = bsClear
        MarginBottom = 15
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          'Histograma del Area P'#233'lvica'
          'Vaquillonas Aptas')
        BottomAxis.ExactDateTime = False
        BottomAxis.Grid.Visible = False
        BottomAxis.LabelsAngle = 330
        BottomAxis.LabelsFont.Charset = DEFAULT_CHARSET
        BottomAxis.LabelsFont.Color = clBlack
        BottomAxis.LabelsFont.Height = -7
        BottomAxis.LabelsFont.Name = 'Arial'
        BottomAxis.LabelsFont.Style = []
        BottomAxis.LabelsSeparation = 0
        LeftAxis.AxisValuesFormat = '#,##0.#'
        LeftAxis.ExactDateTime = False
        LeftAxis.Grid.Visible = False
        LeftAxis.GridCentered = True
        LeftAxis.Title.Caption = 'Frecuencia'
        Legend.Alignment = laTop
        Legend.LegendStyle = lsSeries
        Legend.Visible = False
        RightAxis.Automatic = False
        RightAxis.AutomaticMaximum = False
        RightAxis.AutomaticMinimum = False
        RightAxis.AxisValuesFormat = '#,##0.#'
        RightAxis.ExactDateTime = False
        RightAxis.Grid.Visible = False
        RightAxis.Increment = 5.000000000000000000
        RightAxis.Maximum = 60.000000000000000000
        View3D = False
        OnGetAxisLabel = DBCAreaPAptasGetAxisLabel
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 2
      end
      object DBCAreaPTotal: TDBChart
        Left = 549
        Top = 12
        Width = 345
        Height = 280
        AllowZoom = False
        BackWall.Brush.Color = clWhite
        BackWall.Brush.Style = bsClear
        MarginBottom = 15
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -12
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          'Histograma del Area P'#233'lvica'
          'Total de Vaquillonas')
        BottomAxis.ExactDateTime = False
        BottomAxis.Grid.Visible = False
        BottomAxis.LabelsAngle = 330
        BottomAxis.LabelsFont.Charset = DEFAULT_CHARSET
        BottomAxis.LabelsFont.Color = clBlack
        BottomAxis.LabelsFont.Height = -7
        BottomAxis.LabelsFont.Name = 'Arial'
        BottomAxis.LabelsFont.Style = []
        BottomAxis.LabelsSeparation = 0
        LeftAxis.AxisValuesFormat = '#,##0.#'
        LeftAxis.ExactDateTime = False
        LeftAxis.Grid.Visible = False
        LeftAxis.GridCentered = True
        LeftAxis.Title.Caption = 'Frecuencia'
        Legend.Alignment = laTop
        Legend.LegendStyle = lsSeries
        Legend.Visible = False
        RightAxis.Automatic = False
        RightAxis.AutomaticMaximum = False
        RightAxis.AutomaticMinimum = False
        RightAxis.AxisValuesFormat = '#,##0.#'
        RightAxis.ExactDateTime = False
        RightAxis.Grid.Visible = False
        RightAxis.Increment = 5.000000000000000000
        RightAxis.Maximum = 60.000000000000000000
        View3D = False
        OnGetAxisLabel = DBCAreaPTotalGetAxisLabel
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 1
      end
      object PAptasNOAptas: TPanel
        Left = 1
        Top = 1
        Width = 907
        Height = 255
        Align = alClient
        Color = clWhite
        TabOrder = 0
        object GBVariables: TGroupBox
          Left = 569
          Top = 86
          Width = 312
          Height = 220
          Align = alCustom
          Caption = 'Variables'
          TabOrder = 1
          object Shape1: TShape
            Left = 396
            Top = 190
            Width = 70
            Height = 70
          end
          object DBGrid1: TDBGrid
            Left = 141
            Top = 258
            Width = 345
            Height = 130
            DataSource = DataSource1
            TabOrder = 2
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'Arial'
            TitleFont.Style = []
            Visible = False
          end
          object DGVariables: TDecisionGrid
            Left = 2
            Top = 70
            Width = 308
            Height = 148
            DefaultColWidth = 100
            DefaultRowHeight = 20
            CaptionColor = clMoneyGreen
            CaptionFont.Charset = DEFAULT_CHARSET
            CaptionFont.Color = clBtnText
            CaptionFont.Height = -11
            CaptionFont.Name = 'MS Sans Serif'
            CaptionFont.Style = [fsBold]
            DataColor = clMenu
            DataSumColor = clNone
            DataFont.Charset = DEFAULT_CHARSET
            DataFont.Color = clWindowText
            DataFont.Height = -11
            DataFont.Name = 'MS Sans Serif'
            DataFont.Style = []
            LabelFont.Charset = DEFAULT_CHARSET
            LabelFont.Color = clWindowText
            LabelFont.Height = -11
            LabelFont.Name = 'MS Sans Serif'
            LabelFont.Style = []
            LabelColor = clBtnFace
            LabelSumColor = clInactiveCaption
            DecisionSource = DSVariables
            Dimensions = <
              item
                FieldName = 'Frame'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'Area Pelvica'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'Peso'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'GDR'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'Pre'#241'es por Robo'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'Rechazo'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end
              item
                FieldName = 'Cantidad'
                Color = clNone
                Alignment = taCenter
                Subtotals = False
              end>
            Totals = False
            ShowCubeEditor = False
            Align = alClient
            Color = clWhite
            GridLineWidth = 1
            GridLineColor = clWindowText
            TabOrder = 0
          end
          object DPVariables: TDecisionPivot
            Left = 2
            Top = 16
            Width = 308
            Height = 54
            ButtonAutoSize = True
            DecisionSource = DSVariables
            GroupLayout = xtHorizontal
            Groups = [xtRows, xtColumns]
            ButtonSpacing = 0
            ButtonWidth = 64
            ButtonHeight = 24
            GroupSpacing = 10
            BorderWidth = 0
            BorderStyle = bsNone
            Align = alTop
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Color = clBtnHighlight
            ParentFont = False
            TabOrder = 1
          end
        end
        object GBTactadas: TGroupBox
          Left = 1
          Top = 1
          Width = 905
          Height = 253
          Align = alClient
          Caption = ' Gr'#225'fico  '
          TabOrder = 0
          object DBCAptasNoAptas: TDBChart
            Left = 240
            Top = 24
            Width = 449
            Height = 217
            BackWall.Brush.Color = clWhite
            BottomWall.Brush.Color = clWhite
            BottomWall.Brush.Style = bsClear
            Gradient.EndColor = clWhite
            LeftWall.Brush.Color = clWhite
            LeftWall.Brush.Style = bsClear
            PrintProportional = False
            Title.AdjustFrame = False
            Title.Color = clWhite
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Arial'
            Title.Font.Style = [fsBold]
            Title.Text.Strings = (
              'Aptas - No Aptas'
              '')
            Legend.Alignment = laBottom
            Legend.ColorWidth = 100
            View3DOptions.Elevation = 315
            View3DOptions.Orthogonal = False
            View3DOptions.Perspective = 0
            View3DOptions.Rotation = 360
            OnGetLegendText = DBCAptasNoAptasGetLegendText
            Align = alCustom
            BevelOuter = bvNone
            Color = clWhite
            TabOrder = 0
          end
        end
      end
    end
    object PTop: TPanel
      Left = 1
      Top = 1
      Width = 909
      Height = 79
      Align = alTop
      Color = clWhite
      TabOrder = 2
      object GBSeleccionFechas: TGroupBox
        Left = 105
        Top = 1
        Width = 160
        Height = 77
        Align = alLeft
        Caption = ' Selecci'#243'n de fechas '
        TabOrder = 0
        object LDesde: TLabel
          Left = 18
          Top = 25
          Width = 37
          Height = 14
          Caption = 'Desde: '
        end
        object LHasta: TLabel
          Left = 22
          Top = 48
          Width = 34
          Height = 14
          Caption = 'Hasta: '
        end
        object DTPADesde: TDateTimePickerAuto
          Left = 69
          Top = 17
          Width = 84
          Height = 21
          Date = 38744.433197870370000000
          Time = 38744.433197870370000000
          Color = clWhite
          TabOrder = 0
          OnCloseUp = DTPACloseUpFechas
          OnKeyUp = DTPADesdeKeyUp
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
        object DTPAHasta: TDateTimePickerAuto
          Left = 69
          Top = 43
          Width = 84
          Height = 21
          Date = 38744.433227719900000000
          Time = 38744.433227719900000000
          Color = clWhite
          TabOrder = 1
          OnCloseUp = DTPACloseUpFechas
          OnKeyUp = DTPADesdeKeyUp
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
      end
      object GBGrupos: TGroupBox
        Left = 265
        Top = 1
        Width = 240
        Height = 77
        Align = alLeft
        Caption = ' Selecci'#243'n de grupo de Preservicio '
        Enabled = False
        TabOrder = 1
        object DBLCBASeleccionGrupo: TDBLookupComboBoxAuto
          Left = 16
          Top = 26
          Width = 209
          Height = 22
          Color = clWhite
          DropDownRows = 20
          Enabled = False
          KeyField = 'ID_GRUPO'
          ListField = 'NOMBRE'
          ListSource = DSGrupos
          TabOrder = 0
          OnClick = DBLCBASeleccionGrupoClick
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
      end
      object RGTipo: TRadioGroup
        Left = 1
        Top = 1
        Width = 104
        Height = 77
        Align = alLeft
        Caption = ' Tipo de selecci'#243'n '
        ItemIndex = 0
        Items.Strings = (
          'Fecha'
          'Grupo')
        TabOrder = 2
        OnClick = RGTipoClick
      end
      object GroupBox2: TGroupBox
        Left = 505
        Top = 1
        Width = 216
        Height = 77
        Align = alLeft
        Caption = ' Imprimir Listado '
        TabOrder = 3
        object CBListado: TCheckBox
          Left = 18
          Top = 22
          Width = 164
          Height = 18
          Caption = 'Imprimir Listado de Animales'
          Checked = True
          State = cbChecked
          TabOrder = 0
        end
        object CBDetalle: TCheckBox
          Left = 17
          Top = 52
          Width = 182
          Height = 18
          Caption = 'Imprimir Detalle de Preservicio'
          Checked = True
          State = cbChecked
          TabOrder = 1
        end
      end
      object GConsulta: TGroupBox
        Left = 721
        Top = 1
        Width = 187
        Height = 77
        Align = alClient
        Caption = 'Consulta'
        TabOrder = 4
        object JvIBotonBuscar: TJvImage
          Tag = 4
          Left = 43
          Top = 29
          Width = 97
          Height = 32
          Cursor = crHandPoint
          Picture.Data = {
            
            E9FF0056DF9FC352571EAFFFD9}
          OnClick = JvIBotonBuscarClick
        end
        object BConsulta: TButton
          Left = 56
          Top = 56
          Width = 57
          Height = 17
          Caption = 'Consultar'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 0
          Visible = False
          OnClick = BConsultaClick
        end
      end
    end
  end
  object PBotones: TPanel [5]
    Left = 0
    Top = 615
    Width = 911
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 3
    DesignSize = (
      911
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 909
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 808
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = BBSalirClick
    end
    object LSalir: TLabel
      Left = 819
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBSalirClick
    end
    object IPreview: TImage
      Left = 309
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = BBVerReporteClick
    end
    object LPreview: TLabel
      Left = 306
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBVerReporteClick
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      OnClick = BBXlsClick
    end
    object Label1: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBXlsClick
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 802
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      OnClick = BBXlsClick
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 911
    Caption = 'Hembras Aptas/No Aptas para Servicio'
    inherited LTitulo: TLabel
      Width = 507
    end
    inherited PILeft: TPanel
      Left = 703
    end
  end
  inherited Psocalo: TPanel
    Top = 711
    Width = 911
    inherited JvSocalo: TJvImage
      Width = 911
    end
  end
  inherited ALUniversal: TActionList
    Left = 203
    Top = 30
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 124
    Top = 31
  end
  inherited SDXLS: TSaveDialog
    Left = 38
    Top = 13
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQAnimales
    Columns = <
      item
        DataField = 'RP_ANIMAL'
        Title = 'RP Animal'
      end
      item
        DataField = 'AREA_PELVICA'
        Title = 'Area Pelvica'
      end
      item
        DataField = 'FRAME'
        Title = 'Frame'
      end
      item
        DataField = 'GDR'
        Title = 'Gdr'
      end
      item
        DataField = 'PESO'
        Title = 'Peso'
      end>
    Title.Text = 'Hembras Aptas/No Aptas para Servicio'
    Left = 136
    Top = 9
  end
  inherited WinXP1: TWinXP
    Left = 64
    Top = 16
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 316
    Top = 15
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 268
    Top = 15
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 4
    Top = 23
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 612
    Top = 7
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 564
    Top = 7
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 652
    Top = 7
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 528
    Top = 8
  end
  object DSGrupos: TDataSource
    DataSet = IBQGrupos
    Left = 232
    Top = 5
  end
  object IBQGrupos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from tab_grupos where tipo_evento = 17 and establecimie' +
        'nto = :esta')
    Left = 368
    Top = 21
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object IBQSPAptasNoAptasG: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT COUNT(*), EVALUACION_CALCULADA FROM REP_APTAS_NOAPTAS(:ES' +
        'TA,:FECHA_INI,:FECHA_FIN,:GRUPO) GROUP BY EVALUACION_CALCULADA')
    Left = 96
    Top = 13
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA_INI'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA_FIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'GRUPO'
        ParamType = ptUnknown
      end>
  end
  object DSAnimales: TDataSource
    DataSet = IBQAnimales
    Left = 24
    Top = 309
  end
  object IBQAnimales: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQAnimalesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT * FROM REP_APTAS_NOAPTAS(:ESTA,:FECHA_INI,:FECHA_FIN,:GRU' +
        'PO)'
      '')
    Left = 56
    Top = 309
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA_INI'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA_FIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'GRUPO'
        ParamType = ptUnknown
      end>
    object IBQAnimalesID_ANIMAL: TIntegerField
      FieldName = 'ID_ANIMAL'
      Origin = 'REP_APTAS_NOAPTAS.ID_ANIMAL'
    end
    object IBQAnimalesRP_ANIMAL: TIBStringField
      FieldName = 'RP_ANIMAL'
      Origin = 'REP_APTAS_NOAPTAS.RP_ANIMAL'
      Size = 10
    end
    object IBQAnimalesAREA_PELVICA: TFloatField
      FieldName = 'AREA_PELVICA'
      Origin = 'REP_APTAS_NOAPTAS.AREA_PELVICA'
    end
    object IBQAnimalesFRAME: TIBBCDField
      FieldName = 'FRAME'
      Origin = 'REP_APTAS_NOAPTAS.FRAME'
      Precision = 18
      Size = 2
    end
    object IBQAnimalesID_GDR: TIntegerField
      FieldName = 'ID_GDR'
      Origin = 'REP_APTAS_NOAPTAS.ID_GDR'
    end
    object IBQAnimalesGDR: TIBStringField
      FieldName = 'GDR'
      Origin = 'REP_APTAS_NOAPTAS.GDR'
      Size = 50
    end
    object IBQAnimalesPESO: TFloatField
      FieldName = 'PESO'
      Origin = 'REP_APTAS_NOAPTAS.PESO'
    end
    object IBQAnimalesPRENIEZ_ROBO: TIBStringField
      FieldName = 'PRENIEZ_ROBO'
      Origin = 'REP_APTAS_NOAPTAS.PRENIEZ_ROBO'
      Size = 1
    end
    object IBQAnimalesRECHAZO: TIntegerField
      FieldName = 'RECHAZO'
      Origin = 'REP_APTAS_NOAPTAS.RECHAZO'
    end
    object IBQAnimalesAPROB_MANUAL: TIBStringField
      FieldName = 'APROB_MANUAL'
      Origin = 'REP_APTAS_NOAPTAS.APROB_MANUAL'
      Size = 1
    end
    object IBQAnimalesEVALUACION_CALCULADA: TIBStringField
      FieldName = 'EVALUACION_CALCULADA'
      Origin = 'REP_APTAS_NOAPTAS.EVALUACION_CALCULADA'
      Size = 1
    end
    object IBQAnimalesALTO: TFloatField
      FieldName = 'ALTO'
      Origin = 'REP_APTAS_NOAPTAS.ALTO'
    end
    object IBQAnimalesANCHO: TFloatField
      FieldName = 'ANCHO'
      Origin = 'REP_APTAS_NOAPTAS.ANCHO'
    end
    object IBQAnimalesROBO: TIBStringField
      FieldName = 'ROBO'
      Origin = 'REP_APTAS_NOAPTAS.ROBO'
      Size = 2
    end
    object IBQAnimalesRAZA: TIBStringField
      FieldName = 'RAZA'
      Origin = 'REP_APTAS_NOAPTAS.RAZA'
      Size = 2
    end
  end
  object DCVariables: TDecisionCube
    DimensionMap = <
      item
        ActiveFlag = diAsNeeded
        FieldType = ftString
        Fieldname = 'ID_SENASA'
        BaseName = 'AUX_ANIMALES.ID_SENASA'
        Name = 'Frame'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 12
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftInteger
        Fieldname = 'SEXO'
        BaseName = 'AUX_ANIMALES.SEXO'
        Name = 'Area Pelvica'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 8
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftInteger
        Fieldname = 'PESO'
        BaseName = 'AUX_ANIMALES.PESO'
        Name = 'Peso'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 15
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftString
        Fieldname = 'V1'
        BaseName = 'AUX_ANIMALES.V1'
        Name = 'GDR'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 5
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftString
        Fieldname = 'V2'
        BaseName = 'AUX_ANIMALES.V2'
        Name = 'Pre'#241'es por Robo'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 2
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftString
        Fieldname = 'NOMBRE'
        BaseName = 'COD_RECHAZOS_PRESERVICIO'
        Name = 'Rechazo'
        DerivedFrom = -1
        DimensionType = dimDimension
        BinType = binNone
        ValueCount = 1
        Active = True
      end
      item
        ActiveFlag = diAsNeeded
        FieldType = ftInteger
        Fieldname = 'COUNT'
        BaseName = 'AUX_ANIMALES.ID_ANIMAL'
        Name = 'Cantidad'
        DerivedFrom = -1
        DimensionType = dimCount
        BinType = binNone
        ValueCount = -1
        Active = True
      end>
    ShowProgressDialog = False
    MaxDimensions = 6
    MaxSummaries = 10
    MaxCells = 0
    Left = 507
    Top = 396
  end
  object DSVariables: TDecisionSource
    DecisionCube = DCVariables
    ControlType = xtCheck
    SparseRows = False
    SparseCols = False
    Left = 419
    Top = 412
    DimensionCount = 6
    SummaryCount = 1
    CurrentSummary = 0
    SparseRows = False
    SparseCols = False
    DimensionInfo = (
      1
      0
      1
      0
      0
      1
      1
      1
      1
      0
      2
      0
      1
      0
      0
      2
      1
      1
      1
      0
      1
      2
      1
      2
      0
      2
      2
      1
      2
      0)
  end
  object IBQVariables: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQVariablesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'SELECT AA.ID_SENASA, AA.SEXO, AA.PESO, AA.V1, AA.V2, CRP.NOMBRE,' +
        ' COUNT(AA.ID_ANIMAL) FROM AUX_ANIMALES AA JOIN COD_RECHAZOS_PRES' +
        'ERVICIO CRP ON CRP.ID_RECHAZO = AA.POTRERO GROUP BY AA.ID_SENASA' +
        ', AA.SEXO, AA.PESO, AA.V1, AA.V2, CRP.NOMBRE')
    Left = 427
    Top = 444
  end
  object IBQInsertAux: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'INSERT INTO AUX_ANIMALES (ID_AUX, ID_ANIMAL, ID_SENASA, SEXO, PE' +
        'SO, V1, V2, POTRERO) VALUES (1, :ANIMAL, :FRAME, :AREA_P, :PESO,' +
        ' :GDR, :PRENIEZ, :RECHAZO)')
    Left = 507
    Top = 444
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ANIMAL'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FRAME'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'AREA_P'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'PESO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'GDR'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'PRENIEZ'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'RECHAZO'
        ParamType = ptUnknown
      end>
  end
  object DataSource1: TDataSource
    DataSet = IBQVariables
    Left = 469
    Top = 443
  end
  object IBQCriteriosPreservicio: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select SJP.ID_PARAMETRO, SJP.NOMBRE, RPE.VALOR from SYS_JUEGO_PA' +
        'RAMETROS SJP join REL_PARAMETROS_ESTABLECIMIENTO RPE ON (RPE.par' +
        'ametro = SJP.id_parametro)'
      'where (RPE.establecimiento = :Establecimiento and SJP.tipo = 7 )')
    Left = 732
    Top = 270
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Establecimiento'
        ParamType = ptUnknown
      end>
  end
end
