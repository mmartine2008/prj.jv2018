inherited FRepInseminaciones: TFRepInseminaciones
  Left = 18
  Top = 0
  Caption = 'FRepInseminaciones'
  ClientHeight = 723
  ClientWidth = 980
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 980
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 596
  end
  object PAnimales: TPanel [4]
    Left = 0
    Top = 517
    Width = 980
    Height = 88
    Align = alClient
    Color = clWhite
    TabOrder = 4
    object GBAnimales: TGroupBox
      Left = 1
      Top = 1
      Width = 978
      Height = 86
      Align = alClient
      Caption = 'Animales con diagn'#243'stico de gestaci'#243'n [0]'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object BDBGAnimales: TBitDBGrid
        Left = 2
        Top = 18
        Width = 974
        Height = 66
        Align = alClient
        DataSource = DSRepInseminaciones
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = BDBGAnimalesDblClick
        AllowIncSearch = False
        ColumnVersion = 0
        DefaultColumns = True
        DrawingEllipse = True
        DrawReadOnly = False
        EnterKey = ekNone
        FillFields = False
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        FooterType = foNone
        InternalDrag = False
        LongCaption = True
        MultiCaption = True
        RealTitleFont.Charset = DEFAULT_CHARSET
        RealTitleFont.Color = clWindowText
        RealTitleFont.Height = -8
        RealTitleFont.Name = 'MS Sans Serif'
        RealTitleFont.Style = []
        RunTimePropEditor = True
        ShowCellHint = True
        TitleBtn = tbNone
        Columns = <
          item
            Expanded = False
            FieldName = 'RP_VACA'
            Title.Caption = 'RP Animal'
            Width = 81
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RAZA'
            Title.Caption = 'Raza'
            Width = 68
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FECHA_SINCRO'
            Title.Caption = 'Fecha Sincro'
            Width = 89
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'METODO_SINCRO'
            Title.Caption = 'Metodo'
            Width = 220
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FECHA_IATF'
            Title.Caption = 'Fecha Inseminacion'
            Width = 106
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_TORO'
            Title.Caption = 'Toro'
            Width = 99
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_RESP_IATF'
            Title.Caption = 'Inseminador'
            Width = 68
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FECHA_DG'
            Title.Caption = 'Fecha Diag'
            Width = 84
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_DIAG'
            Title.Caption = 'Diagnostico'
            Width = 72
            Visible = True
          end>
      end
    end
  end
  object PObservaciones: TPanel [5]
    Left = 0
    Top = 488
    Width = 980
    Height = 29
    Align = alTop
    Color = clWhite
    TabOrder = 5
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 978
      Height = 27
      Align = alClient
      Caption = 'Observaciones'
      TabOrder = 0
      object MObservaciones: TMemo
        Left = 2
        Top = 16
        Width = 974
        Height = 9
        Align = alClient
        ScrollBars = ssVertical
        TabOrder = 0
      end
    end
  end
  object PFiltros: TPanel [6]
    Left = 0
    Top = 179
    Width = 980
    Height = 94
    Align = alTop
    TabOrder = 6
    object RGFiltros: TRadioGroup
      Left = 1
      Top = 1
      Width = 640
      Height = 92
      Align = alLeft
      Caption = ' Filtros '
      Color = clWhite
      Columns = 2
      ItemIndex = 2
      Items.Strings = (
        'Rodeo'
        'Categoria'
        'Todos'
        'Tipo de Servicio'
        'Tipo de Registro')
      ParentColor = False
      TabOrder = 1
      OnClick = RGFiltrosClick
    end
    object GroupBox3: TGroupBox
      Left = 641
      Top = 1
      Width = 338
      Height = 92
      Align = alClient
      Caption = 'Opciones'
      Color = clWhite
      ParentColor = False
      TabOrder = 0
      object CBListado: TCheckBox
        Left = 34
        Top = 37
        Width = 105
        Height = 18
        Caption = 'Imprimir Listado'
        Checked = True
        State = cbChecked
        TabOrder = 0
      end
    end
    object DBLCBARodeos: TDBLookupComboBoxAuto
      Left = 106
      Top = 14
      Width = 179
      Height = 22
      Color = clWhite
      DropDownRows = 20
      Enabled = False
      KeyField = 'ID_RODEO'
      ListField = 'NOMBRE'
      ListSource = DMSoftvet.DSRodeo
      TabOrder = 2
      OnCloseUp = DBLCBARodeosCloseUp
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object CBACategoria: TComboBoxAuto
      Left = 106
      Top = 43
      Width = 179
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      Enabled = False
      ItemHeight = 14
      ItemIndex = 0
      TabOrder = 3
      Text = 'VACA'
      OnCloseUp = CBACategoriaCloseUp
      Items.Strings = (
        'VACA'
        'VAQUILLONA')
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object CBATipoServicio: TComboBoxAuto
      Left = 427
      Top = 15
      Width = 179
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      Enabled = False
      ItemHeight = 14
      ItemIndex = 0
      TabOrder = 4
      Text = 'INSEMINACI'#211'N ARTIFICIAL'
      OnCloseUp = CBATipoServicioCloseUp
      Items.Strings = (
        'INSEMINACI'#211'N ARTIFICIAL'
        'SERVICIO A CORRAL')
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object DBLCBATipoRegistro: TDBLookupComboBoxAuto
      Left = 427
      Top = 41
      Width = 179
      Height = 22
      Color = clWhite
      DropDownRows = 20
      Enabled = False
      KeyField = 'ID_SUBCATEGORIA'
      ListField = 'NOMBRE'
      ListSource = DMSoftvet.DSSubCategoriaRaza
      TabOrder = 5
      OnCloseUp = DBLCBATipoRegistroCloseUp
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
  end
  object PServicio: TPanel [7]
    Left = 0
    Top = 65
    Width = 980
    Height = 114
    Align = alTop
    TabOrder = 2
    object GBServicio: TGroupBox
      Left = 440
      Top = 1
      Width = 337
      Height = 112
      Align = alLeft
      Caption = 'Servicio'
      Color = clWhite
      Enabled = False
      ParentColor = False
      TabOrder = 0
      object Label1: TLabel
        Left = 43
        Top = 46
        Width = 48
        Height = 14
        Caption = 'Servicios:'
      end
      object DBLCBAServicio: TDBLookupComboBoxAuto
        Left = 32
        Top = 42
        Width = 257
        Height = 22
        Color = clWhite
        Enabled = False
        KeyField = 'ID_SERVICIO'
        ListField = 'NOMBRE'
        ListSource = DSServicios
        TabOrder = 0
        OnCloseUp = DBLCBAServicioCloseUp
        OnKeyUp = DBLCBAServicioKeyUp
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object GBFechas: TGroupBox
      Left = 200
      Top = 1
      Width = 240
      Height = 112
      Align = alLeft
      Caption = 'Fechas'
      Color = clWhite
      ParentColor = False
      TabOrder = 1
      object Label2: TLabel
        Left = 26
        Top = 34
        Width = 34
        Height = 14
        Caption = 'Desde:'
      end
      object Label3: TLabel
        Left = 26
        Top = 69
        Width = 31
        Height = 14
        Caption = 'Hasta:'
      end
      object DTPADesde: TDateTimePickerAuto
        Left = 78
        Top = 30
        Width = 121
        Height = 21
        Date = 39685.631138773150000000
        Time = 39685.631138773150000000
        Color = clWhite
        TabOrder = 0
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object DTPAHasta: TDateTimePickerAuto
        Left = 78
        Top = 65
        Width = 121
        Height = 21
        Date = 39685.631387974530000000
        Time = 39685.631387974530000000
        Color = clWhite
        TabOrder = 1
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
    object RGTipo: TRadioGroup
      Left = 1
      Top = 1
      Width = 199
      Height = 112
      Align = alLeft
      Caption = 'Tipo de Selecci'#243'n'
      Color = clWhite
      ItemIndex = 0
      Items.Strings = (
        'Fecha'
        'Servicio')
      ParentColor = False
      TabOrder = 2
      OnClick = RGTipoClick
    end
    object GConsulta: TGroupBox
      Left = 777
      Top = 1
      Width = 202
      Height = 112
      Align = alClient
      Caption = 'Consulta'
      Color = clWhite
      ParentColor = False
      TabOrder = 3
      object JvIBotonBuscar: TJvImage
        Tag = 4
        Left = 51
        Top = 39
        Width = 97
        Height = 32
        Cursor = crHandPoint
        Picture.Data = {
          
          E9FF0056DF9FC352571EAFFFD9}
        OnClick = JvIBotonBuscarClick
      end
      object BConsulta: TButton
        Left = 64
        Top = 88
        Width = 65
        Height = 17
        Caption = 'Consultar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        Visible = False
        OnClick = BConsultaClick
      end
    end
  end
  object PGraficos: TPanel [8]
    Left = 0
    Top = 273
    Width = 980
    Height = 215
    Align = alTop
    Color = clWhite
    TabOrder = 3
    object GBGraficos: TGroupBox
      Left = 1
      Top = 1
      Width = 488
      Height = 213
      Align = alLeft
      Caption = 'Grafico'
      TabOrder = 0
      object Panel1: TPanel
        Left = 34
        Top = 43
        Width = 663
        Height = 165
        TabOrder = 0
        Visible = False
        object DBCPorToro: TDBChart
          Left = -94
          Top = -46
          Width = 807
          Height = 315
          AllowZoom = False
          AnimatedZoomSteps = 10
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          MarginBottom = 25
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            '% Pre'#241'ez Seg'#250'n Toro')
          BottomAxis.LabelsAngle = 270
          BottomAxis.LabelsFont.Charset = ANSI_CHARSET
          BottomAxis.LabelsFont.Color = clBlack
          BottomAxis.LabelsFont.Height = -8
          BottomAxis.LabelsFont.Name = 'Trebuchet MS'
          BottomAxis.LabelsFont.Style = [fsBold]
          BottomAxis.LabelsSize = 1
          BottomAxis.TickLength = 2
          Chart3DPercent = 5
          LeftAxis.Title.Caption = '% '
          Legend.Alignment = laTop
          Legend.LegendStyle = lsSeries
          Legend.TextStyle = ltsPlain
          RightAxis.LabelsMultiLine = True
          View3DOptions.Perspective = 8
          OnGetAxisLabel = DBCPorToroGetAxisLabel
          OnGetLegendText = DBCGetLegendText
          Align = alCustom
          BevelOuter = bvNone
          BevelWidth = 0
          Color = clWhite
          TabOrder = 2
        end
        object DBCInseminador: TDBChart
          Left = 260
          Top = -52
          Width = 349
          Height = 316
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          MarginBottom = 25
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            '% Pre'#241'ez Seg'#250'n Inseminador')
          BottomAxis.LabelsAngle = 330
          BottomAxis.LabelsFont.Charset = DEFAULT_CHARSET
          BottomAxis.LabelsFont.Color = clBlack
          BottomAxis.LabelsFont.Height = -7
          BottomAxis.LabelsFont.Name = 'Arial'
          BottomAxis.LabelsFont.Style = []
          LeftAxis.Title.Caption = '%'
          Legend.Alignment = laTop
          Legend.LegendStyle = lsSeries
          OnGetAxisLabel = DBCInseminadorGetAxisLabel
          OnGetLegendText = DBCGetLegendText
          Align = alCustom
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 1
        end
        object DBCCC: TDBChart
          Left = 245
          Top = -55
          Width = 352
          Height = 316
          AllowZoom = False
          AnimatedZoomSteps = 10
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            '% Pre'#241'ez Seg'#250'n Condici'#243'n Corporal')
          BottomAxis.ExactDateTime = False
          BottomAxis.Increment = 1.000000000000000000
          BottomAxis.LabelsSeparation = 0
          BottomAxis.LabelsSize = 1
          BottomAxis.TickInnerLength = 3
          BottomAxis.TickLength = 2
          LeftAxis.Title.Caption = '% '
          Legend.Alignment = laTop
          Legend.LegendStyle = lsSeries
          Legend.TextStyle = ltsPlain
          OnGetAxisLabel = DBCCCGetAxisLabel
          OnGetLegendText = DBCGetLegendText
          Align = alCustom
          BevelOuter = bvNone
          BevelWidth = 0
          Color = clWhite
          TabOrder = 3
        end
        object DBCMetodo: TDBChart
          Left = 135
          Top = 116
          Width = 307
          Height = 316
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          MarginBottom = 25
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            '% Pre'#241'ez Seg'#250'n Metodo de Sincronizaci'#243'n')
          BottomAxis.LabelsAngle = 330
          BottomAxis.LabelsFont.Charset = DEFAULT_CHARSET
          BottomAxis.LabelsFont.Color = clBlack
          BottomAxis.LabelsFont.Height = -7
          BottomAxis.LabelsFont.Name = 'Arial'
          BottomAxis.LabelsFont.Style = []
          LeftAxis.Title.Caption = '%'
          Legend.Alignment = laTop
          Legend.LegendStyle = lsSeries
          OnGetAxisLabel = DBCMetodoGetAxisLabel
          OnGetLegendText = DBCGetLegendText
          Align = alCustom
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 0
        end
        object DBCGDR: TDBChart
          Left = 73
          Top = 177
          Width = 352
          Height = 315
          BackWall.Brush.Color = clWhite
          BackWall.Brush.Style = bsClear
          LeftWall.Color = clWhite
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Title.Text.Strings = (
            '% Pre'#241'ez Seg'#250'n GDR')
          BottomAxis.LabelsSeparation = 0
          BottomAxis.TickInnerLength = 3
          BottomAxis.TickLength = 2
          LeftAxis.Title.Caption = '%'
          Legend.Alignment = laTop
          Legend.LegendStyle = lsSeries
          OnGetAxisLabel = DBCGDRGetAxisLabel
          OnGetLegendText = DBCGetLegendText
          Align = alCustom
          BevelOuter = bvNone
          Color = clWhite
          TabOrder = 4
        end
      end
      object DBCTotales: TDBChart
        Left = 2
        Top = 16
        Width = 484
        Height = 195
        BackWall.Brush.Color = clWhite
        LeftWall.Color = clWhite
        MarginBottom = 25
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clBlack
        Title.Font.Height = -11
        Title.Font.Name = 'Arial'
        Title.Font.Style = [fsBold]
        Title.Text.Strings = (
          '% Pre'#241'ez')
        BottomAxis.LabelsFont.Charset = DEFAULT_CHARSET
        BottomAxis.LabelsFont.Color = clBlack
        BottomAxis.LabelsFont.Height = -7
        BottomAxis.LabelsFont.Name = 'Arial'
        BottomAxis.LabelsFont.Style = []
        Legend.Alignment = laTop
        Legend.TextStyle = ltsPlain
        View3DOptions.Elevation = 315
        View3DOptions.Orthogonal = False
        View3DOptions.Perspective = 0
        View3DOptions.Rotation = 360
        Align = alClient
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 1
      end
    end
    object GBServicios: TGroupBox
      Left = 489
      Top = 1
      Width = 490
      Height = 213
      Align = alClient
      Caption = 'Servicios'
      TabOrder = 1
      object SGServicios: TStringGrid
        Left = 2
        Top = 60
        Width = 486
        Height = 151
        Align = alClient
        BorderStyle = bsNone
        FixedColor = clMoneyGreen
        FixedCols = 0
        FixedRows = 0
        Options = [goFixedVertLine, goFixedHorzLine]
        TabOrder = 0
      end
      object Panel2: TPanel
        Left = 2
        Top = 16
        Width = 486
        Height = 44
        Align = alTop
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 1
        object Label4: TLabel
          Left = 103
          Top = 9
          Width = 58
          Height = 16
          Caption = 'Servicio'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label5: TLabel
          Left = 319
          Top = 9
          Width = 66
          Height = 16
          Caption = '% Pre'#241'ez'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
  end
  object PBotones: TPanel [9]
    Left = 0
    Top = 605
    Width = 980
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 7
    DesignSize = (
      980
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 978
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 904
      Top = 18
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = BBSalirClick
    end
    object LSalir: TLabel
      Left = 915
      Top = 71
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBSalirClick
    end
    object IExpExcel: TImage
      Left = 153
      Top = 15
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      OnClick = BBXlsClick
    end
    object IPreview: TImage
      Left = 309
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = BBImprimirReporteClick
    end
    object LPreview: TLabel
      Left = 306
      Top = 75
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBImprimirReporteClick
    end
    object IAyuda: TImage
      Left = 16
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object LExpExcel: TLabel
      Left = 131
      Top = 71
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBXlsClick
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 871
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      OnClick = BBXlsClick
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 980
    Caption = 'Informe de Inseminaci'#243'n Artificial/Servicio a Corral'
    inherited LTitulo: TLabel
      Width = 576
      Caption = 'Informe de Inseminaci'#243'n Artificial/Servicio a Corral'
    end
    inherited PILeft: TPanel
      Left = 772
    end
  end
  inherited Psocalo: TPanel
    Top = 701
    Width = 980
    inherited JvSocalo: TJvImage
      Width = 980
    end
  end
  inherited ALUniversal: TActionList
    Left = 115
    Top = 65534
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 36
    Top = 65535
  end
  inherited SDXLS: TSaveDialog
    Left = 62
    Top = 65533
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQRepInseminacion
    Columns = <
      item
        DataField = 'RP_VACA'
        Title = 'RP Animal'
      end
      item
        DataField = 'RAZA'
        Title = 'Raza'
      end
      item
        DataField = 'FECHA_SINCRO'
        Title = 'Fecha Sincro'
      end
      item
        DataField = 'METODO_SINCRO'
        Title = 'M'#233'todo de Sincronizaci'#243'n'
      end
      item
        DataField = 'FECHA_IATF'
        Title = 'Fecha Inseminaci'#243'n'
      end
      item
        DataField = 'NOM_TORO'
        Title = 'Toro'
      end
      item
        DataField = 'NOM_RESP_IATF'
        Title = 'Inseminador'
      end
      item
        DataField = 'FECHA_DG'
        Title = 'Fecha Diag'
      end
      item
        DataField = 'NOM_DIAG'
        Title = 'Diagn'#243'stico'
      end>
    Title.Text = 'Informe de Inseminaci'#243'n Artificial/Servicio a Corral'
    Left = 88
    Top = 1
  end
  inherited WinXP1: TWinXP
    Left = 576
    Top = 0
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 548
    Top = 65535
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 660
    Top = 65535
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 628
    Top = 65535
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 148
    Top = 65527
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 692
    Top = 65535
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 596
    Top = 65535
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 8
    Top = 0
  end
  object IBQServicios: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQServiciosAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * FROM SP_SERVICIOS_CON_DIAGNOSTICO(:ESTA)')
    Left = 473
    Top = 49
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end>
  end
  object DSServicios: TDataSource
    DataSet = IBQServicios
    Left = 513
    Top = 41
  end
  object IBQRepInseminacion: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQRepInseminacionAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from REP_INSEMINACIONES(:ESTA,:SERVICIO,:TIPO,:DESDE,:H' +
        'ASTA,:CATEG,:RODEO,:REG,:SERV)')
    Left = 785
    Top = 58
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ESTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'SERVICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'TIPO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DESDE'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'HASTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CATEG'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'RODEO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'REG'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'SERV'
        ParamType = ptUnknown
      end>
    object IBQRepInseminacionVACA: TIntegerField
      FieldName = 'VACA'
      Origin = 'REP_INSEMINACIONES.VACA'
    end
    object IBQRepInseminacionNOM_SERVICIO: TIBStringField
      FieldName = 'NOM_SERVICIO'
      Origin = 'REP_INSEMINACIONES.NOM_SERVICIO'
      Size = 50
    end
    object IBQRepInseminacionRP_VACA: TIBStringField
      FieldName = 'RP_VACA'
      Origin = 'REP_INSEMINACIONES.RP_VACA'
      Size = 10
    end
    object IBQRepInseminacionRAZA: TIBStringField
      FieldName = 'RAZA'
      Origin = 'REP_INSEMINACIONES.RAZA'
      Size = 2
    end
    object IBQRepInseminacionFECHA_SINCRO: TDateField
      FieldName = 'FECHA_SINCRO'
      Origin = 'REP_INSEMINACIONES.FECHA_SINCRO'
    end
    object IBQRepInseminacionMETODO_SINCRO: TIBStringField
      FieldName = 'METODO_SINCRO'
      Origin = 'REP_INSEMINACIONES.METODO_SINCRO'
      Size = 100
    end
    object IBQRepInseminacionFECHA_IATF: TDateField
      FieldName = 'FECHA_IATF'
      Origin = 'REP_INSEMINACIONES.FECHA_IATF'
    end
    object IBQRepInseminacionTORO_IATF: TIntegerField
      FieldName = 'TORO_IATF'
      Origin = 'REP_INSEMINACIONES.TORO_IATF'
    end
    object IBQRepInseminacionNOM_TORO: TIBStringField
      FieldName = 'NOM_TORO'
      Origin = 'REP_INSEMINACIONES.NOM_TORO'
      Size = 100
    end
    object IBQRepInseminacionRESP_IATF: TIntegerField
      FieldName = 'RESP_IATF'
      Origin = 'REP_INSEMINACIONES.RESP_IATF'
    end
    object IBQRepInseminacionNOM_RESP_IATF: TIBStringField
      FieldName = 'NOM_RESP_IATF'
      Origin = 'REP_INSEMINACIONES.NOM_RESP_IATF'
      Size = 100
    end
    object IBQRepInseminacionFECHA_DG: TDateField
      FieldName = 'FECHA_DG'
      Origin = 'REP_INSEMINACIONES.FECHA_DG'
    end
    object IBQRepInseminacionDIAGNOSTICO: TIntegerField
      FieldName = 'DIAGNOSTICO'
      Origin = 'REP_INSEMINACIONES.DIAGNOSTICO'
    end
    object IBQRepInseminacionNOM_DIAG: TIBStringField
      FieldName = 'NOM_DIAG'
      Origin = 'REP_INSEMINACIONES.NOM_DIAG'
      Size = 12
    end
    object IBQRepInseminacionCONDICION_CORPORAL: TIntegerField
      FieldName = 'CONDICION_CORPORAL'
      Origin = 'REP_INSEMINACIONES.CONDICION_CORPORAL'
    end
    object IBQRepInseminacionGDR: TIntegerField
      FieldName = 'GDR'
      Origin = 'REP_INSEMINACIONES.GDR'
    end
    object IBQRepInseminacionVALOR_CC: TFloatField
      FieldKind = fkLookup
      FieldName = 'VALOR_CC'
      LookupDataSet = IBQCondCorp
      LookupKeyFields = 'ID_CONDICION_CORPORAL'
      LookupResultField = 'VALOR'
      KeyFields = 'CONDICION_CORPORAL'
      Lookup = True
    end
  end
  object DSRepInseminaciones: TDataSource
    DataSet = IBQRepInseminacion
    Left = 817
    Top = 42
  end
  object IBQCondCorp: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select id_condicion_corporal, valor_escala_5 as valor from cod_c' +
        'ondiciones_corporales')
    Left = 745
    Top = 162
    object IBQCondCorpVALOR: TFloatField
      FieldName = 'VALOR'
      Origin = 'COD_CONDICIONES_CORPORALES.VALOR_ESCALA_5'
      Required = True
    end
    object IBQCondCorpID_CONDICION_CORPORAL: TIntegerField
      FieldName = 'ID_CONDICION_CORPORAL'
      Origin = 'COD_CONDICIONES_CORPORALES.ID_CONDICION_CORPORAL'
      Required = True
    end
  end
end
