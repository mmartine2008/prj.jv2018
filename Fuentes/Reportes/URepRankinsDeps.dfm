inherited FRepRankinsDeps: TFRepRankinsDeps
  Left = 262
  Top = 23
  Caption = 'FRepRankinsDeps'
  ClientHeight = 717
  ClientWidth = 963
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 963
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 579
  end
  object PDEPsUruguay: TPanel [4]
    Left = 176
    Top = 505
    Width = 983
    Height = 145
    Color = clWhite
    TabOrder = 4
    Visible = False
    object Shape40: TShape
      Left = 9
      Top = 69
      Width = 947
      Height = 74
    end
    object Shape41: TShape
      Left = 9
      Top = 9
      Width = 706
      Height = 61
      Brush.Color = clBlack
    end
    object Shape42: TShape
      Left = 714
      Top = 34
      Width = 36
      Height = 36
    end
    object Shape43: TShape
      Left = 748
      Top = 34
      Width = 36
      Height = 36
    end
    object Shape44: TShape
      Left = 783
      Top = 34
      Width = 35
      Height = 36
    end
    object Shape45: TShape
      Left = 817
      Top = 34
      Width = 36
      Height = 36
    end
    object Shape46: TShape
      Left = 852
      Top = 34
      Width = 35
      Height = 36
    end
    object Shape47: TShape
      Left = 886
      Top = 34
      Width = 36
      Height = 36
    end
    object Shape48: TShape
      Left = 921
      Top = 34
      Width = 35
      Height = 36
    end
    object Shape49: TShape
      Left = 713
      Top = 9
      Width = 243
      Height = 27
      Brush.Color = clBlack
    end
    object Label73: TLabel
      Left = 719
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label74: TLabel
      Left = 719
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label75: TLabel
      Left = 754
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label76: TLabel
      Left = 754
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label77: TLabel
      Left = 788
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label78: TLabel
      Left = 788
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label79: TLabel
      Left = 823
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label80: TLabel
      Left = 823
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label81: TLabel
      Left = 857
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label82: TLabel
      Left = 857
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label83: TLabel
      Left = 892
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label84: TLabel
      Left = 892
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label85: TLabel
      Left = 926
      Top = 52
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label86: TLabel
      Left = 926
      Top = 37
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Shape50: TShape
      Left = 714
      Top = 69
      Width = 36
      Height = 74
    end
    object Shape51: TShape
      Left = 748
      Top = 69
      Width = 36
      Height = 74
    end
    object Shape52: TShape
      Left = 783
      Top = 69
      Width = 35
      Height = 74
    end
    object Shape53: TShape
      Left = 817
      Top = 69
      Width = 36
      Height = 74
    end
    object Shape54: TShape
      Left = 852
      Top = 69
      Width = 35
      Height = 74
    end
    object Shape55: TShape
      Left = 886
      Top = 69
      Width = 36
      Height = 74
    end
    object Shape56: TShape
      Left = 921
      Top = 69
      Width = 35
      Height = 74
    end
    object Shape57: TShape
      Left = 715
      Top = 103
      Width = 241
      Height = 1
    end
    object Shape58: TShape
      Left = 9
      Top = 69
      Width = 61
      Height = 74
    end
    object LAnioU: TLabel
      Left = 22
      Top = 98
      Width = 33
      Height = 13
      Caption = 'LAnio'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object LNombreU: TLabel
      Left = 78
      Top = 78
      Width = 43
      Height = 13
      Caption = 'LNombre'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LNombreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
    object Label89: TLabel
      Left = 332
      Top = 14
      Width = 33
      Height = 13
      Caption = 'Criador'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label90: TLabel
      Left = 418
      Top = 14
      Width = 66
      Height = 36
      Alignment = taCenter
      AutoSize = False
      Caption = 'C'#243'digo Propietario'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object Label91: TLabel
      Left = 522
      Top = 14
      Width = 18
      Height = 13
      Caption = 'Nhij'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label92: TLabel
      Left = 570
      Top = 14
      Width = 27
      Height = 13
      Caption = 'NhijM'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label93: TLabel
      Left = 622
      Top = 14
      Width = 26
      Height = 13
      Caption = 'NhijU'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label94: TLabel
      Left = 672
      Top = 14
      Width = 36
      Height = 36
      Alignment = taCenter
      AutoSize = False
      Caption = 'Dato PD'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object Shape59: TShape
      Left = 660
      Top = 69
      Width = 55
      Height = 74
    end
    object Shape60: TShape
      Left = 608
      Top = 69
      Width = 55
      Height = 74
    end
    object Shape61: TShape
      Left = 557
      Top = 69
      Width = 55
      Height = 74
    end
    object Shape62: TShape
      Left = 505
      Top = 69
      Width = 55
      Height = 74
    end
    object Shape63: TShape
      Left = 402
      Top = 69
      Width = 104
      Height = 74
    end
    object Shape64: TShape
      Left = 307
      Top = 69
      Width = 96
      Height = 74
    end
    object Label95: TLabel
      Left = 78
      Top = 103
      Width = 24
      Height = 14
      Caption = 'HBU:'
    end
    object Label96: TLabel
      Left = 78
      Top = 121
      Width = 22
      Height = 14
      Caption = 'Nac:'
    end
    object LCriadorU: TLabel
      Left = 317
      Top = 101
      Width = 48
      Height = 14
      Caption = 'LCriadorU'
    end
    object LPropU: TLabel
      Left = 414
      Top = 103
      Width = 35
      Height = 14
      Caption = 'LPropU'
    end
    object LNhij: TLabel
      Left = 514
      Top = 103
      Width = 23
      Height = 14
      Caption = 'LNhij'
    end
    object LNhijM: TLabel
      Left = 566
      Top = 103
      Width = 31
      Height = 14
      Caption = 'LNhijM'
    end
    object LNhijU: TLabel
      Left = 618
      Top = 103
      Width = 30
      Height = 14
      Caption = 'LNhijU'
    end
    object LDatoPD: TLabel
      Left = 670
      Top = 103
      Width = 41
      Height = 14
      Caption = 'LDatoPD'
    end
    object Label97: TLabel
      Left = 826
      Top = 14
      Width = 19
      Height = 13
      Caption = 'P18'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label98: TLabel
      Left = 923
      Top = 14
      Width = 31
      Height = 13
      Caption = 'E.G.C.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label99: TLabel
      Left = 888
      Top = 14
      Width = 31
      Height = 13
      Caption = 'A.O.B.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label100: TLabel
      Left = 857
      Top = 14
      Width = 20
      Height = 13
      Caption = 'C.E.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label101: TLabel
      Left = 795
      Top = 14
      Width = 14
      Height = 13
      Caption = 'HL'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label102: TLabel
      Left = 759
      Top = 14
      Width = 15
      Height = 13
      Caption = 'PD'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label103: TLabel
      Left = 724
      Top = 14
      Width = 15
      Height = 13
      Caption = 'PN'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object LDepNacerU: TLabel
      Left = 720
      Top = 81
      Width = 54
      Height = 14
      Caption = 'LDepNacer'
    end
    object LPrecNacerU: TLabel
      Left = 720
      Top = 116
      Width = 57
      Height = 14
      Caption = 'LPrecNacer'
    end
    object LDepEGCU: TLabel
      Left = 927
      Top = 80
      Width = 46
      Height = 14
      Caption = 'LDepEGC'
    end
    object LPrecEGCU: TLabel
      Left = 927
      Top = 115
      Width = 49
      Height = 14
      Caption = 'LPrecEGC'
    end
    object LDepAOBU: TLabel
      Left = 893
      Top = 81
      Width = 48
      Height = 14
      Caption = 'LDepAOB'
    end
    object LPrecAOBU: TLabel
      Left = 893
      Top = 116
      Width = 51
      Height = 14
      Caption = 'LPrecAOB'
    end
    object LDepCEU: TLabel
      Left = 859
      Top = 81
      Width = 38
      Height = 14
      Caption = 'LDepCE'
    end
    object LPrecCEU: TLabel
      Left = 859
      Top = 116
      Width = 41
      Height = 14
      Caption = 'LPrecCE'
    end
    object LDepP18: TLabel
      Left = 824
      Top = 81
      Width = 43
      Height = 14
      Caption = 'LDepP18'
    end
    object LPrecP18: TLabel
      Left = 824
      Top = 116
      Width = 46
      Height = 14
      Caption = 'LPrecP18'
    end
    object LPrecHL: TLabel
      Left = 789
      Top = 116
      Width = 41
      Height = 14
      Caption = 'LPrecHL'
    end
    object LDepHL: TLabel
      Left = 789
      Top = 81
      Width = 38
      Height = 14
      Caption = 'LDepHL'
    end
    object LDepDestU: TLabel
      Left = 756
      Top = 81
      Width = 47
      Height = 14
      Caption = 'LDepDest'
    end
    object LPrecDestU: TLabel
      Left = 755
      Top = 116
      Width = 50
      Height = 14
      Caption = 'LPrecDest'
    end
    object LHBU: TLabel
      Left = 112
      Top = 103
      Width = 29
      Height = 13
      Caption = 'LHBU'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LNombreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
    object LNacimientoU: TLabel
      Left = 112
      Top = 121
      Width = 67
      Height = 13
      Caption = 'LNacimientoU'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LNombreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
  end
  object PDEP: TPanel [5]
    Left = 0
    Top = 453
    Width = 963
    Height = 138
    Align = alBottom
    Color = clWhite
    TabOrder = 3
    object Shape1: TShape
      Left = 9
      Top = 60
      Width = 947
      Height = 75
    end
    object Shape2: TShape
      Left = 9
      Top = 0
      Width = 533
      Height = 61
      Brush.Color = clBlack
    end
    object Shape3: TShape
      Left = 542
      Top = 0
      Width = 414
      Height = 27
      Brush.Color = clBlack
    end
    object Shape4: TShape
      Left = 542
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape5: TShape
      Left = 576
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape6: TShape
      Left = 611
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape7: TShape
      Left = 645
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape8: TShape
      Left = 680
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape9: TShape
      Left = 714
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape10: TShape
      Left = 748
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape11: TShape
      Left = 783
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape12: TShape
      Left = 817
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape13: TShape
      Left = 852
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape14: TShape
      Left = 886
      Top = 26
      Width = 36
      Height = 35
    end
    object Shape15: TShape
      Left = 921
      Top = 26
      Width = 35
      Height = 35
    end
    object Shape16: TShape
      Left = 576
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape17: TShape
      Left = 542
      Top = 60
      Width = 35
      Height = 75
    end
    object Shape18: TShape
      Left = 611
      Top = 60
      Width = 35
      Height = 75
    end
    object Shape19: TShape
      Left = 645
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape20: TShape
      Left = 680
      Top = 60
      Width = 35
      Height = 75
    end
    object Shape21: TShape
      Left = 714
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape22: TShape
      Left = 748
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape23: TShape
      Left = 783
      Top = 60
      Width = 35
      Height = 75
    end
    object Shape24: TShape
      Left = 817
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape25: TShape
      Left = 852
      Top = 60
      Width = 35
      Height = 75
    end
    object Shape26: TShape
      Left = 886
      Top = 60
      Width = 36
      Height = 75
    end
    object Shape27: TShape
      Left = 542
      Top = 97
      Width = 414
      Height = 1
    end
    object Shape28: TShape
      Left = 9
      Top = 60
      Width = 61
      Height = 75
    end
    object Label30: TLabel
      Left = 90
      Top = 17
      Width = 37
      Height = 13
      Caption = 'Nombre'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label31: TLabel
      Left = 306
      Top = 17
      Width = 22
      Height = 13
      Caption = 'HBA'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label32: TLabel
      Left = 375
      Top = 12
      Width = 65
      Height = 35
      Alignment = taCenter
      AutoSize = False
      Caption = 'A'#241'o de Nacimiento'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object Label33: TLabel
      Left = 500
      Top = 10
      Width = 23
      Height = 13
      Caption = 'Crias'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label34: TLabel
      Left = 493
      Top = 30
      Width = 32
      Height = 13
      Caption = 'Rodeo'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label35: TLabel
      Left = 547
      Top = 6
      Width = 25
      Height = 13
      Caption = 'Gest.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label36: TLabel
      Left = 579
      Top = 6
      Width = 29
      Height = 13
      Caption = 'Nacer'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label37: TLabel
      Left = 615
      Top = 6
      Width = 25
      Height = 13
      Caption = 'Dest.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label38: TLabel
      Left = 645
      Top = 6
      Width = 30
      Height = 13
      Caption = 'Leche'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label39: TLabel
      Left = 685
      Top = 6
      Width = 22
      Height = 13
      Caption = 'Final'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label40: TLabel
      Left = 719
      Top = 6
      Width = 20
      Height = 13
      Caption = 'C.E.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label41: TLabel
      Left = 751
      Top = 6
      Width = 27
      Height = 13
      Caption = 'Altura'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label42: TLabel
      Left = 785
      Top = 6
      Width = 32
      Height = 13
      Caption = 'E.G.D.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label43: TLabel
      Left = 820
      Top = 6
      Width = 31
      Height = 13
      Caption = 'E.G.C.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label44: TLabel
      Left = 854
      Top = 6
      Width = 31
      Height = 13
      Caption = 'A.O.B.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label45: TLabel
      Left = 888
      Top = 6
      Width = 28
      Height = 13
      Caption = '% G.I.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label46: TLabel
      Left = 922
      Top = 6
      Width = 33
      Height = 13
      Caption = '% C.M.'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label47: TLabel
      Left = 78
      Top = 96
      Width = 31
      Height = 14
      Caption = 'Padre:'
    end
    object Label48: TLabel
      Left = 78
      Top = 112
      Width = 33
      Height = 14
      Caption = 'Madre:'
    end
    object Label49: TLabel
      Left = 547
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label50: TLabel
      Left = 582
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label51: TLabel
      Left = 616
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label52: TLabel
      Left = 650
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label53: TLabel
      Left = 685
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label54: TLabel
      Left = 719
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label55: TLabel
      Left = 754
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label56: TLabel
      Left = 788
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label57: TLabel
      Left = 823
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label58: TLabel
      Left = 857
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label59: TLabel
      Left = 892
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label60: TLabel
      Left = 926
      Top = 28
      Width = 22
      Height = 13
      Caption = 'DEP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label61: TLabel
      Left = 547
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label62: TLabel
      Left = 582
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label63: TLabel
      Left = 616
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label64: TLabel
      Left = 650
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label65: TLabel
      Left = 685
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label66: TLabel
      Left = 719
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label67: TLabel
      Left = 754
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label68: TLabel
      Left = 788
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label69: TLabel
      Left = 823
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label70: TLabel
      Left = 857
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label71: TLabel
      Left = 892
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label72: TLabel
      Left = 926
      Top = 43
      Width = 22
      Height = 13
      Caption = 'Prec'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -4
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object LDepCM: TLabel
      Left = 927
      Top = 70
      Width = 40
      Height = 14
      Caption = 'LDepCM'
    end
    object LPrecCM: TLabel
      Left = 927
      Top = 106
      Width = 43
      Height = 14
      Caption = 'LPrecCM'
    end
    object LDepGI: TLabel
      Left = 893
      Top = 71
      Width = 35
      Height = 14
      Caption = 'LDepGI'
    end
    object LPrecGI: TLabel
      Left = 893
      Top = 107
      Width = 38
      Height = 14
      Caption = 'LPrecGI'
    end
    object LDepAOB: TLabel
      Left = 858
      Top = 71
      Width = 48
      Height = 14
      Caption = 'LDepAOB'
    end
    object LPrecAOB: TLabel
      Left = 858
      Top = 107
      Width = 51
      Height = 14
      Caption = 'LPrecAOB'
    end
    object LDepEGC: TLabel
      Left = 824
      Top = 71
      Width = 46
      Height = 14
      Caption = 'LDepEGC'
    end
    object LPrecEGC: TLabel
      Left = 824
      Top = 107
      Width = 49
      Height = 14
      Caption = 'LPrecEGC'
    end
    object LDepEGD: TLabel
      Left = 788
      Top = 71
      Width = 46
      Height = 14
      Caption = 'LDepEGD'
    end
    object LPrecEGD: TLabel
      Left = 788
      Top = 107
      Width = 49
      Height = 14
      Caption = 'LPrecEGD'
    end
    object LDepAltura: TLabel
      Left = 754
      Top = 71
      Width = 54
      Height = 14
      Caption = 'LDepAltura'
    end
    object LPrecAltura: TLabel
      Left = 754
      Top = 107
      Width = 57
      Height = 14
      Caption = 'LPrecAltura'
    end
    object LDepCE: TLabel
      Left = 720
      Top = 71
      Width = 38
      Height = 14
      Caption = 'LDepCE'
    end
    object LPrecCE: TLabel
      Left = 720
      Top = 107
      Width = 41
      Height = 14
      Caption = 'LPrecCE'
    end
    object LDepFinal: TLabel
      Left = 686
      Top = 71
      Width = 47
      Height = 14
      Caption = 'LDepFinal'
    end
    object LPrecFinal: TLabel
      Left = 686
      Top = 107
      Width = 50
      Height = 14
      Caption = 'LPrecFinal'
    end
    object LDepLeche: TLabel
      Left = 652
      Top = 71
      Width = 55
      Height = 14
      Caption = 'LDepLeche'
    end
    object LPrecLeche: TLabel
      Left = 652
      Top = 107
      Width = 58
      Height = 14
      Caption = 'LPrecLeche'
    end
    object LDepDest: TLabel
      Left = 618
      Top = 71
      Width = 47
      Height = 14
      Caption = 'LDepDest'
    end
    object LPrecDest: TLabel
      Left = 617
      Top = 107
      Width = 50
      Height = 14
      Caption = 'LPrecDest'
    end
    object LDepNacer: TLabel
      Left = 583
      Top = 71
      Width = 54
      Height = 14
      Caption = 'LDepNacer'
    end
    object LPrecNacer: TLabel
      Left = 583
      Top = 107
      Width = 57
      Height = 14
      Caption = 'LPrecNacer'
    end
    object LDepGest: TLabel
      Left = 548
      Top = 71
      Width = 48
      Height = 14
      Caption = 'LDepGest'
    end
    object LPrecGest: TLabel
      Left = 548
      Top = 107
      Width = 51
      Height = 14
      Caption = 'LPrecGest'
    end
    object LCrias: TLabel
      Left = 495
      Top = 71
      Width = 31
      Height = 14
      Caption = 'LCrias'
    end
    object LRodeo: TLabel
      Left = 491
      Top = 103
      Width = 37
      Height = 14
      Caption = 'LRodeo'
    end
    object LNacimiento: TLabel
      Left = 376
      Top = 71
      Width = 59
      Height = 13
      Caption = 'LNacimiento'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object LHBA: TLabel
      Left = 302
      Top = 71
      Width = 28
      Height = 13
      Caption = 'LHBA'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object LNombre: TLabel
      Left = 78
      Top = 71
      Width = 43
      Height = 13
      Caption = 'LNombre'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LNombreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
    object LPadre: TLabel
      Left = 121
      Top = 96
      Width = 34
      Height = 13
      Caption = 'LPadre'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LPadreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
    object LMadre: TLabel
      Left = 121
      Top = 112
      Width = 36
      Height = 13
      Caption = 'LMadre'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      OnClick = LMadreClick
      OnMouseEnter = LNombreMouseEnter
      OnMouseLeave = LNombreMouseLeave
    end
    object LAnio: TLabel
      Left = 22
      Top = 90
      Width = 33
      Height = 13
      Caption = 'LAnio'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Ptop: TPanel [6]
    Left = 0
    Top = 65
    Width = 963
    Height = 388
    Align = alClient
    Color = clWhite
    TabOrder = 2
    object Panel1: TPanel
      Left = 632
      Top = 87
      Width = 330
      Height = 300
      Align = alClient
      Color = clWhite
      TabOrder = 3
      object IVerAnimales: TImage
        Left = 65
        Top = 8
        Width = 77
        Height = 59
        Cursor = crHandPoint
        AutoSize = True
        Picture.Data = {
          
          00000049454E44AE426082}
        OnClick = BBVerAnimalesClick
      end
      object Label88: TLabel
        Left = 155
        Top = 27
        Width = 76
        Height = 16
        Cursor = crHandPoint
        Caption = 'Ver Animales'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        Transparent = True
        OnClick = BBVerAnimalesClick
      end
      object GBAnimales: TGroupBox
        Left = 1
        Top = 51
        Width = 328
        Height = 248
        Align = alBottom
        Caption = 'Animales[0]'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        object BDBGAnimales: TBitDBGrid
          Left = 2
          Top = 18
          Width = 324
          Height = 228
          Align = alClient
          DataSource = DSAnimales
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -13
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnCellClick = BDBGAnimalesCellClick
          OnDblClick = BDBGAnimalesDblClick
          AllowIncSearch = False
          ColumnVersion = 0
          DefaultColumns = True
          DrawingEllipse = True
          DrawReadOnly = False
          EnterKey = ekNone
          FillFields = False
          FooterColor = clWindow
          FooterFont.Charset = DEFAULT_CHARSET
          FooterFont.Color = clWindowText
          FooterFont.Height = -11
          FooterFont.Name = 'MS Sans Serif'
          FooterFont.Style = []
          FooterType = foNone
          InternalDrag = False
          LongCaption = True
          MultiCaption = True
          RealTitleFont.Charset = DEFAULT_CHARSET
          RealTitleFont.Color = clWindowText
          RealTitleFont.Height = -8
          RealTitleFont.Name = 'MS Sans Serif'
          RealTitleFont.Style = []
          RunTimePropEditor = True
          ShowCellHint = True
          TitleBtn = tbNone
          Columns = <
            item
              Expanded = False
              FieldName = 'RP_ANIMAL'
              Title.Caption = 'RP Animal'
              Width = 78
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'ID_HBA'
              Title.Caption = 'HBA'
              Width = 68
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOMBRE_ANIMAL'
              Title.Caption = 'Nombre'
              Width = 137
              Visible = True
            end>
        end
      end
      object BBVerAnimales: TButton
        Left = -78
        Top = 28
        Width = 137
        Height = 33
        Caption = 'Ver animales'
        TabOrder = 1
        Visible = False
        OnClick = BBVerAnimalesClick
      end
    end
    object GroupBox1: TGroupBox
      Left = -111
      Top = 258
      Width = 710
      Height = 164
      Align = alCustom
      Caption = ' Selecci'#243'n de animales '
      TabOrder = 0
      Visible = False
      object BBAgregar: TBitBtn
        Left = 293
        Top = 67
        Width = 61
        Height = 27
        Caption = '>'
        TabOrder = 0
        OnClick = BBAgregarClick
      end
      object BBEliminar: TBitBtn
        Left = 293
        Top = 101
        Width = 61
        Height = 27
        Caption = '<'
        Enabled = False
        TabOrder = 1
        OnClick = BBEliminarClick
      end
      object RGOrden: TRadioGroup
        Left = 233
        Top = 17
        Width = 199
        Height = 44
        Caption = 'Orden'
        Columns = 2
        ItemIndex = 0
        Items.Strings = (
          'Ascendente'
          'Descendente')
        TabOrder = 2
      end
      object GroupBox3: TGroupBox
        Left = 17
        Top = 20
        Width = 199
        Height = 122
        Caption = 'Campos a Seleccionar'
        TabOrder = 3
        object LBCamposSeleccionar: TListBox
          Left = 2
          Top = 16
          Width = 195
          Height = 104
          Align = alClient
          ItemHeight = 14
          TabOrder = 0
        end
      end
      object GroupBox4: TGroupBox
        Left = 454
        Top = 23
        Width = 200
        Height = 121
        Caption = 'Campos Seleccionados'
        TabOrder = 4
        object SGSeleccionados: TStringGrid
          Left = 2
          Top = 16
          Width = 196
          Height = 103
          Align = alClient
          ColCount = 2
          DefaultRowHeight = 15
          FixedCols = 0
          RowCount = 2
          TabOrder = 0
          ColWidths = (
            104
            64)
        end
      end
    end
    object GBFiltro: TGroupBox
      Left = 1
      Top = 1
      Width = 961
      Height = 86
      Align = alTop
      Caption = 'Buscar por:'
      Color = clWhite
      ParentColor = False
      TabOrder = 1
      object GBFiltros: TGroupBox
        Left = 150
        Top = 16
        Width = 809
        Height = 68
        Align = alClient
        Caption = 'Filtros'
        TabOrder = 0
        object CBAnio: TCheckBox
          Tag = 21
          Left = 17
          Top = 28
          Width = 44
          Height = 18
          Caption = 'A'#241'o'
          Enabled = False
          TabOrder = 0
          OnClick = CBAnioClick
        end
        object EAAnio: TEditAuto
          Left = 72
          Top = 26
          Width = 41
          Height = 22
          CharCase = ecUpperCase
          Color = clWhite
          Enabled = False
          TabOrder = 1
          Text = '2008'
          OnKeyPress = EAAnioKeyPress
          EsClave = False
          EsRequerido = False
          ISParam = True
        end
        object UDAnio: TUpDown
          Left = 113
          Top = 26
          Width = 17
          Height = 22
          Associate = EAAnio
          Enabled = False
          Min = 1900
          Max = 2100
          Position = 2008
          TabOrder = 2
          Thousands = False
        end
        object CBSexo: TCheckBox
          Tag = 21
          Left = 138
          Top = 28
          Width = 53
          Height = 18
          Caption = 'Sexo'
          Enabled = False
          TabOrder = 3
          OnClick = CBSexoClick
        end
        object CBoxSexo: TComboBox
          Left = 197
          Top = 25
          Width = 131
          Height = 22
          AutoComplete = False
          Style = csDropDownList
          Color = clWhite
          Enabled = False
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 4
          Text = 'MACHOS'
          Items.Strings = (
            'MACHOS'
            'HEMBRAS')
        end
        object CBTipoAnimal: TCheckBox
          Tag = 21
          Left = 345
          Top = 19
          Width = 70
          Height = 34
          Caption = 'Tipo de Animal'
          Enabled = False
          TabOrder = 5
          WordWrap = True
          OnClick = CBTipoAnimalClick
        end
        object CBoxTipoAnimal: TComboBox
          Left = 418
          Top = 26
          Width = 126
          Height = 22
          AutoComplete = False
          Style = csDropDownList
          Color = clWhite
          Enabled = False
          ItemHeight = 14
          ItemIndex = 0
          TabOrder = 6
          Text = 'INTERNOS'
          OnKeyPress = CBoxTipoAnimalKeyPress
          Items.Strings = (
            'INTERNOS'
            'EXTERNOS')
        end
        object CBFuente: TCheckBox
          Tag = 21
          Left = 551
          Top = 28
          Width = 62
          Height = 18
          Caption = 'Fuente'
          Enabled = False
          TabOrder = 7
          OnClick = CBFuenteClick
        end
        object DBLCBAFuentes: TDBLookupComboBoxAuto
          Left = 618
          Top = 26
          Width = 176
          Height = 22
          Color = clWhite
          Enabled = False
          KeyField = 'ID_FUENTE_DEPS'
          ListField = 'NOMBRE'
          ListSource = DSFuentes
          TabOrder = 8
          EsClave = False
          EsRequerido = False
          ISParam = False
        end
      end
      object GroupBox5: TGroupBox
        Left = 2
        Top = 16
        Width = 148
        Height = 68
        Align = alLeft
        TabOrder = 1
        object RBFiltrar: TRadioButton
          Left = 17
          Top = 38
          Width = 62
          Height = 22
          Caption = 'Filtrar'
          TabOrder = 0
          OnClick = RBFiltrarClick
        end
        object RBTodas: TRadioButton
          Left = 17
          Top = 13
          Width = 62
          Height = 23
          Caption = 'Todos'
          Checked = True
          TabOrder = 1
          TabStop = True
          OnClick = RBTodasClick
        end
      end
    end
    object GBCamposDEPs: TGroupBox
      Left = 1
      Top = 87
      Width = 631
      Height = 300
      Align = alLeft
      Caption = 'Filtro de DEPs'
      TabOrder = 2
      object Label1: TLabel
        Left = 75
        Top = 52
        Width = 83
        Height = 14
        Caption = 'Largo Gestaci'#243'n:'
      end
      object Label2: TLabel
        Left = 86
        Top = 75
        Width = 70
        Height = 14
        Caption = 'Peso al Nacer:'
      end
      object Label3: TLabel
        Left = 79
        Top = 98
        Width = 78
        Height = 14
        Caption = 'Peso al Destete:'
      end
      object Label4: TLabel
        Left = 127
        Top = 121
        Width = 33
        Height = 14
        Caption = 'Leche:'
      end
      object Label5: TLabel
        Left = 107
        Top = 143
        Width = 52
        Height = 14
        Caption = 'Peso Final:'
      end
      object Label6: TLabel
        Left = 42
        Top = 166
        Width = 116
        Height = 14
        Caption = 'Circunferencia Escrotal:'
        WordWrap = True
      end
      object Label7: TLabel
        Left = 130
        Top = 190
        Width = 32
        Height = 14
        Caption = 'Altura:'
      end
      object Label8: TLabel
        Left = 33
        Top = 212
        Width = 125
        Height = 14
        Caption = 'Espesor de Grasa Dorsal:'
      end
      object Label9: TLabel
        Left = 13
        Top = 234
        Width = 144
        Height = 14
        Caption = 'Espesor de Grasa de Cadera:'
      end
      object Label10: TLabel
        Left = 76
        Top = 256
        Width = 83
        Height = 14
        Caption = 'Area Ojo de Bife:'
      end
      object Label11: TLabel
        Left = 45
        Top = 279
        Width = 114
        Height = 14
        Caption = '% Grasa Intramuscular:'
      end
      object Label12: TLabel
        Left = 61
        Top = 302
        Width = 100
        Height = 14
        Caption = '% Cortes Minoristas:'
      end
      object Caracteristicas: TLabel
        Left = 73
        Top = 24
        Width = 85
        Height = 13
        Caption = 'Caracteristicas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label14: TLabel
        Left = 185
        Top = 24
        Width = 77
        Height = 13
        Caption = 'Rango V'#225'lido'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label16: TLabel
        Left = 280
        Top = 24
        Width = 53
        Height = 13
        Caption = 'Promedio'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label18: TLabel
        Left = 349
        Top = 24
        Width = 71
        Height = 13
        Caption = 'DEP M'#237'nimo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label19: TLabel
        Left = 435
        Top = 24
        Width = 72
        Height = 13
        Caption = 'DEP M'#225'ximo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label20: TLabel
        Left = 530
        Top = 15
        Width = 83
        Height = 26
        Alignment = taCenter
        Caption = 'PREC M'#237'nima [0.00 a 0.99]'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        WordWrap = True
      end
      object Label13: TLabel
        Left = 195
        Top = 52
        Width = 52
        Height = 14
        Caption = '-3.3 a +3.9'
      end
      object LPromGest: TLabel
        Left = 280
        Top = 52
        Width = 53
        Height = 14
        Caption = 'LPromGest'
      end
      object LPromNac: TLabel
        Left = 280
        Top = 75
        Width = 49
        Height = 14
        Caption = 'LPromNac'
      end
      object LPromDest: TLabel
        Left = 280
        Top = 98
        Width = 52
        Height = 14
        Caption = 'LPromDest'
      end
      object LPromLeche: TLabel
        Left = 280
        Top = 121
        Width = 60
        Height = 14
        Caption = 'LPromLeche'
      end
      object LPromFinal: TLabel
        Left = 280
        Top = 143
        Width = 52
        Height = 14
        Caption = 'LPromFinal'
      end
      object LPromCirc: TLabel
        Left = 280
        Top = 166
        Width = 49
        Height = 14
        Caption = 'LPromCirc'
      end
      object LPromAltura: TLabel
        Left = 280
        Top = 190
        Width = 59
        Height = 14
        Caption = 'LPromAltura'
      end
      object LPromEGD: TLabel
        Left = 280
        Top = 212
        Width = 51
        Height = 14
        Caption = 'LPromEGD'
      end
      object LPromEGC: TLabel
        Left = 280
        Top = 234
        Width = 51
        Height = 14
        Caption = 'LPromEGC'
      end
      object LPromAOB: TLabel
        Left = 280
        Top = 256
        Width = 53
        Height = 14
        Caption = 'LPromAOB'
      end
      object LPromGI: TLabel
        Left = 280
        Top = 279
        Width = 40
        Height = 14
        Caption = 'LPromGI'
      end
      object LPromCM: TLabel
        Left = 280
        Top = 302
        Width = 45
        Height = 14
        Caption = 'LPromCM'
      end
      object Label15: TLabel
        Left = 195
        Top = 75
        Width = 52
        Height = 14
        Caption = '-5.1 a +4.5'
      end
      object Label17: TLabel
        Left = 188
        Top = 98
        Width = 64
        Height = 14
        Caption = '-19.7 a +31.2'
      end
      object Label21: TLabel
        Left = 188
        Top = 121
        Width = 64
        Height = 14
        Caption = '-18.7 a +16.6'
      end
      object Label22: TLabel
        Left = 188
        Top = 143
        Width = 64
        Height = 14
        Caption = '-52.3 a +51.1'
      end
      object Label23: TLabel
        Left = 195
        Top = 166
        Width = 52
        Height = 14
        Caption = '-2.2 a +4.1'
      end
      object Label24: TLabel
        Left = 195
        Top = 190
        Width = 52
        Height = 14
        Caption = '-3.5 a +7.3'
      end
      object Label25: TLabel
        Left = 195
        Top = 212
        Width = 52
        Height = 14
        Caption = '-1.2 a +3.2'
      end
      object Label26: TLabel
        Left = 195
        Top = 234
        Width = 52
        Height = 14
        Caption = '-2.4 a +4.5'
      end
      object Label27: TLabel
        Left = 188
        Top = 256
        Width = 64
        Height = 14
        Caption = '-10.0 a +10.0'
      end
      object Label28: TLabel
        Left = 195
        Top = 279
        Width = 52
        Height = 14
        Caption = '-0.4 a +0.8'
      end
      object Label29: TLabel
        Left = 196
        Top = 302
        Width = 52
        Height = 14
        Caption = '-3.5 a +2.6'
      end
      object Shape30: TShape
        Left = 6
        Top = 252
        Width = 586
        Height = 2
      end
      object Shape32: TShape
        Left = 5
        Top = 275
        Width = 586
        Height = 2
      end
      object Shape33: TShape
        Left = 5
        Top = 297
        Width = 586
        Height = 2
      end
      object Shape34: TShape
        Left = 6
        Top = 229
        Width = 586
        Height = 2
      end
      object Shape35: TShape
        Left = 6
        Top = 207
        Width = 586
        Height = 2
      end
      object Shape36: TShape
        Left = 6
        Top = 184
        Width = 586
        Height = 2
      end
      object Shape37: TShape
        Left = 6
        Top = 162
        Width = 586
        Height = 2
      end
      object Shape38: TShape
        Left = 6
        Top = 139
        Width = 586
        Height = 2
      end
      object Shape29: TShape
        Left = 6
        Top = 116
        Width = 586
        Height = 2
      end
      object Shape31: TShape
        Left = 6
        Top = 94
        Width = 586
        Height = 2
      end
      object Shape39: TShape
        Left = 6
        Top = 71
        Width = 586
        Height = 2
      end
      object MinNac: TEditAuto
        Tag = 4
        Left = 360
        Top = 71
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 0
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxNac: TEditAuto
        Tag = 5
        Left = 446
        Top = 71
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 1
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecNac: TEditAuto
        Tag = 6
        Left = 541
        Top = 71
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 2
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecLeche: TEditAuto
        Tag = 12
        Left = 541
        Top = 116
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 3
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinDest: TEditAuto
        Tag = 7
        Left = 360
        Top = 94
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 4
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecDest: TEditAuto
        Tag = 9
        Left = 541
        Top = 94
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 5
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinLeche: TEditAuto
        Tag = 10
        Left = 360
        Top = 116
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 6
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxLeche: TEditAuto
        Tag = 11
        Left = 446
        Top = 116
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 7
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxDest: TEditAuto
        Tag = 8
        Left = 446
        Top = 94
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 8
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinFinal: TEditAuto
        Tag = 13
        Left = 360
        Top = 139
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 9
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxFinal: TEditAuto
        Tag = 14
        Left = 446
        Top = 139
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 10
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecFinal: TEditAuto
        Tag = 15
        Left = 541
        Top = 139
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 11
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinCirc: TEditAuto
        Tag = 16
        Left = 360
        Top = 162
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 12
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxCirc: TEditAuto
        Tag = 17
        Left = 446
        Top = 162
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 13
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecCirc: TEditAuto
        Tag = 18
        Left = 541
        Top = 162
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 14
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecAOB: TEditAuto
        Tag = 30
        Left = 541
        Top = 252
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 15
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecCM: TEditAuto
        Tag = 36
        Left = 541
        Top = 297
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 16
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxCM: TEditAuto
        Tag = 35
        Left = 446
        Top = 297
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 17
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecEGC: TEditAuto
        Tag = 27
        Left = 541
        Top = 229
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 18
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecGI: TEditAuto
        Tag = 33
        Left = 541
        Top = 275
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 19
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinCM: TEditAuto
        Tag = 34
        Left = 360
        Top = 297
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 20
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinEGD: TEditAuto
        Tag = 22
        Left = 360
        Top = 207
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 21
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxGI: TEditAuto
        Tag = 32
        Left = 446
        Top = 275
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 22
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinGI: TEditAuto
        Tag = 31
        Left = 360
        Top = 275
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 23
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxEGD: TEditAuto
        Tag = 23
        Left = 446
        Top = 207
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 24
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxAOB: TEditAuto
        Tag = 29
        Left = 446
        Top = 252
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 25
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinAOB: TEditAuto
        Tag = 28
        Left = 360
        Top = 252
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 26
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecEGD: TEditAuto
        Tag = 24
        Left = 541
        Top = 207
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 27
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxEGC: TEditAuto
        Tag = 26
        Left = 446
        Top = 229
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 28
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinEGC: TEditAuto
        Tag = 25
        Left = 360
        Top = 229
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 29
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinGes: TEditAuto
        Tag = 1
        Left = 360
        Top = 48
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 30
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxGes: TEditAuto
        Tag = 2
        Left = 446
        Top = 48
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 31
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecGes: TEditAuto
        Tag = 3
        Left = 541
        Top = 48
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 32
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object PrecAltura: TEditAuto
        Tag = 21
        Left = 541
        Top = 184
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 33
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MinAltura: TEditAuto
        Tag = 19
        Left = 360
        Top = 184
        Width = 52
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 34
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
      object MaxAltura: TEditAuto
        Tag = 20
        Left = 446
        Top = 184
        Width = 53
        Height = 22
        CharCase = ecUpperCase
        Color = clWhite
        TabOrder = 35
        OnExit = ParametrosExit
        EsClave = False
        EsRequerido = False
        ISParam = False
      end
    end
  end
  object PBotones: TPanel [7]
    Left = 0
    Top = 591
    Width = 963
    Height = 85
    Align = alBottom
    Color = clWhite
    TabOrder = 5
    DesignSize = (
      963
      85)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 961
      Height = 83
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 880
      Top = 11
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = BBSalirClick
    end
    object LSalir: TLabel
      Left = 891
      Top = 64
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBSalirClick
    end
    object IExpExcel: TImage
      Left = 153
      Top = 8
      Width = 54
      Height = 54
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        AE426082}
      Visible = False
    end
    object IPreview: TImage
      Left = 309
      Top = 4
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = BBImprimirReporteClick
    end
    object LPreview: TLabel
      Left = 306
      Top = 68
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = BBImprimirReporteClick
    end
    object IAyuda: TImage
      Left = 16
      Top = 8
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 20
      Top = 65
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object Label87: TLabel
      Left = 131
      Top = 64
      Width = 97
      Height = 16
      Cursor = crHandPoint
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 854
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 963
    Caption = 'Ranking de DEPs'
    inherited LTitulo: TLabel
      Width = 559
    end
    inherited PILeft: TPanel
      Left = 755
    end
  end
  inherited Psocalo: TPanel
    Top = 676
    Width = 963
  end
  inherited ALUniversal: TActionList
    Left = 11
    Top = 6
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 244
    Top = 23
  end
  inherited SDXLS: TSaveDialog
    Left = 190
    Top = 5
  end
  inherited DTXLS: TDataToXLS
    Left = 752
    Top = 9
  end
  inherited WinXP1: TWinXP
    Left = 432
    Top = 0
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 348
    Top = 15
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 500
    Top = 7
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 580
    Top = 7
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 396
    Top = 7
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 292
    Top = 65527
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 468
    Top = 7
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 120
    Top = 8
  end
  object DSAnimales: TDataSource
    DataSet = IBQDeps
    Left = 40
    Top = 8
  end
  object IBQDeps: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQDepsAfterOpen
    BeforeOpen = IBQDepsBeforeOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT * FROM REP_RANKING_DEPS(:ESTA,'
      '  :MIN_GESTDEP,'
      '  :MIN_NACERDEP,'
      '  :MIN_DESTDEP,'
      '  :MIN_LECHEDEP,'
      '  :MIN_FINALDEP,'
      '  :MIN_CEDEP,'
      '  :MIN_ALTURADEP,'
      '  :MIN_EGDDEP,'
      '  :MIN_EGCDEP,'
      '  :MIN_AOBDEP,'
      '  :MIN_PORGIDEP,'
      '  :MIN_PORCMDEP,'
      '  :MAX_GESTDEP,'
      '  :MAX_NACERDEP,'
      '  :MAX_DESTDEP,'
      '  :MAX_LECHEDEP,'
      '  :MAX_FINALDEP,'
      '  :MAX_CEDEP,'
      '  :MAX_ALTURADEP,'
      '  :MAX_EGDDEP,'
      '  :MAX_EGCDEP,'
      '  :MAX_AOBDEP,'
      '  :MAX_PORGIDEP,'
      '  :MAX_PORCMDEP,'
      '  :GESTPREC_MIN,'
      '  :NACERPREC_MIN,'
      '  :DESTPREC_MIN,'
      '  :LECHEPREC_MIN,'
      '  :FINALPREC_MIN,'
      '  :CEPREC_MIN,'
      '  :ALTURAPREC_MIN,'
      '  :EGDPREC_MIN,'
      '  :EGCPREC_MIN,'
      '  :AOBPREC_MIN,'
      '  :PORGIPREC_MIN,'
      '  :PORCMPREC_MIN)')
    Left = 72
    Top = 8
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ESTA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_GESTDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_NACERDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_DESTDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_LECHEDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_FINALDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_CEDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_ALTURADEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_EGDDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_EGCDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_AOBDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_PORGIDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MIN_PORCMDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_GESTDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_NACERDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_DESTDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_LECHEDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_FINALDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_CEDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_ALTURADEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_EGDDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_EGCDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_AOBDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_PORGIDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'MAX_PORCMDEP'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'GESTPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'NACERPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DESTPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'LECHEPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FINALPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CEPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ALTURAPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'EGDPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'EGCPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'AOBPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'PORGIPREC_MIN'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'PORCMPREC_MIN'
        ParamType = ptUnknown
      end>
    object IBQDepsANIO: TIntegerField
      FieldName = 'ANIO'
      Origin = 'REP_RANKING_DEPS.ANIO'
    end
    object IBQDepsANIMALRET: TIntegerField
      FieldName = 'ANIMALRET'
      Origin = 'REP_RANKING_DEPS.ANIMALRET'
    end
    object IBQDepsANIMALEXTRET: TIntegerField
      FieldName = 'ANIMALEXTRET'
      Origin = 'REP_RANKING_DEPS.ANIMALEXTRET'
    end
    object IBQDepsESTABLECIMIENTO: TIntegerField
      FieldName = 'ESTABLECIMIENTO'
      Origin = 'REP_RANKING_DEPS.ESTABLECIMIENTO'
    end
    object IBQDepsNOMBRE_ANIMAL: TIBStringField
      FieldName = 'NOMBRE_ANIMAL'
      Origin = 'REP_RANKING_DEPS.NOMBRE_ANIMAL'
      Size = 100
    end
    object IBQDepsID_HBA: TIBStringField
      FieldName = 'ID_HBA'
      Origin = 'REP_RANKING_DEPS.ID_HBA'
      Size = 10
    end
    object IBQDepsNACIMIENTO: TIntegerField
      FieldName = 'NACIMIENTO'
      Origin = 'REP_RANKING_DEPS.NACIMIENTO'
    end
    object IBQDepsGESTDEP: TIBBCDField
      FieldName = 'GESTDEP'
      Origin = 'REP_RANKING_DEPS.GESTDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsNACERDEP: TIBBCDField
      FieldName = 'NACERDEP'
      Origin = 'REP_RANKING_DEPS.NACERDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsDESTDEP: TIBBCDField
      FieldName = 'DESTDEP'
      Origin = 'REP_RANKING_DEPS.DESTDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsLECHEDEP: TIBBCDField
      FieldName = 'LECHEDEP'
      Origin = 'REP_RANKING_DEPS.LECHEDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsFINALDEP: TIBBCDField
      FieldName = 'FINALDEP'
      Origin = 'REP_RANKING_DEPS.FINALDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsCEDEP: TIBBCDField
      FieldName = 'CEDEP'
      Origin = 'REP_RANKING_DEPS.CEDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsALTURADEP: TIBBCDField
      FieldName = 'ALTURADEP'
      Origin = 'REP_RANKING_DEPS.ALTURADEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsEGDDEP: TIBBCDField
      FieldName = 'EGDDEP'
      Origin = 'REP_RANKING_DEPS.EGDDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsEGCDEP: TIBBCDField
      FieldName = 'EGCDEP'
      Origin = 'REP_RANKING_DEPS.EGCDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsAOBDEP: TIBBCDField
      FieldName = 'AOBDEP'
      Origin = 'REP_RANKING_DEPS.AOBDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsPORGIDEP: TIBBCDField
      FieldName = 'PORGIDEP'
      Origin = 'REP_RANKING_DEPS.PORGIDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsPORCMDEP: TIBBCDField
      FieldName = 'PORCMDEP'
      Origin = 'REP_RANKING_DEPS.PORCMDEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsGESTPREC: TIBBCDField
      FieldName = 'GESTPREC'
      Origin = 'REP_RANKING_DEPS.GESTPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsNACERPREC: TIBBCDField
      FieldName = 'NACERPREC'
      Origin = 'REP_RANKING_DEPS.NACERPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsDESTPREC: TIBBCDField
      FieldName = 'DESTPREC'
      Origin = 'REP_RANKING_DEPS.DESTPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsLECHEPREC: TIBBCDField
      FieldName = 'LECHEPREC'
      Origin = 'REP_RANKING_DEPS.LECHEPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsFINALPREC: TIBBCDField
      FieldName = 'FINALPREC'
      Origin = 'REP_RANKING_DEPS.FINALPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsCEPREC: TIBBCDField
      FieldName = 'CEPREC'
      Origin = 'REP_RANKING_DEPS.CEPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsALTURAPREC: TIBBCDField
      FieldName = 'ALTURAPREC'
      Origin = 'REP_RANKING_DEPS.ALTURAPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsEGDPREC: TIBBCDField
      FieldName = 'EGDPREC'
      Origin = 'REP_RANKING_DEPS.EGDPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsEGCPREC: TIBBCDField
      FieldName = 'EGCPREC'
      Origin = 'REP_RANKING_DEPS.EGCPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsAOBPREC: TIBBCDField
      FieldName = 'AOBPREC'
      Origin = 'REP_RANKING_DEPS.AOBPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsPORGIPREC: TIBBCDField
      FieldName = 'PORGIPREC'
      Origin = 'REP_RANKING_DEPS.PORGIPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsPORCMPREC: TIBBCDField
      FieldName = 'PORCMPREC'
      Origin = 'REP_RANKING_DEPS.PORCMPREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsCRIAS: TIntegerField
      FieldName = 'CRIAS'
      Origin = 'REP_RANKING_DEPS.CRIAS'
    end
    object IBQDepsRODEO: TIntegerField
      FieldName = 'RODEO'
      Origin = 'REP_RANKING_DEPS.RODEO'
    end
    object IBQDepsCRIADOR: TIBStringField
      FieldName = 'CRIADOR'
      Origin = 'REP_RANKING_DEPS.CRIADOR'
      Size = 50
    end
    object IBQDepsCODIGO_PROPIETARIO: TIBStringField
      FieldName = 'CODIGO_PROPIETARIO'
      Origin = 'REP_RANKING_DEPS.CODIGO_PROPIETARIO'
      Size = 10
    end
    object IBQDepsNHIJ: TIntegerField
      FieldName = 'NHIJ'
      Origin = 'REP_RANKING_DEPS.NHIJ'
    end
    object IBQDepsNHIJM: TIntegerField
      FieldName = 'NHIJM'
      Origin = 'REP_RANKING_DEPS.NHIJM'
    end
    object IBQDepsNHIJU: TIntegerField
      FieldName = 'NHIJU'
      Origin = 'REP_RANKING_DEPS.NHIJU'
    end
    object IBQDepsDATO_PD: TIBStringField
      FieldName = 'DATO_PD'
      Origin = 'REP_RANKING_DEPS.DATO_PD'
      Size = 10
    end
    object IBQDepsP18DEP: TIBBCDField
      FieldName = 'P18DEP'
      Origin = 'REP_RANKING_DEPS.P18DEP'
      Precision = 9
      Size = 2
    end
    object IBQDepsP18PREC: TIBBCDField
      FieldName = 'P18PREC'
      Origin = 'REP_RANKING_DEPS.P18PREC'
      Precision = 9
      Size = 2
    end
    object IBQDepsPADRE: TIBStringField
      FieldName = 'PADRE'
      Origin = 'REP_RANKING_DEPS.PADRE'
      Size = 100
    end
    object IBQDepsMADRE: TIBStringField
      FieldName = 'MADRE'
      Origin = 'REP_RANKING_DEPS.MADRE'
      Size = 100
    end
    object IBQDepsSEXO: TIntegerField
      FieldName = 'SEXO'
      Origin = 'REP_RANKING_DEPS.SEXO'
    end
    object IBQDepsFUENTE: TIBStringField
      FieldName = 'FUENTE'
      Origin = 'REP_RANKING_DEPS.FUENTE'
      Size = 100
    end
    object IBQDepsID_FUENTE: TIntegerField
      FieldName = 'ID_FUENTE'
      Origin = 'REP_RANKING_DEPS.ID_FUENTE'
    end
    object IBQDepsIDPADRE: TIntegerField
      FieldName = 'IDPADRE'
      Origin = 'REP_RANKING_DEPS.IDPADRE'
    end
    object IBQDepsIDMADRE: TIntegerField
      FieldName = 'IDMADRE'
      Origin = 'REP_RANKING_DEPS.IDMADRE'
    end
    object IBQDepsTIPOMADRE: TIBStringField
      FieldName = 'TIPOMADRE'
      Origin = 'REP_RANKING_DEPS.TIPOMADRE'
      Size = 1
    end
    object IBQDepsTIPOPADRE: TIBStringField
      FieldName = 'TIPOPADRE'
      Origin = 'REP_RANKING_DEPS.TIPOPADRE'
      Size = 1
    end
    object IBQDepsRP_ANIMAL: TIBStringField
      FieldName = 'RP_ANIMAL'
      Origin = 'REP_RANKING_DEPS.RP_ANIMAL'
      Size = 10
    end
  end
  object IBQFuentes: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQFuentesAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_fuentes_deps')
    Left = 540
  end
  object DSFuentes: TDataSource
    DataSet = IBQFuentes
    Left = 596
  end
  object IBQEstabAnimalExt: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select establecimiento from tab_animales_externos where id_anima' +
        'l_externo = :animal')
    Left = 833
    Top = 10
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'animal'
        ParamType = ptUnknown
      end>
  end
  object IBQPromediosDEPs: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select avg(gestdep) as promgest, avg(nacerdep) as promnacer, avg' +
        '(destdep) as promdest, avg(lechedep) as promleche, avg(finaldep)' +
        ' as promfinal, avg(cedep) as promce, avg(alturadep) as promaltur' +
        'a, avg(egddep) as promegd, avg(egcdep) as promegc, avg(aobdep) a' +
        's promaob, avg(porcmdep) as promcm, avg(porgidep) as promgi from' +
        ' rep_ranking_deps(:establecimiento, null, null, null, null, null' +
        ','
      
        '  null, null, null, null, null, null, null, null, null, null, nu' +
        'll, null, null, null, null,'
      
        '  null, null, null, null, null, null, null, null, null, null, nu' +
        'll, null, null, null,null, null)')
    Left = 659
    Top = 369
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'establecimiento'
        ParamType = ptUnknown
      end>
  end
end
