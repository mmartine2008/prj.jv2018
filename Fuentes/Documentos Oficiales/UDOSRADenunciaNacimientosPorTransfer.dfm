inherited FDOSRADenunciaNacimientosPorTransfer: TFDOSRADenunciaNacimientosPorTransfer
  Left = 280
  Top = 72
  Caption = 'FDOSRADenunciaNacimientosPorTransfer'
  ClientHeight = 699
  ClientWidth = 950
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 950
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 634
  end
  object GBFechas: TGroupBox [4]
    Left = 0
    Top = 65
    Width = 817
    Height = 86
    Align = alLeft
    Caption = ' Selecci'#243'n de fechas '
    TabOrder = 2
    object LabelDesde: TLabel
      Left = 9
      Top = 23
      Width = 31
      Height = 14
      Caption = 'Desde'
    end
    object LabelHasta: TLabel
      Left = 9
      Top = 54
      Width = 28
      Height = 14
      Caption = 'Hasta'
    end
    object DTPInicio: TDateTimePicker
      Left = 52
      Top = 17
      Width = 200
      Height = 21
      Date = 0.597731296293204600
      Time = 0.597731296293204600
      TabOrder = 0
      OnCloseUp = DTPInicioCloseUp
      OnKeyUp = DTPInicioKeyUp
    end
    object DTPFin: TDateTimePicker
      Left = 52
      Top = 52
      Width = 200
      Height = 21
      Date = 0.597731296293204600
      Time = 0.597731296293204600
      TabOrder = 1
      OnCloseUp = DTPInicioCloseUp
      OnKeyUp = DTPInicioKeyUp
    end
    object RGTipoReporte: TRadioGroup
      Left = 370
      Top = 7
      Width = 119
      Height = 73
      Caption = ' Tipo de Reporte  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = []
      ItemIndex = 0
      Items.Strings = (
        'SRA'
        'SRA Vac'#237'o')
      ParentFont = False
      TabOrder = 2
      Visible = False
      OnClick = RGTipoReporteClick
    end
  end
  object GroupBox3: TGroupBox [5]
    Left = 0
    Top = 151
    Width = 950
    Height = 53
    Align = alBottom
    TabOrder = 3
    object LabelRaza: TLabel
      Left = 777
      Top = 60
      Width = 61
      Height = 14
      Caption = 'C'#243'digo Raza'
    end
    object Label2: TLabel
      Left = 273
      Top = 23
      Width = 38
      Height = 14
      Caption = 'Criador:'
    end
    object LabelCriadorNumero: TLabel
      Left = 490
      Top = 23
      Width = 53
      Height = 14
      Caption = 'Criador N'#186':'
    end
    object Label1: TLabel
      Left = 10
      Top = 23
      Width = 28
      Height = 14
      Caption = 'Raza:'
    end
    object Label3: TLabel
      Left = 648
      Top = 23
      Width = 40
      Height = 14
      Caption = 'Folio N'#186':'
    end
    object Label4: TLabel
      Left = 792
      Top = 23
      Width = 29
      Height = 14
      Caption = 'L'#237'nea:'
    end
    object EARaza: TEditAuto
      Left = 778
      Top = 60
      Width = 92
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      MaxLength = 30
      TabOrder = 0
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object EACriador: TEditAuto
      Left = 313
      Top = 19
      Width = 152
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      MaxLength = 30
      TabOrder = 1
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object EACriadorNumero: TEditAuto
      Left = 548
      Top = 19
      Width = 75
      Height = 22
      CharCase = ecUpperCase
      Color = clWhite
      MaxLength = 30
      TabOrder = 2
      EsClave = False
      EsRequerido = False
      ISParam = False
    end
    object CBxRaza: TComboBox
      Left = 40
      Top = 19
      Width = 212
      Height = 22
      Cursor = crHandPoint
      Style = csDropDownList
      ItemHeight = 14
      TabOrder = 3
      OnChange = CBxRazaChange
      OnCloseUp = CBxRazaCloseUp
    end
    object EAFolio: TEdit
      Left = 691
      Top = 19
      Width = 78
      Height = 22
      TabOrder = 4
    end
    object EALinea: TEdit
      Left = 824
      Top = 19
      Width = 98
      Height = 22
      TabOrder = 5
    end
  end
  object GroupBox1: TGroupBox [6]
    Left = 0
    Top = 204
    Width = 950
    Height = 377
    Align = alBottom
    Caption = ' Animales [0] '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    object Panel1: TPanel
      Left = 2
      Top = 18
      Width = 946
      Height = 25
      Align = alTop
      TabOrder = 0
      object Shape1: TShape
        Left = 1
        Top = 1
        Width = 363
        Height = 23
        Align = alLeft
        Brush.Color = 12708528
      end
      object Shape2: TShape
        Left = 621
        Top = 1
        Width = 167
        Height = 23
        Align = alLeft
        Brush.Color = 7975173
      end
      object Shape3: TShape
        Left = 455
        Top = 1
        Width = 166
        Height = 23
        Align = alLeft
        Brush.Color = 13821650
      end
      object Shape4: TShape
        Left = 364
        Top = 1
        Width = 91
        Height = 23
        Align = alLeft
        Brush.Color = 7319666
      end
      object Label5: TLabel
        Left = 1
        Top = 3
        Width = 365
        Height = 25
        Alignment = taCenter
        AutoSize = False
        Caption = 'Datos del Producto'
        Color = 12708528
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = True
        Layout = tlCenter
      end
      object Label6: TLabel
        Left = 365
        Top = 1
        Width = 90
        Height = 25
        Alignment = taCenter
        AutoSize = False
        Caption = 'Receptora'
        Color = 7319666
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = True
        Layout = tlCenter
      end
      object Label7: TLabel
        Left = 455
        Top = 1
        Width = 169
        Height = 25
        Alignment = taCenter
        AutoSize = False
        Caption = 'Madre Biol'#243'gica'
        Color = 13821650
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Transparent = True
        Layout = tlCenter
      end
      object Label8: TLabel
        Left = 623
        Top = 1
        Width = 165
        Height = 25
        Alignment = taCenter
        AutoSize = False
        BiDiMode = bdLeftToRight
        Caption = 'Datos de Padre'
        Color = 7319666
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = False
        Transparent = True
        Layout = tlCenter
      end
    end
    object BitDBGrid1: TBitDBGrid
      Left = 2
      Top = 43
      Width = 946
      Height = 332
      Align = alClient
      DataSource = DSDenunciaNacimientos
      Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -13
      TitleFont.Name = 'Arial'
      TitleFont.Style = [fsBold]
      AllowIncSearch = False
      ColumnVersion = 0
      DefaultColumns = True
      DrawingEllipse = True
      DrawReadOnly = False
      EnterKey = ekNone
      FillFields = False
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      FooterType = foNone
      InternalDrag = False
      LongCaption = True
      MultiCaption = True
      RealTitleFont.Charset = DEFAULT_CHARSET
      RealTitleFont.Color = clWindowText
      RealTitleFont.Height = -8
      RealTitleFont.Name = 'MS Sans Serif'
      RealTitleFont.Style = []
      RunTimePropEditor = True
      ShowCellHint = True
      TitleBtn = tbNone
      Columns = <
        item
          Alignment = taCenter
          Color = 12708528
          Expanded = False
          FieldName = 'RPHIJO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Rp'
          Title.Color = 12708528
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12708528
          Expanded = False
          FieldName = 'NOMBRE'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nombre'
          Title.Color = 12708528
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 180
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12708528
          Expanded = False
          FieldName = 'FECHA_NAC'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Fecha Nac.'
          Title.Color = 12708528
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 7319666
          Expanded = False
          FieldName = 'RPRECEPTORA'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Rp'
          Title.Color = 7319666
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 13821650
          Expanded = False
          FieldName = 'RPMADRE'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Rp'
          Title.Color = 13821650
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 13821650
          Expanded = False
          FieldName = 'HBAMADRE'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'H.B.A.'
          Title.Color = 13821650
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 7975173
          Expanded = False
          FieldName = 'RPPADRE'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Rp'
          Title.Color = 7975173
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 7975173
          Expanded = False
          FieldName = 'HBAPADRE'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'H.B.A.'
          Title.Color = 7975173
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'FECHA_SERVICIO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Fecha Serv.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'FECHA_TRANSFERENCIA'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Fecha Transfer.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 100
          Visible = True
        end>
    end
  end
  object PBotones: TPanel [7]
    Left = 0
    Top = 581
    Width = 950
    Height = 96
    Align = alBottom
    Color = clWhite
    TabOrder = 5
    DesignSize = (
      950
      96)
    object IFondoBar: TImage
      Left = 1
      Top = 1
      Width = 948
      Height = 94
      Align = alClient
      Picture.Data = {
        
        E7E9797F08ABF9FD13F2BF8BDF8FC287FFD9}
      Stretch = True
    end
    object ISalir: TImage
      Left = 759
      Top = 16
      Width = 51
      Height = 63
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        000049454E44AE426082}
      OnClick = ISalirClick
    end
    object LSalir: TLabel
      Left = 770
      Top = 70
      Width = 26
      Height = 16
      Cursor = crHandPoint
      Caption = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object IPreview: TImage
      Left = 492
      Top = 11
      Width = 64
      Height = 64
      Cursor = crHandPoint
      AutoSize = True
      Picture.Data = {
        
        4E44AE426082}
      OnClick = IPreviewClick
    end
    object LPreview: TLabel
      Left = 490
      Top = 72
      Width = 69
      Height = 16
      Cursor = crHandPoint
      Caption = 'Vista Previa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object IAyuda: TImage
      Left = 103
      Top = 15
      Width = 42
      Height = 57
      AutoSize = True
      Picture.Data = {
        
        4ACE16D0FF02C3F45F516127121B0000000049454E44AE426082}
      OnClick = AAyudaExecute
    end
    object LAyuda: TLabel
      Left = 107
      Top = 72
      Width = 37
      Height = 16
      Cursor = crHandPoint
      Caption = 'Ayuda'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      OnClick = AAyudaExecute
    end
    object IExportExcel: TImage
      Left = 296
      Top = 15
      Width = 57
      Height = 57
      Cursor = crHandPoint
      Picture.Data = {
        
        AE426082}
      OnClick = IExportExcelClick
    end
    object LExportarExcel: TLabel
      Left = 275
      Top = 72
      Width = 103
      Height = 17
      Cursor = crHandPoint
      AutoSize = False
      Caption = 'Exportar a Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object BBImprimirReporte: TBitBtn
      Left = 279
      Top = 122
      Width = 122
      Height = 32
      Caption = 'Vista Preliminar'
      TabOrder = 0
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BBSalir: TBitBtn
      Left = 841
      Top = 114
      Width = 104
      Height = 32
      Anchors = [akTop, akRight]
      Caption = 'Salir'
      TabOrder = 1
      Glyph.Data = {
        
        C2EAEAEBFFFFFFFFFFFF}
    end
    object BBAyuda: TBitBtn
      Left = 8
      Top = 130
      Width = 32
      Height = 32
      Action = AAyuda
      TabOrder = 2
      Glyph.Data = {
        
        ECD8E9ECD8E9ECD8E9EC}
    end
    object BBXls: TBitBtn
      Left = 78
      Top = 130
      Width = 45
      Height = 32
      Enabled = False
      ParentShowHint = False
      
      TabOrder = 3
      Glyph.Data = {
        
        6E004F946B004D916800488D630042895F00317A4F00327C5100}
    end
  end
  inherited PTitulo: TPanel
    Width = 950
    Caption = 'Denuncia de Nacimientos por Transferencia Embrionaria'
    inherited LTitulo: TLabel
      Width = 546
      Caption = 'Denuncia de Nacimientos por Transferencia Embrionaria'
    end
    inherited PILeft: TPanel
      Left = 742
    end
  end
  inherited Psocalo: TPanel
    Top = 677
    Width = 950
    inherited JvSocalo: TJvImage
      Width = 950
    end
  end
  object GroupBox2: TGroupBox [10]
    Left = 817
    Top = 65
    Width = 133
    Height = 86
    Align = alClient
    Caption = 'Consulta'
    TabOrder = 6
    object JvIBotonBuscar: TJvImage
      Tag = 4
      Left = 51
      Top = 34
      Width = 97
      Height = 32
      Cursor = crHandPoint
      Picture.Data = {
        
        E9FF0056DF9FC352571EAFFFD9}
      OnClick = JvIBotonBuscarClick
    end
    object BConsulta: TButton
      Left = 64
      Top = 8
      Width = 65
      Height = 17
      Caption = 'Consultar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      Visible = False
      OnClick = BConsultaClick
    end
  end
  inherited ALUniversal: TActionList
    Left = 595
    Top = 86
  end
  inherited IBSPEstadisticas: TIBStoredProc
    Left = 44
    Top = 15
  end
  inherited SDXLS: TSaveDialog
    Left = 686
    Top = 85
  end
  inherited DTXLS: TDataToXLS
    DataSet = IBQRepDenunciaNacimientos
    Columns = <
      item
        DataField = 'RPHIJO'
        Title = 'Rp Producto'
        ColWidth = 4500
      end
      item
        DataField = 'NOMBRE'
        Title = 'Nombre Producto'
        ColWidth = 4500
      end
      item
        DataField = 'FECHA_NAC'
        Title = 'Fec. Nac. Producto'
        ColWidth = 4500
      end
      item
        DataField = 'RPRECEPTORA'
        Title = 'Rp Receptora'
        ColWidth = 4500
      end
      item
        DataField = 'RPMADRE'
        Title = 'Rp Madre'
        ColWidth = 4500
      end
      item
        DataField = 'HBAMADRE'
        Title = 'H.B.A. Madre'
        ColWidth = 4500
      end
      item
        DataField = 'RPPADRE'
        Title = 'Rp Padre'
        ColWidth = 4500
      end
      item
        DataField = 'HBAPADRE'
        Title = 'H.B.A. Padre'
        ColWidth = 4500
      end
      item
        DataField = 'FECHA_SERVICIO'
        Title = 'Fec. Serv.'
        ColWidth = 4500
      end
      item
        DataField = 'FECHA_TRANSFERENCIA'
        Title = 'Fec. Transfer.'
        ColWidth = 4500
      end>
    Left = 720
    Top = 89
  end
  inherited WinXP1: TWinXP
    Left = 656
    Top = 88
  end
  inherited IBSPBorrarTablasAuxiliares: TIBStoredProc
    Left = 20
    Top = 47
  end
  inherited IBSPFixEstadoLactacion: TIBStoredProc
    Left = 44
    Top = 47
  end
  inherited IBStoredFixIDTratamiento: TIBStoredProc
    Left = 108
    Top = 15
  end
  inherited IBSPGeneradores: TIBStoredProc
    Left = 76
    Top = 47
  end
  inherited IBSPFixTabEstablecimientoPais: TIBStoredProc
    Left = 12
    Top = 15
  end
  inherited IBSPFIXRelGruposEventos: TIBStoredProc
    Left = 76
    Top = 15
  end
  inherited IBQGetCantAnimales: TIBQuery
    Left = 112
    Top = 48
  end
  inherited PLVersion: TPngImageList
    Left = 624
    Top = 88
  end
  object IBQRepDenunciaNacimientos: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select * from REP_DENUNCIA_NAC_SRA_TRANSFER(:esta, :desde, :hast' +
        'a, :vacio, :raza)')
    Left = 584
    Top = 89
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'esta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'desde'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'hasta'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'vacio'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
    object IBQRepDenunciaNacimientosIDMADRE: TIntegerField
      FieldName = 'IDMADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.IDMADRE'
    end
    object IBQRepDenunciaNacimientosID_RAZA: TIntegerField
      FieldName = 'ID_RAZA'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.ID_RAZA'
    end
    object IBQRepDenunciaNacimientosRPMADRE: TIBStringField
      FieldName = 'RPMADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.RPMADRE'
      Size = 10
    end
    object IBQRepDenunciaNacimientosHBAMADRE: TIBStringField
      FieldName = 'HBAMADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.HBAMADRE'
      Size = 10
    end
    object IBQRepDenunciaNacimientosNOMBREMADRE: TIBStringField
      FieldName = 'NOMBREMADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NOMBREMADRE'
      Size = 150
    end
    object IBQRepDenunciaNacimientosRPPADRE: TIBStringField
      FieldName = 'RPPADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.RPPADRE'
      Size = 10
    end
    object IBQRepDenunciaNacimientosHBAPADRE: TIBStringField
      FieldName = 'HBAPADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.HBAPADRE'
      Size = 10
    end
    object IBQRepDenunciaNacimientosNOMBREPADRE: TIBStringField
      FieldName = 'NOMBREPADRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NOMBREPADRE'
      Size = 150
    end
    object IBQRepDenunciaNacimientosRPRECEPTORA: TIBStringField
      FieldName = 'RPRECEPTORA'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.RPRECEPTORA'
      Size = 10
    end
    object IBQRepDenunciaNacimientosDIAI: TIntegerField
      FieldName = 'DIAI'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.DIAI'
    end
    object IBQRepDenunciaNacimientosMESI: TIntegerField
      FieldName = 'MESI'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.MESI'
    end
    object IBQRepDenunciaNacimientosANIOI: TIntegerField
      FieldName = 'ANIOI'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.ANIOI'
    end
    object IBQRepDenunciaNacimientosDIAF: TIntegerField
      FieldName = 'DIAF'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.DIAF'
    end
    object IBQRepDenunciaNacimientosMESF: TIntegerField
      FieldName = 'MESF'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.MESF'
    end
    object IBQRepDenunciaNacimientosANIOF: TIntegerField
      FieldName = 'ANIOF'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.ANIOF'
    end
    object IBQRepDenunciaNacimientosDIAS: TIntegerField
      FieldName = 'DIAS'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.DIAS'
    end
    object IBQRepDenunciaNacimientosMESS: TIntegerField
      FieldName = 'MESS'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.MESS'
    end
    object IBQRepDenunciaNacimientosANIOS: TIntegerField
      FieldName = 'ANIOS'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.ANIOS'
    end
    object IBQRepDenunciaNacimientosIDHIJO: TIntegerField
      FieldName = 'IDHIJO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.IDHIJO'
    end
    object IBQRepDenunciaNacimientosRPHIJO: TIBStringField
      FieldName = 'RPHIJO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.RPHIJO'
      Size = 10
    end
    object IBQRepDenunciaNacimientosNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NOMBRE'
      Size = 100
    end
    object IBQRepDenunciaNacimientosTIPO_PARTO: TIntegerField
      FieldName = 'TIPO_PARTO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.TIPO_PARTO'
    end
    object IBQRepDenunciaNacimientosPESO_NACIMIENTO_1: TFloatField
      FieldName = 'PESO_NACIMIENTO_1'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.PESO_NACIMIENTO_1'
    end
    object IBQRepDenunciaNacimientosPESO_NACIMIENTO_PROM: TFloatField
      FieldName = 'PESO_NACIMIENTO_PROM'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.PESO_NACIMIENTO_PROM'
    end
    object IBQRepDenunciaNacimientosSEXO_1: TIntegerField
      FieldName = 'SEXO_1'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.SEXO_1'
    end
    object IBQRepDenunciaNacimientosNOMBRESEXO: TIBStringField
      FieldName = 'NOMBRESEXO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NOMBRESEXO'
    end
    object IBQRepDenunciaNacimientosFECHA_NAC: TDateField
      FieldName = 'FECHA_NAC'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.FECHA_NAC'
    end
    object IBQRepDenunciaNacimientosCOLOR: TIntegerField
      FieldName = 'COLOR'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.COLOR'
    end
    object IBQRepDenunciaNacimientosCRIA: TIBStringField
      FieldName = 'CRIA'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.CRIA'
      Size = 1
    end
    object IBQRepDenunciaNacimientosMELLIZO: TIntegerField
      FieldName = 'MELLIZO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.MELLIZO'
    end
    object IBQRepDenunciaNacimientosPREFIJO: TIBStringField
      FieldName = 'PREFIJO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.PREFIJO'
      Size = 10
    end
    object IBQRepDenunciaNacimientosNRO_REG_ASOC_M: TIBStringField
      FieldName = 'NRO_REG_ASOC_M'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NRO_REG_ASOC_M'
      Size = 10
    end
    object IBQRepDenunciaNacimientosNRO_REG_ASOC_P: TIBStringField
      FieldName = 'NRO_REG_ASOC_P'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.NRO_REG_ASOC_P'
      Size = 10
    end
    object IBQRepDenunciaNacimientosCTRO_NRO: TIntegerField
      FieldName = 'CTRO_NRO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.CTRO_NRO'
    end
    object IBQRepDenunciaNacimientosCTRO_NOMBRE: TIBStringField
      FieldName = 'CTRO_NOMBRE'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.CTRO_NOMBRE'
      Size = 255
    end
    object IBQRepDenunciaNacimientosFECHA_TRANSFERENCIA: TDateField
      FieldName = 'FECHA_TRANSFERENCIA'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.FECHA_TRANSFERENCIA'
    end
    object IBQRepDenunciaNacimientosFECHA_SERVICIO: TDateField
      FieldName = 'FECHA_SERVICIO'
      Origin = 'REP_DENUNCIA_NAC_SRA_TRANSFER.FECHA_SERVICIO'
    end
  end
  object IBQGetCodigoRazaSra: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select cod_raza_sra from cod_razas_sra where id_raza_huella = :r' +
        'aza')
    Left = 736
    Top = 41
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
    object IBQGetCodigoRazaSraCOD_RAZA_SRA: TIntegerField
      FieldName = 'COD_RAZA_SRA'
      Origin = 'COD_RAZAS_SRA.COD_RAZA_SRA'
    end
  end
  object IBQGetRazas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_razas where visible = 1')
    Left = 744
    Top = 97
  end
  object DSDenunciaNacimientos: TDataSource
    DataSet = IBQRepDenunciaNacimientos
    Left = 616
    Top = 89
  end
  object IBQCodRaza: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select id_raza from cod_razas where sinonimo = :raza')
    Left = 640
    Top = 121
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'raza'
        ParamType = ptUnknown
      end>
  end
end
