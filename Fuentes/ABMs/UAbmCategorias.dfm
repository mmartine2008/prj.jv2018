inherited FABMCategorias: TFABMCategorias
  Left = 270
  Top = 50
  Caption = 'Huella - Categor'#237'as.'
  ClientHeight = 520
  ClientWidth = 663
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 663
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 279
  end
  inherited PBotones: TPanel
    Top = 402
    Width = 663
    inherited IFondo: TImage
      Width = 663
    end
    inherited IInsertar: TImage
      Height = 48
      Picture.Data = {
        
        990C5B570000000049454E44AE426082}
    end
    inherited BBGuardar: TBitBtn
      Left = 490
    end
    inherited BBCancelar: TBitBtn
      Left = 529
    end
    inherited BBSalir: TBitBtn
      Left = 577
    end
    inherited BBElegir: TBitBtn
      Left = 412
    end
  end
  inherited PEditar: TPanel
    Top = 218
    Width = 663
    Height = 184
    object LNombre: TLabel
      Left = 17
      Top = 28
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Nombre:'
      Layout = tlCenter
    end
    object LCodigo: TLabel
      Left = 17
      Top = 59
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'C'#243'digo:'
      Layout = tlCenter
    end
    object LSinonimo: TLabel
      Left = 17
      Top = 90
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'Sin'#243'nimo:'
      Layout = tlCenter
    end
    object LSexo: TLabel
      Left = 17
      Top = 122
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Sexo:'
      Layout = tlCenter
      Visible = False
    end
    object DBECodigo: TDBEditAuto
      Left = 86
      Top = 59
      Width = 79
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'CODIGO'
      DataSource = DSAbmSimple
      TabOrder = 1
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBENombre: TDBEditAuto
      Left = 86
      Top = 28
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBESinonimo: TDBEditAuto
      Left = 86
      Top = 90
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'SINONIMO'
      DataSource = DSAbmSimple
      TabOrder = 2
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
    object DBLCBASexo: TDBLookupComboBoxAuto
      Left = 86
      Top = 122
      Width = 156
      Height = 22
      Color = clMoneyGreen
      DataField = 'TIPO_SEXO'
      DataSource = DSAbmSimple
      KeyField = 'ID_TIPO_SEXO'
      ListField = 'NOMBRE'
      ListSource = DMSoftvet.DSTipoSexo
      TabOrder = 3
      Visible = False
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object RGCateg: TRadioGroup
      Left = 392
      Top = 16
      Width = 225
      Height = 145
      Caption = 'Pertenece a la Categor'#237'a'
      ItemIndex = 0
      Items.Strings = (
        'Ternero'
        'Novillo'
        'Toro'
        'Ternera'
        'Vaquillona'
        'Vaca')
      TabOrder = 4
    end
  end
  inherited PNavegar: TPanel
    inherited GBCantidad: TGroupBox
      inherited BDBGAbm: TBitDBGrid
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Caption = 'C'#243'digo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Width = 250
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SINONIMO'
            Title.Caption = 'Sin'#243'nimo'
            Width = 250
            Visible = True
          end>
      end
    end
  end
  inherited PTitulo: TPanel
    Width = 663
    Caption = 'Categor'#237'as de Animales'
    inherited LTitulo: TLabel
      Width = 259
    end
    inherited PILeft: TPanel
      Left = 455
    end
  end
  inherited Psocalo: TPanel
    Top = 479
    Width = 663
    inherited JvSocalo: TJvImage
      Width = 663
    end
  end
  inherited IBQAbmSimple: TIBQuery
    SQL.Strings = (
      
        'select id_categoria, codigo, nombre, sinonimo, tipo_sexo, ref_ca' +
        'teg'
      'from cod_categorias tp')
    GeneratorField.Field = 'ID_CATEGORIA'
    GeneratorField.Generator = 'GEN_ID_CODCATEGORIA'
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      '  NOMBRE,'
      '  ID_CATEGORIA,'
      '  SINONIMO,'
      '  TIPO_SEXO,'
      '  CODIGO,'
      '  REF_CATEG'
      'from COD_CATEGORIAS '
      'where'
      '  ID_CATEGORIA = :ID_CATEGORIA')
    ModifySQL.Strings = (
      'update COD_CATEGORIAS'
      'set'
      '  ID_CATEGORIA = :ID_CATEGORIA,'
      '  CODIGO = :CODIGO,'
      '  NOMBRE = :NOMBRE,'
      '  SINONIMO = :SINONIMO,'
      '  TIPO_SEXO = :TIPO_SEXO,'
      '  REF_CATEG = :REF_CATEG'
      'where'
      '  ID_CATEGORIA = :OLD_ID_CATEGORIA')
    InsertSQL.Strings = (
      'insert into COD_CATEGORIAS'
      '  (ID_CATEGORIA, CODIGO, NOMBRE, SINONIMO, TIPO_SEXO, REF_CATEG)'
      'values'
      
        '  (:ID_CATEGORIA, :CODIGO, :NOMBRE, :SINONIMO, :TIPO_SEXO, :REF_' +
        'CATEG)')
    DeleteSQL.Strings = (
      'delete from COD_CATEGORIAS'
      'where'
      '  ID_CATEGORIA = :OLD_ID_CATEGORIA')
  end
end
