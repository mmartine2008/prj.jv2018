inherited FABMCentrosInseminacion: TFABMCentrosInseminacion
  Left = 341
  Top = 124
  Caption = 'Huella - Centros de Inseminaci'#243'n.'
  ClientHeight = 517
  ClientWidth = 766
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Width = 766
    Picture.Data = {
      
      3B}
  end
  inherited Image4: TImage
    Width = 382
  end
  inherited PBotones: TPanel
    Top = 402
    Width = 766
    Height = 74
    inherited IFondo: TImage
      Width = 766
      Height = 74
    end
    inherited IAyuda: TImage
      Top = 3
    end
    inherited LAyuda: TLabel
      Left = 18
    end
    inherited IInsertar: TImage
      Top = 0
      Height = 48
      Picture.Data = {
        
        990C5B570000000049454E44AE426082}
    end
    inherited IEliminar: TImage
      Top = 0
    end
    inherited LEliminar: TLabel
      Left = 160
    end
    inherited IEditar: TImage
      Left = 224
    end
    inherited LEditar: TLabel
      Left = 226
      Width = 30
      Caption = ' Editar'
    end
    inherited LBuscar: TLabel
      Width = 38
      Caption = ' Buscar'
    end
    inherited BBGuardar: TBitBtn
      Left = 594
    end
    inherited BBCancelar: TBitBtn
      Left = 680
    end
    inherited BBSalir: TBitBtn
      Left = 680
    end
    inherited BBElegir: TBitBtn
      Left = 508
    end
  end
  inherited PEditar: TPanel
    Top = 302
    Width = 766
    object LNombre: TLabel
      Left = 17
      Top = 52
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Nombre:'
      Layout = tlCenter
    end
    object LCodigo: TLabel
      Left = 17
      Top = 17
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'C'#243'digo:'
      Layout = tlCenter
    end
    object DBECodigo: TDBEditAuto
      Left = 86
      Top = 17
      Width = 79
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'CODIGO'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBENombre: TDBEditAuto
      Left = 86
      Top = 52
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 1
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  inherited PNavegar: TPanel
    Width = 719
    inherited GBBuscar: TGroupBox
      Width = 715
      inherited EValorBuscar: TEdit
        Width = 496
      end
    end
    inherited GBFiltrar: TGroupBox
      Width = 715
      inherited EValorFiltrar: TEdit
        Width = 491
      end
    end
    inherited GBCantidad: TGroupBox
      Width = 715
      inherited BDBGAbm: TBitDBGrid
        Width = 711
        ParentFont = False
        TitleFont.Height = -13
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Caption = 'C'#243'digo'
            Width = 215
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Width = 293
            Visible = True
          end>
      end
    end
  end
  inherited PTitulo: TPanel
    Width = 766
    Caption = 'Centros de Inseminaci'#243'n'
    inherited LTitulo: TLabel
      Width = 382
    end
    inherited PILeft: TPanel
      Left = 578
    end
  end
  inherited Psocalo: TPanel
    Top = 476
    Width = 766
  end
  inherited IBQAbmSimple: TIBQuery
    SQL.Strings = (
      'select tp.id_centro, tp.codigo, tp.nombre'
      'from cod_centros_inseminacion tp where (tp.id_centro<> 0)')
    GeneratorField.Field = 'ID_CENTRO'
    GeneratorField.Generator = 'GEN_ID_CODCENTROINSEMINACION'
    object IBQAbmSimpleID_CENTRO: TIntegerField
      FieldName = 'ID_CENTRO'
      Origin = 'COD_CENTROS_INSEMINACION.ID_CENTRO'
      Required = True
    end
    object IBQAbmSimpleCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = 'COD_CENTROS_INSEMINACION.CODIGO'
      Required = True
      Size = 5
    end
    object IBQAbmSimpleNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'COD_CENTROS_INSEMINACION.NOMBRE'
      Required = True
      Size = 50
    end
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      'from cod_centros_inseminacion '
      'where'
      '  ID_CENTRO = :ID_CENTRO')
    ModifySQL.Strings = (
      'update cod_centros_inseminacion'
      'set'
      '  ID_CENTRO = :ID_CENTRO,'
      '  CODIGO = :CODIGO,'
      '  NOMBRE = :NOMBRE'
      'where'
      '  ID_CENTRO = :OLD_ID_CENTRO')
    InsertSQL.Strings = (
      'insert into cod_centros_inseminacion'
      '  (ID_CENTRO, CODIGO, NOMBRE)'
      'values'
      '  (:ID_CENTRO, :CODIGO, :NOMBRE)')
    DeleteSQL.Strings = (
      'delete from cod_centros_inseminacion'
      'where'
      '  ID_CENTRO = :OLD_ID_CENTRO')
  end
end
