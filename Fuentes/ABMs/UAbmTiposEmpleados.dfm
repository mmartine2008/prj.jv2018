inherited FABMTiposEmpleados: TFABMTiposEmpleados
  Top = 244
  Caption = 'Huella - Tipos de Empleados.'
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Picture.Data = {
      
      3B}
  end
  inherited PNavegar: TPanel
    TabOrder = 1
    inherited GBCantidad: TGroupBox
      inherited BDBGAbm: TBitDBGrid
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Caption = 'C'#243'digo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SINONIMO'
            Title.Caption = 'Sin'#243'nimo'
            Visible = True
          end>
      end
    end
  end
  inherited PEditar: TPanel
    object LCodigo: TLabel
      Left = 17
      Top = 17
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'C'#243'digo:'
      Layout = tlCenter
    end
    object LNombre: TLabel
      Left = 17
      Top = 52
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Nombre:'
      Layout = tlCenter
    end
    object LSinonimo: TLabel
      Left = 17
      Top = 86
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'Sin'#243'nimo:'
      Layout = tlCenter
    end
    object DBECodigo: TDBEditAuto
      Left = 86
      Top = 17
      Width = 79
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'CODIGO'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBENombre: TDBEditAuto
      Left = 86
      Top = 52
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 1
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBESinonimo: TDBEditAuto
      Left = 86
      Top = 86
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'SINONIMO'
      DataSource = DSAbmSimple
      TabOrder = 2
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  inherited PTitulo: TPanel
    Caption = 'Tipos de Empleados'
  end
  inherited IBQAbmSimple: TIBQuery
    SQL.Strings = (
      'select id_tipos_empleados, codigo, nombre, sinonimo'
      'from cod_tipos_empleados tp')
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      'from cod_tipos_empleados '
      'where'
      '  ID_TIPOS_EMPLEADOS = :ID_TIPOS_EMPLEADOS and'
      '  CODIGO = :CODIGO and'
      '  NOMBRE = :NOMBRE and'
      '  SINONIMO = :SINONIMO')
    ModifySQL.Strings = (
      'update cod_tipos_empleados'
      'set'
      '  ID_TIPOS_EMPLEADOS = :ID_TIPOS_EMPLEADOS,'
      '  CODIGO = :CODIGO,'
      '  NOMBRE = :NOMBRE,'
      '  SINONIMO = :SINONIMO'
      'where'
      '  ID_TIPOS_EMPLEADOS = :OLD_ID_TIPOS_EMPLEADOS and'
      '  CODIGO = :OLD_CODIGO and'
      '  NOMBRE = :OLD_NOMBRE and'
      '  SINONIMO = :OLD_SINONIMO')
    InsertSQL.Strings = (
      'insert into cod_tipos_empleados'
      '  (ID_TIPOS_EMPLEADOS, CODIGO, NOMBRE, SINONIMO)'
      'values'
      '  (:ID_TIPOS_EMPLEADOS, :CODIGO, :NOMBRE, :SINONIMO)')
    DeleteSQL.Strings = (
      'delete from cod_tipos_empleados'
      'where'
      '  ID_TIPOS_EMPLEADOS = :OLD_ID_TIPOS_EMPLEADOS and'
      '  CODIGO = :OLD_CODIGO and'
      '  NOMBRE = :OLD_NOMBRE and'
      '  SINONIMO = :OLD_SINONIMO')
  end
end
