inherited FABMCronologiaDentaria: TFABMCronologiaDentaria
  Left = 268
  Top = 186
  Caption = 'Huella - Cronolog'#237'a Dentar'#237'a.'
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Picture.Data = {
      
      3B}
  end
  inherited PBotones: TPanel
    Top = 346
    inherited IInsertar: TImage
      Height = 48
      Picture.Data = {
        
        990C5B570000000049454E44AE426082}
    end
  end
  inherited PEditar: TPanel
    Top = 246
    object LCodigo: TLabel
      Left = 17
      Top = 17
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'C'#243'digo:'
      Layout = tlCenter
    end
    object LNombre: TLabel
      Left = 17
      Top = 52
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Nombre:'
      Layout = tlCenter
    end
    object LSinonimo: TLabel
      Left = 17
      Top = 86
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'Sin'#243'nimo:'
      Layout = tlCenter
    end
    object DBECodigo: TDBEditAuto
      Left = 86
      Top = 17
      Width = 79
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'CODIGO'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBENombre: TDBEditAuto
      Left = 86
      Top = 52
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 1
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBESinonimo: TDBEditAuto
      Left = 86
      Top = 86
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = clMoneyGreen
      DataField = 'SINONIMO'
      DataSource = DSAbmSimple
      TabOrder = 2
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  inherited PNavegar: TPanel
    inherited GBCantidad: TGroupBox
      inherited BDBGAbm: TBitDBGrid
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Caption = 'Codigo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Width = 250
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SINONIMO'
            Title.Caption = 'Sinonimo'
            Width = 270
            Visible = True
          end>
      end
    end
  end
  inherited PTitulo: TPanel
    Caption = 'Cronolog'#237'as Dentarias'
  end
  inherited Psocalo: TPanel
    Top = 423
  end
  inherited ALUniversal: TActionList
    Top = 56
  end
  inherited IlAbmSimple: TImageList
    Top = 54
  end
  inherited DSAbmSimple: TDataSource
    Top = 54
  end
  inherited PMGrilla: TPopupMenu
    Top = 54
  end
  inherited IBQAbmSimple: TIBQuery
    SQL.Strings = (
      'select id_cronologia_dentaria, codigo, nombre, sinonimo '
      'from cod_cronologias_dentarias tp')
    GeneratorField.Field = 'ID_CRONOLOGIA_DENTARIA'
    GeneratorField.Generator = 'GEN_ID_CODCRONOLOGIADENTARIA'
    Top = 56
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      '  NOMBRE,'
      '  SINONIMO,'
      '  ID_CRONOLOGIA_DENTARIA,'
      '  CODIGO'
      'from COD_CRONOLOGIAS_DENTARIAS '
      'where'
      '  ID_CRONOLOGIA_DENTARIA = :ID_CRONOLOGIA_DENTARIA')
    ModifySQL.Strings = (
      'update COD_CRONOLOGIAS_DENTARIAS'
      'set'
      '  ID_CRONOLOGIA_DENTARIA = :ID_CRONOLOGIA_DENTARIA,'
      '  CODIGO = :CODIGO,'
      '  NOMBRE = :NOMBRE,'
      '  SINONIMO = :SINONIMO'
      'where'
      '  ID_CRONOLOGIA_DENTARIA = :OLD_ID_CRONOLOGIA_DENTARIA')
    InsertSQL.Strings = (
      'insert into COD_CRONOLOGIAS_DENTARIAS'
      '  (ID_CRONOLOGIA_DENTARIA, CODIGO, NOMBRE, SINONIMO)'
      'values'
      '  (:ID_CRONOLOGIA_DENTARIA, :CODIGO, :NOMBRE, :SINONIMO)')
    DeleteSQL.Strings = (
      'delete from COD_CRONOLOGIAS_DENTARIAS'
      'where'
      '  ID_CRONOLOGIA_DENTARIA = :OLD_ID_CRONOLOGIA_DENTARIA')
    Top = 56
  end
end
