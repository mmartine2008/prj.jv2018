inherited FABMFuentesDeps: TFABMFuentesDeps
  Left = 242
  Top = 113
  Caption = ''
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Picture.Data = {
      
      3B}
  end
  inherited PBotones: TPanel
    inherited IInsertar: TImage
      Left = 91
      Height = 48
      Picture.Data = {
        
        990C5B570000000049454E44AE426082}
    end
  end
  inherited PEditar: TPanel
    object Label1: TLabel
      Left = 26
      Top = 34
      Width = 36
      Height = 14
      Caption = 'Fuente:'
    end
    object Label2: TLabel
      Left = 26
      Top = 80
      Width = 28
      Height = 14
      Caption = 'Raza:'
    end
    object DBEANombreFuente: TDBEditAuto
      Left = 74
      Top = 32
      Width = 366
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
    object DBLCBARaza: TDBLookupComboBoxAuto
      Left = 74
      Top = 78
      Width = 251
      Height = 22
      Color = 14679807
      DataField = 'RAZA'
      DataSource = DSAbmSimple
      KeyField = 'ID_RAZA'
      ListField = 'NOMBRE'
      ListSource = DSRaza
      TabOrder = 1
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  inherited PNavegar: TPanel
    inherited GBCantidad: TGroupBox
      inherited BDBGAbm: TBitDBGrid
        ParentFont = False
        TitleFont.Height = -13
        Columns = <
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Fuente'
            Width = 299
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_RAZA'
            Title.Caption = 'Raza'
            Visible = True
          end>
      end
    end
  end
  inherited PTitulo: TPanel
    Caption = 'Fuentes de DEPs'
    inherited LTitulo: TLabel
      Caption = 'Fuentes de DEPs'
    end
  end
  inherited IBQAbmSimple: TIBQuery
    BeforePost = IBQAbmSimpleBeforePost
    SQL.Strings = (
      'select * from cod_fuentes_deps ')
    GeneratorField.Field = 'ID_FUENTE_DEPS'
    GeneratorField.Generator = 'GEN_ID_FUENTE_DEPS'
    object IBQAbmSimpleID_FUENTE_DEPS: TIntegerField
      FieldName = 'ID_FUENTE_DEPS'
      Origin = 'COD_FUENTES_DEPS.ID_FUENTE_DEPS'
      Required = True
    end
    object IBQAbmSimpleNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'COD_FUENTES_DEPS.NOMBRE'
      Size = 100
    end
    object IBQAbmSimpleRAZA: TIntegerField
      FieldName = 'RAZA'
      Origin = 'COD_FUENTES_DEPS.RAZA'
    end
    object IBQAbmSimpleNOM_RAZA: TStringField
      FieldKind = fkLookup
      FieldName = 'NOM_RAZA'
      LookupDataSet = IBQRazas
      LookupKeyFields = 'ID_RAZA'
      LookupResultField = 'NOMBRE'
      KeyFields = 'RAZA'
      Size = 50
      Lookup = True
    end
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select *'
      'from cod_fuentes_deps '
      'where'
      '  ID_FUENTE_DEPS = :ID_FUENTE_DEPS')
    ModifySQL.Strings = (
      'update cod_fuentes_deps'
      'set'
      '  ID_FUENTE_DEPS = :ID_FUENTE_DEPS,'
      '  NOMBRE = :NOMBRE,'
      '  RAZA = :RAZA'
      'where'
      '  ID_FUENTE_DEPS = :OLD_ID_FUENTE_DEPS')
    InsertSQL.Strings = (
      'insert into cod_fuentes_deps'
      '  (ID_FUENTE_DEPS, NOMBRE, RAZA)'
      'values'
      '  (:ID_FUENTE_DEPS, :NOMBRE, :RAZA)')
    DeleteSQL.Strings = (
      'delete from cod_fuentes_deps'
      'where'
      '  ID_FUENTE_DEPS = :OLD_ID_FUENTE_DEPS')
  end
  object IBQRazas: TIBQuery
    Database = FPrincipal.IBDPrincipal
    Transaction = FPrincipal.IBTPrincipal
    AfterOpen = IBQRazasAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from cod_razas')
    Left = 320
    Top = 37
    object IBQRazasDIAS_GESTACION: TIntegerField
      FieldName = 'DIAS_GESTACION'
      Origin = 'COD_RAZAS.DIAS_GESTACION'
      Required = True
    end
    object IBQRazasID_RAZA: TIntegerField
      FieldName = 'ID_RAZA'
      Origin = 'COD_RAZAS.ID_RAZA'
      Required = True
    end
    object IBQRazasCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = 'COD_RAZAS.CODIGO'
      Required = True
      Size = 2
    end
    object IBQRazasNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Origin = 'COD_RAZAS.NOMBRE'
      Required = True
      Size = 50
    end
    object IBQRazasSINONIMO: TIBStringField
      FieldName = 'SINONIMO'
      Origin = 'COD_RAZAS.SINONIMO'
      Required = True
      Size = 50
    end
  end
  object DSRaza: TDataSource
    DataSet = IBQRazas
    Left = 352
    Top = 40
  end
end
