inherited FABMTiposBaja: TFABMTiposBaja
  Left = 270
  Top = 181
  Caption = 'Huella - Tipo de Baja.'
  PixelsPerInch = 96
  TextHeight = 14
  inherited ITope: TImage
    Picture.Data = {
      
      3B}
  end
  inherited PNavegar: TPanel
    TabOrder = 1
    inherited GBCantidad: TGroupBox
      inherited BDBGAbm: TBitDBGrid
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Caption = 'C'#243'digo'
            Width = 53
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Caption = 'Nombre'
            Width = 250
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SINONIMO'
            Title.Caption = 'Sin'#243'nimo'
            Width = 250
            Visible = True
          end>
      end
    end
  end
  inherited PEditar: TPanel
    Top = 213
    Height = 162
    object LNombre: TLabel
      Left = 17
      Top = 52
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Nombre:'
      Layout = tlCenter
    end
    object LCodigo: TLabel
      Left = 17
      Top = 17
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'C'#243'digo:'
      Layout = tlCenter
    end
    object LSinonimo: TLabel
      Left = 17
      Top = 86
      Width = 56
      Height = 23
      AutoSize = False
      Caption = 'Sin'#243'nimo:'
      Layout = tlCenter
    end
    object LMensaje: TLabel
      Left = 17
      Top = 121
      Width = 56
      Height = 22
      AutoSize = False
      Caption = 'Mensaje:'
      Layout = tlCenter
    end
    object DBECodigo: TDBEditAuto
      Left = 86
      Top = 17
      Width = 79
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'CODIGO'
      DataSource = DSAbmSimple
      TabOrder = 0
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBENombre: TDBEditAuto
      Left = 86
      Top = 52
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'NOMBRE'
      DataSource = DSAbmSimple
      TabOrder = 1
      EsClave = True
      EsRequerido = True
      ISParam = False
    end
    object DBESinonimo: TDBEditAuto
      Left = 86
      Top = 86
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'SINONIMO'
      DataSource = DSAbmSimple
      TabOrder = 2
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
    object DBEMensaje: TDBEditAuto
      Left = 86
      Top = 121
      Width = 263
      Height = 22
      CharCase = ecUpperCase
      Color = 14679807
      DataField = 'MSG_ESTADO'
      DataSource = DSAbmSimple
      TabOrder = 3
      EsClave = False
      EsRequerido = True
      ISParam = False
    end
  end
  inherited PTitulo: TPanel
    Caption = 'Tipos de Baja'
  end
  inherited IBQAbmSimple: TIBQuery
    SQL.Strings = (
      'select id_tipo_baja, codigo, nombre, sinonimo, msg_estado'
      'from cod_tipos_baja tp')
    GeneratorField.Field = 'ID_TIPO_BAJA'
    GeneratorField.Generator = 'GEN_ID_CODTIPOSBAJA'
  end
  inherited IBUSQLAbmSimple: TIBUpdateSQL
    RefreshSQL.Strings = (
      'Select '
      'from cod_tipos_baja '
      'where'
      '  ID_TIPO_BAJA = :ID_TIPO_BAJA')
    ModifySQL.Strings = (
      'update cod_tipos_baja'
      'set'
      '  ID_TIPO_BAJA = :ID_TIPO_BAJA,'
      '  CODIGO = :CODIGO,'
      '  NOMBRE = :NOMBRE,'
      '  SINONIMO = :SINONIMO,'
      '  MSG_ESTADO = :MSG_ESTADO'
      'where'
      '  ID_TIPO_BAJA = :OLD_ID_TIPO_BAJA')
    InsertSQL.Strings = (
      'insert into cod_tipos_baja'
      '  (ID_TIPO_BAJA, CODIGO, NOMBRE, SINONIMO, MSG_ESTADO)'
      'values'
      '  (:ID_TIPO_BAJA, :CODIGO, :NOMBRE, :SINONIMO, :MSG_ESTADO)')
    DeleteSQL.Strings = (
      'delete from cod_tipos_baja'
      'where'
      '  ID_TIPO_BAJA = :OLD_ID_TIPO_BAJA')
  end
end
