...
//  Caption := Traducir('Gestión de Bovinos de Carne . V3.51');

  if ( ServidorBD = '') and (dirBD = '') then
    FPrincipal.Caption := 'Huella. Gestión de Bovinos de Carne. '+h_version +' [Connected to: Default]'
  else
  if ( ServidorBD = '') and (dirBD <> '') then
    FPrincipal.Caption := 'Huella. Gestión de Bovinos de Carne. '+h_version+' [Connected to: '+dirBD+']'
  else
    FPrincipal.Caption := 'Huella. Gestión de Bovinos de Carne. '+h_version+' [Connected to:'+  ServidorBD+']';
...
begin
  ...
  begin
     MostrarMensaje(tmAdvertencia,'No hay potreros disponibles para la creación del servicio.');
...
begin
  ...
  AgregarAccion(AEveInseminacionArtificial.Name,'Inseminación Artificial',2,JvIEventos3N,JvFolEServicios,'IAR');
end;
 ...
 {$ELSE}
  ...
  begin
     MostrarMensaje(tmError, 'Base de Datos innacessible'+
                              ServidorBD + DirBD +']'+ 'La aplicacion se cerrará.');
     FPrincipal.Close();
     Application.Terminate();
  end;
...
begin
 ...
 { try
    ...
    begin
      ...
      begin
        {if (MostrarMensaje(tmConsulta,'La base de datos debe ser actualizada antes de ejecutarse el sistema. Desea descargar la actualización? ') = mrYes ) then
...
begin
  ...
  begin
     MostrarMensaje(tmInformacion, 'No disponible en esta versión');
  end
...
begin
  ...
  AgregarAccion(ARepPorcentajePrenez.Name,'Porcentaje Preñez',3,JvIReportes3N, JvFolRGestacion,'PPR');
end;
...
begin
  ...
  AgregarAccion(ARepDistribucionPrenez.Name,'Distribucion de Preñez',3,JvIReportes3N,JvFolRGestacion,'DDP');
end;
...
begin
  ...
  AgregarAccion(ARepVsPrenez.Name,'Comparacion de Preñez',3,JvIReportes3N,JvFolRGestacion,'CDP');
end;
...
begin
  ...
  AgregarAccion(ARepListadoEventos.Name,'Reportes Genéricos',3,nil,JvFolRGenericos,'');
end;
...
//nuevo codigo
  if (not verificarLlave()) then   //llave nuevo codigo
   begin
    TimerLlave.Enabled:= false;
    If (MostrarMensaje(tmConsulta, 'Usted ha retirado la llave de seguridad de Huella. Si la tiene coloquelá en su equipo.') = mrYes) then
      begin
       TimerLlave.Enabled:= true;
      end
     else
...
{ DAIANA - 19.12.2014 - Incidencia 413 - Fin }
  AgregarAccion(ADODocTransitoElectronico.Name,'Documento Tránsito Electrónico',3,JvIReportes3N,JvFolROficial,'DTE');
end;
...
begin
  ...
  AgregarAccion(AEveDeclaracionAFIP.Name,'Declaración AFIP',2,JvIEventos3N,JvFolECria,'DEA');
end;
...
begin
  ...
  try
    ...
    IBTPrincipal.Commit;          //TAL VEZ PONERLO MÁS ARRIBA

    IBSPCambiarCategoria.ExecProc; //cambia de categoría las terneras
...
// a los establecimientos que estén creados les agrega el pais en este caso argentina. Si desean pueden cambiar el Pais.
...
begin
  ...
  try
    begin
    //verifico si cerro bien la última vez que se usó Huella
    Ini := TIniFile.Create (copy(Application.ExeName, 0, length(Application.ExeName) -10)+'huella.ini');    
...
begin
  ...
  {$IFDEF DEMO}
    {$IFDEF EC}
      MostrarMensaje(tmInformacion,'Para actualizar la versión de Educación Continua envia un mail a sac@softhuella.com.ar y se le enviara la actualización');
    {$ELSE}
      MostrarMensaje(tmInformacion,'No se puede actualizar la versión Demo');
    {$ENDIF}
  {$ELSE}
    if IsConnected then
    begin
...
begin
  ...
  AgregarAccion(AImpERA.Name,'Importacion de Evaluación Genética',1,JvAnimales3N,JvFolDEPS,'IDE');
end;
...
begin
  ...
  begin
    ...
    case TP of
      ...
      ttmConfig : LEtiquetaPestaniaAnterior.Caption := 'Configuración';
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Busqueda[cant].Categoria.Caption := 'Códigos';
        end;
        ...
        begin
          Busqueda[cant].Icono.Picture := IBConfig.Picture;
          Busqueda[cant].Categoria.Caption := 'Configuración';
        end;
  ...
  begin
    ...
    if ABuscar = '' then
      Busqueda[0].Leyenda.Caption := 'No se ha ingresado ninguna referencia de búsqueda';
    if (Abuscar <> '') and (Length(ABuscar) < 3) then
      Busqueda[0].Leyenda.Caption := 'Debe ingresar al menos 3(tres) caracteres para realizar la búsqueda';
    if (Abuscar <> '') and (Length(ABuscar) >= 3) then
...
begin
  ...
  begin
    ...
    AgregarAccion(AConfigElectronico.Name,'Configuración del Lector Electrónico',4,nil,JvFolCaravanaElect,'');
  end
  else
  begin
    MostrarMensaje(tmInformacion, 'No disponible en esta versión');
...
begin
  ...
  begin
    ...
    F.ShowModal;
    F.Destroy;
    SendMessage(FindWindow(nil, 'Sistema de Administración de Clientes, Stock y Ventas'), WM_USER + 2, 1, 1);
  end;
end;
...
begin
  inherited;
  JvBHint.ActivateHint(JvISimulador,#13+'Simulador de Inversión'+#13);
end;
...
begin
  inherited;
  JvBHint.ActivateHint(JvIContable,#13+'Sistema de Administración de Clientes, Facturación y Stock'+#13);
end;
