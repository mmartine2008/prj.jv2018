...
BEGIN
    ...
    CAST(ULTIMO_PESO AS VARCHAR(7)), CAST(CIRCUNFERENCIA_ESCROTAL AS VARCHAR(7)),
          ESTADO_LACTACION, GDR, CAST(CONDICION_CORPORAL  AS VARCHAR(7)), SUBCATEGORIA, COLOR, NOMBRE, APODO, FECHA_NACIMIENTO, INFORMADO_AFIP
          ...
          :CANT_CRIAS, :ID_GDR, :ID_COND, :ID_SUBCAT, :ID_COLOR, :NOMBRE, :APODO, :FECHA_NAC, :INFO_AFIP_AUX;
     ...
     AUX_IDENT = APODO_AUX;
      END
          ...
          SELECT FIRST 1 ID_EVENTO, FECHA FROM EVE_EVENTOS WHERE ANIMAL = :ID_ANIMAL AND ESTABLECIMIENTO = :ID_ESTAB
          AND (TIPO = 27) AND ID_EVENTO <> :EVE_SANI ORDER BY FECHA DESC INTO :EVE_SANI_2, :FECHA_ULTIMO_TRAT;
          IF (EVE_SANI_2 IS NOT NULL) THEN
             SELECT CT.NOMBRE FROM EVE_TRATAMIENTO ET JOIN COD_TRATAMIENTO CT ON ET.ID_TRATAMIENTO = CT.ID_TRATAMIENTO
             WHERE ID_EVENTO = :EVE_SANI_2 INTO :ULTIMO_TRAT;
          --SERVICIOS
          TIPO_SERV = NULL;
          EVE_SERVICIO = NULL;
